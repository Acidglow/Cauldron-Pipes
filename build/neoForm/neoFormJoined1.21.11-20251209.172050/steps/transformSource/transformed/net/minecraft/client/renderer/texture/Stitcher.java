...
public class Stitcher<T extends Stitcher.Entry> {
    ...
    private static final Comparator<Stitcher.Holder<?>> HOLDER_COMPARATOR = Comparator.<Stitcher.Holder<?>, Integer>comparing(p_118201_ -> -p_118201_.height)
        ...
        .thenComparing(p_465724_ -> p_465724_.entry.name());
    ...
    public Stitcher(int maxWidth, int maxHeight, int mipLevel, int padding) {
        this.mipLevel = mipLevel;
        this.maxWidth = maxWidth;
        this.maxHeight = maxHeight;
        this.padding = 1 << mipLevel << Mth.clamp(padding - 1, 0, 4);
    }
    ...
    public void registerSprite(T stitcherEntry) {
        Stitcher.Holder<T> holder = new Stitcher.Holder<>(
            stitcherEntry,
            smallestFittingMinTexel(stitcherEntry.width() + this.padding * 2, this.mipLevel),
            smallestFittingMinTexel(stitcherEntry.height() + this.padding * 2, this.mipLevel)
        );
    ...
    public void gatherSprites(Stitcher.SpriteLoader<T> loader) {
        for (Stitcher.Region<T> region : this.storage) {
            region.walk(loader, this.padding);
        }
    }
    ...
    public static class Region<T extends Stitcher.Entry> {
        ...
        public void walk(Stitcher.SpriteLoader<T> loader, int padding) {
            if (this.holder != null) {
                loader.load(this.holder.entry, this.getX(), this.getY(), padding);
            } else if (this.subSlots != null) {
                for (Stitcher.Region<T> region : this.subSlots) {
                    region.walk(loader, padding);
                }
            }
        }
    ...
    public interface SpriteLoader<T extends Stitcher.Entry> {
        void load(T entry, int x, int y, int padding);
    }
}
