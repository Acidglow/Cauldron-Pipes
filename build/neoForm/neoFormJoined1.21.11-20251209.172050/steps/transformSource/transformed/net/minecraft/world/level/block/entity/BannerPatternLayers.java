...
public record BannerPatternLayers(List<BannerPatternLayers.Layer> layers) implements TooltipProvider {
    ...
    @Override
    public void addToTooltip(Item.TooltipContext p_400214_, Consumer<Component> p_400134_, TooltipFlag p_400157_, DataComponentGetter p_399701_) {
        for (int i = 0; i < Math.min(this.layers().size(), 6); i++) {
            p_400134_.accept(this.layers().get(i).description().withStyle(ChatFormatting.GRAY));
        }
    }
