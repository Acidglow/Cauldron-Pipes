...
public class CreateWorldScreen extends Screen {
    ...
    public static void testWorld(Minecraft minecraft, Runnable onClose) {
        WorldCreationContextMapper worldcreationcontextmapper = (p_460245_, p_460246_, p_460247_) -> new WorldCreationContext(
            p_460247_.worldGenSettings().options(),
            p_460247_.worldGenSettings().dimensions(),
            p_460246_,
            p_460245_,
            p_460247_.dataConfiguration(),
    ...
    @Override
    public boolean keyPressed(KeyEvent p_446101_) {
        if (this.tabNavigationBar.keyPressed(p_446101_)) {
            return true;
        } else if (super.keyPressed(p_446101_)) {
            return true;
        } else if (p_446101_.isConfirmation()) {
            this.onCreate();
            return true;
        } else {
            return false;
    ...
    public static Path createTempDataPackDirFromExistingWorld(Path datapackDir, Minecraft minecraft) {
        ...
        try (Stream<Path> stream = Files.walk(datapackDir)) {
            stream.filter(p_373677_ -> !p_373677_.equals(datapackDir)).forEach(p_460250_ -> {
                ...
                copyBetweenDirs(datapackDir, path, p_460250_);
            });
        } catch (UncheckedIOException | IOException ioexception) {
