...
public abstract class HangingEntity extends BlockAttachedEntity {
    ...
    @Override
    protected void defineSynchedData(SynchedEntityData.Builder p_425635_) {
        p_425635_.define(DATA_DIRECTION, DEFAULT_DIRECTION);
    }

    @Override
    public void onSyncedDataUpdated(EntityDataAccessor<?> p_426322_) {
        super.onSyncedDataUpdated(p_426322_);
        if (p_426322_.equals(DATA_DIRECTION)) {
    ...
    protected boolean canCoexist(boolean itemFrame) {
        Predicate<HangingEntity> predicate = p_445344_ -> {
            boolean flag = !itemFrame && p_445344_.getType() == this.getType();
            boolean flag1 = p_445344_.getDirection() == this.getDirection();
            return p_445344_ != this && (flag || flag1);
        };
        return !this.level().hasEntities(EntityTypeTest.forClass(HangingEntity.class), this.getPopBox(), predicate);
    }

    protected boolean hasLevelCollision(AABB boundingBox) {
        Level level = this.level();
        return !level.noBlockCollision(this, boundingBox) || !level.noBorderCollision(this, boundingBox);
    }
