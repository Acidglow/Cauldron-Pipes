...
public class WorldBorder extends SavedData {
    ...
    public void lerpSizeBetween(double from, double to, long duration, long begin) {
        this.extent = (WorldBorder.BorderExtent)(from == to
            ...
            : new WorldBorder.MovingBorderExtent(from, to, duration, begin));
        this.setDirty();

        for (BorderChangeListener borderchangelistener : this.getListeners()) {
            borderchangelistener.onLerpSize(this, from, to, duration, begin);
        }
    }
    ...
    class MovingBorderExtent implements WorldBorder.BorderExtent {
        ...
        MovingBorderExtent(double from, double to, long lerpDuration, long lerpBegin) {
            this.from = from;
            this.to = to;
        ...
        @Override
        public double getMinX(float p_458931_) {
            return Mth.clamp(
                WorldBorder.this.getCenterX() - Mth.lerp((double)p_458931_, this.getPreviousSize(), this.getSize()) / 2.0,
        ...
        @Override
        public double getMinZ(float p_459101_) {
            return Mth.clamp(
                WorldBorder.this.getCenterZ() - Mth.lerp((double)p_459101_, this.getPreviousSize(), this.getSize()) / 2.0,
                (double)(-WorldBorder.this.absoluteMaxSize),
                (double)WorldBorder.this.absoluteMaxSize
            );
        }
        ...
        @Override
        public double getMaxX(float p_459058_) {
            return Mth.clamp(
                WorldBorder.this.getCenterX() + Mth.lerp((double)p_459058_, this.getPreviousSize(), this.getSize()) / 2.0,
                (double)(-WorldBorder.this.absoluteMaxSize),
                (double)WorldBorder.this.absoluteMaxSize
            );
        }
        ...
        @Override
        public double getMaxZ(float p_458923_) {
            return Mth.clamp(
                WorldBorder.this.getCenterZ() + Mth.lerp((double)p_458923_, this.getPreviousSize(), this.getSize()) / 2.0,
                (double)(-WorldBorder.this.absoluteMaxSize),
                (double)WorldBorder.this.absoluteMaxSize
            );
        }
    ...
    class StaticBorderExtent implements WorldBorder.BorderExtent {
        ...
        @Override
        public double getMinX(float p_459077_) {
            return this.minX;
        }

        @Override
        public double getMaxX(float p_459090_) {
            return this.maxX;
        }

        @Override
        public double getMinZ(float p_459053_) {
            return this.minZ;
        }

        @Override
        public double getMaxZ(float p_459030_) {
            return this.maxZ;
        }
