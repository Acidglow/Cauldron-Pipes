...
public class BeaconBlockEntity extends BlockEntity implements MenuProvider, Nameable, BeaconBeamOwner {
    ...
    public static void tick(Level level, BlockPos pos, BlockState state, BeaconBlockEntity blockEntity) {
        ...
        BeaconBeamOwner.Section beaconbeamowner$section = blockEntity.checkingBeamSections.isEmpty()
            ? null
        ...
        for (int i1 = 0; i1 < 10 && blockpos.getY() <= l; i1++) {
            ...
            if (j1 != null) {
                if (blockEntity.checkingBeamSections.size() <= 1) {
                    ...
                    blockEntity.checkingBeamSections.add(beaconbeamowner$section);
    ...
    private static void storeEffect(ValueOutput output, String key, @Nullable Holder<MobEffect> effect) {
        if (effect != null) {
            effect.unwrapKey().ifPresent(p_466684_ -> output.putString(key, p_466684_.identifier().toString()));
        }
    }
    ...
    @Override
    protected void loadAdditional(ValueInput p_421952_) {
        super.loadAdditional(p_421952_);
        this.primaryPower = loadEffect(p_421952_, "primary_effect");
        this.secondaryPower = loadEffect(p_421952_, "secondary_effect");
        this.name = parseCustomNameSafe(p_421952_, "CustomName");
        this.lockKey = LockCode.fromTag(p_421952_);
    }

    @Override
    protected void saveAdditional(ValueOutput p_422018_) {
        super.saveAdditional(p_422018_);
        storeEffect(p_422018_, "primary_effect", this.primaryPower);
        storeEffect(p_422018_, "secondary_effect", this.secondaryPower);
        p_422018_.putInt("Levels", this.levels);
        p_422018_.storeNullable("CustomName", ComponentSerialization.CODEC, this.name);
        this.lockKey.addToTag(p_422018_);
    }
    ...
    @Override
    public void removeComponentsFromTag(ValueOutput p_421511_) {
        p_421511_.discard("CustomName");
        p_421511_.discard("lock");
    }
