...
public record AttributeType<Value>(
    ...
    public static <Value> AttributeType<Value> ofInterpolated(
        Codec<Value> valueCodec, Map<AttributeModifier.OperationId, AttributeModifier<Value, ?>> modifierLibrary, LerpFunction<Value> spatialLerp
    ) {
        return ofInterpolated(valueCodec, modifierLibrary, spatialLerp, spatialLerp);
    }
    ...
    private static <Value> Codec<AttributeModifier<Value, ?>> createModifierCodec(Map<AttributeModifier.OperationId, AttributeModifier<Value, ?>> modifierLibrary) {
