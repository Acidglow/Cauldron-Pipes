...
public class ChunkMap extends SimpleRegionStorage implements ChunkHolder.PlayerProvider, GeneratingChunkMap {
    ...
    /**
     * Same as {@link #updatingChunkMap}, but immutable for access from other threads. <em>This should never be mutated.</em>
     */
    ...
    protected void tick(BooleanSupplier hasTimeLeft) {
        ...
        if (!this.level.noSave()) {
            this.processUnloads(hasTimeLeft);
        }
    ...
    private void processUnloads(BooleanSupplier hasTimeLeft) {
        ...
        Runnable runnable;
        while ((j > 0 || hasTimeLeft.getAsBoolean()) && (runnable = this.unloadQueue.poll()) != null) {
            j--;
            runnable.run();
        }

        this.saveChunksEagerly(hasTimeLeft);
    }

    private void saveChunksEagerly(BooleanSupplier hasTimeLeft) {
        long i = Util.getMillis();
        int j = 0;
        LongIterator longiterator = this.chunksToEagerlySave.iterator();

        while (j < 20 && this.activeChunkWrites.get() < 128 && hasTimeLeft.getAsBoolean() && longiterator.hasNext()) {
        ...
        if (p_347638_.targetStatus() == ChunkStatus.EMPTY) {
            ...
            try {
                ...
                if (chunkaccess == null) {
                    ...
                    return p_347638_.apply(this.worldGenContext, p_347552_, chunkaccess);
                }
    ...
    public static CompoundTag getChunkDataFixContextTag(ResourceKey<Level> level, Optional<ResourceKey<MapCodec<? extends ChunkGenerator>>> generatorKey) {
        CompoundTag compoundtag = new CompoundTag();
        compoundtag.putString("dimension", level.identifier().toString());
        generatorKey.ifPresent(p_466334_ -> compoundtag.putString("generator", p_466334_.identifier().toString()));
        return compoundtag;
    }
    ...
    boolean anyPlayerCloseEnoughTo(BlockPos pos, int distance) {
        Vec3 vec3 = new Vec3(pos);

        for (ServerPlayer serverplayer : this.playerMap.getAllPlayers()) {
            if (this.playerIsCloseEnoughTo(serverplayer, vec3, distance)) {
                return true;
            }
        }
    ...
    private boolean playerIsCloseEnoughTo(ServerPlayer player, Vec3 pos, int distance) {
        if (player.isSpectator()) {
            return false;
        } else {
    ...
    class TrackedEntity implements ServerEntity.Synchronizer {
        ...
        @Override
        public void sendToTrackingPlayersAndSelf(Packet<? super ClientGamePacketListener> p_449188_) {
            this.sendToTrackingPlayers(p_449188_);
            if (this.entity instanceof ServerPlayer serverplayer) {
                serverplayer.connection.send(p_449188_);
            }
        }

        @Override
        public void sendToTrackingPlayersFiltered(Packet<? super ClientGamePacketListener> p_449059_, Predicate<ServerPlayer> p_449352_) {
            for (ServerPlayerConnection serverplayerconnection : this.seenBy) {
                if (p_449352_.test(serverplayerconnection.getPlayer())) {
                    serverplayerconnection.send(p_449059_);
                }
            }
        }
