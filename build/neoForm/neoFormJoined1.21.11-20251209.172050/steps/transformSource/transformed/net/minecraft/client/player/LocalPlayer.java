...
public class LocalPlayer extends AbstractClientPlayer {
    ...
    @Override
    public boolean startRiding(Entity p_108667_, boolean p_108668_, boolean p_435382_) {
        if (!super.startRiding(p_108667_, p_108668_, p_435382_)) {
            return false;
        } else {
            if (p_108667_ instanceof AbstractMinecart abstractminecart) {
                this.minecraft
                    .getSoundManager()
                    .play(new RidingMinecartSoundInstance(this, abstractminecart, true, SoundEvents.MINECART_INSIDE_UNDERWATER, 0.0F, 0.75F, 1.0F));
                this.minecraft
                    .getSoundManager()
                    .play(new RidingMinecartSoundInstance(this, abstractminecart, false, SoundEvents.MINECART_INSIDE, 0.0F, 0.75F, 1.0F));
                ...
                this.minecraft
                    .getSoundManager()
                    .play(
                        new RidingEntitySoundInstance(
                            this, abstractnautilus, true, SoundEvents.NAUTILUS_RIDING, abstractnautilus.getSoundSource(), 0.0F, 1.0F, 5.0F
                        )
                    );
            }
    ...
    @Override
    public void setReducedDebugInfo(boolean p_435576_) {
        super.setReducedDebugInfo(p_435576_);
    ...
    public void setExperienceValues(float currentXP, int maxXP, int level) {
        if (currentXP != this.experienceProgress) {
    ...
    @Override
    public void openMinecartCommandBlock(MinecartCommandBlock p_478137_) {
        this.minecraft.setScreen(new MinecartCommandBlockEditScreen(p_478137_));
    }
    ...
    @Override
    public void openTestBlock(TestBlockEntity p_398045_) {
        this.minecraft.setScreen(new TestBlockEditScreen(p_398045_));
    }

    @Override
    public void openTestInstanceBlock(TestInstanceBlockEntity p_397905_) {
        this.minecraft.setScreen(new TestInstanceBlockEditScreen(p_397905_));
    }
    ...
    @Override
    public void openDialog(Holder<Dialog> p_425675_) {
        this.connection.showDialog(p_425675_, this.minecraft.screen);
    }
    ...
    @Override
    public void handleCreativeModeItemDrop(ItemStack p_365052_) {
        this.minecraft.gameMode.handleCreativeModeItemDrop(p_365052_);
    }
    ...
    public HitResult raycastHitResult(float partialTick, Entity cameraEntity) {
        ItemStack itemstack = this.getActiveItem();
        AttackRange attackrange = itemstack.get(DataComponents.ATTACK_RANGE);
        double d0 = this.blockInteractionRange();
        HitResult hitresult = null;
        if (attackrange != null) {
            hitresult = attackrange.getClosesetHit(cameraEntity, partialTick, EntitySelector.CAN_BE_PICKED);
            if (hitresult instanceof BlockHitResult) {
                hitresult = filterHitResult(hitresult, cameraEntity.getEyePosition(partialTick), d0);
            }
        }

        if (hitresult == null || hitresult.getType() == HitResult.Type.MISS) {
            double d1 = this.entityInteractionRange();
            hitresult = pick(cameraEntity, d0, d1, partialTick);
        }

        return hitresult;
    }

    private static HitResult pick(Entity cameraEntity, double blockRange, double entityRange, float partialTick) {
        double d0 = Math.max(blockRange, entityRange);
        double d1 = Mth.square(d0);
        Vec3 vec3 = cameraEntity.getEyePosition(partialTick);
        HitResult hitresult = cameraEntity.pick(d0, partialTick, false);
        ...
        Vec3 vec31 = cameraEntity.getViewVector(partialTick);
        Vec3 vec32 = vec3.add(vec31.x * d0, vec31.y * d0, vec31.z * d0);
        float f = 1.0F;
        AABB aabb = cameraEntity.getBoundingBox().expandTowards(vec31.scale(d0)).inflate(1.0, 1.0, 1.0);
        EntityHitResult entityhitresult = ProjectileUtil.getEntityHitResult(cameraEntity, vec3, vec32, aabb, EntitySelector.CAN_BE_PICKED, d1);
        return entityhitresult != null && entityhitresult.getLocation().distanceToSqr(vec3) < d2
            ? filterHitResult(entityhitresult, vec3, entityRange)
            : filterHitResult(hitresult, vec3, blockRange);
    }

    private static HitResult filterHitResult(HitResult hitResult, Vec3 eyePos, double blockRange) {
        Vec3 vec3 = hitResult.getLocation();
        if (!vec3.closerThan(eyePos, blockRange)) {
            Vec3 vec31 = hitResult.getLocation();
            Direction direction = Direction.getApproximateNearest(vec31.x - eyePos.x, vec31.y - eyePos.y, vec31.z - eyePos.z);
