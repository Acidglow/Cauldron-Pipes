...
public record CircleGizmo(Vec3 pos, float radius, GizmoStyle style) implements Gizmo {
    ...
    @Override
    public void emit(GizmoPrimitives p_455055_, float p_458020_) {
        if (this.style.hasStroke() || this.style.hasFill()) {
            ...
            if (this.style.hasFill()) {
                int j = this.style.multipliedFill(p_458020_);
                p_455055_.addTriangleFan(avec3, j);
            }

            if (this.style.hasStroke()) {
                int k = this.style.multipliedStroke(p_458020_);

                for (int l = 0; l < 20; l++) {
                    p_455055_.addLine(avec3[l], avec3[l + 1], k, this.style.strokeWidth());
                }
            }
        }
    }
