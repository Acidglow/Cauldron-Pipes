...
public record ArrowGizmo(Vec3 start, Vec3 end, int color, float width) implements Gizmo {
    ...
    @Override
    public void emit(GizmoPrimitives p_455938_, float p_458233_) {
        int i = ARGB.multiplyAlpha(this.color, p_458233_);
        p_455938_.addLine(this.start, this.end, i, this.width);
        ...
        for (Vector3f vector3f : avector3f) {
            p_455938_.addLine(this.end.add(vector3f.x, vector3f.y, vector3f.z), this.end, i, this.width);
        }
    }
}
