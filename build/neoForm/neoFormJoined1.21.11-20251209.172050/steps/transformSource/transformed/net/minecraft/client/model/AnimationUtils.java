...
public class AnimationUtils {
    ...
    public static void animateCrossbowCharge(ModelPart rightArm, ModelPart leftArm, float maxChargeDuration, float ticksUsingItem, boolean rightHanded) {
        ModelPart modelpart = rightHanded ? rightArm : leftArm;
        ModelPart modelpart1 = rightHanded ? leftArm : rightArm;
        modelpart.yRot = rightHanded ? -0.8F : 0.8F;
        modelpart.xRot = -0.97079635F;
        modelpart1.xRot = modelpart.xRot;
        float f = Mth.clamp(ticksUsingItem, 0.0F, maxChargeDuration);
        float f1 = f / maxChargeDuration;
        modelpart1.yRot = Mth.lerp(f1, 0.4F, 0.85F) * (rightHanded ? 1 : -1);
    ...
    public static <T extends UndeadRenderState> void animateZombieArms(ModelPart leftArm, ModelPart rightArm, boolean isAggressive, T renderState) {
        boolean flag = renderState.swingAnimationType != SwingAnimationType.STAB;
        if (flag) {
            float f = renderState.attackTime;
            float f1 = (float) -Math.PI / (isAggressive ? 1.5F : 2.25F);
            float f2 = Mth.sin(f * (float) Math.PI);
            float f3 = Mth.sin((1.0F - (1.0F - f) * (1.0F - f)) * (float) Math.PI);
            rightArm.zRot = 0.0F;
            rightArm.yRot = -(0.1F - f2 * 0.6F);
            rightArm.xRot = f1;
            rightArm.xRot += f2 * 1.2F - f3 * 0.4F;
            leftArm.zRot = 0.0F;
            leftArm.yRot = 0.1F - f2 * 0.6F;
            leftArm.xRot = f1;
            leftArm.xRot += f2 * 1.2F - f3 * 0.4F;
        }

        bobArms(rightArm, leftArm, renderState.ageInTicks);
    }
}
