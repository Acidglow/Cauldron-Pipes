...
public class Warden extends Monster implements VibrationSystem {
    ...
    @Override
    public boolean isInvulnerableTo(ServerLevel p_376345_, DamageSource p_219427_) {
        return this.isDiggingOrEmerging() && !p_219427_.is(DamageTypeTags.BYPASSES_INVULNERABILITY) ? true : super.isInvulnerableTo(p_376345_, p_219427_);
    }
    ...
    public boolean doHurtTarget(ServerLevel p_376574_, Entity p_219472_) {
        p_376574_.broadcastEntityEvent(this, (byte)4);
    ...
    protected void customServerAiStep(ServerLevel p_376677_) {
        ...
        this.getBrain().tick(p_376677_, this);
        profilerfiller.pop();
        super.customServerAiStep(p_376677_);
        if ((this.tickCount + this.getId()) % 120 == 0) {
            applyDarknessAround(p_376677_, this.position(), this, 20);
        }

        if (this.tickCount % 20 == 0) {
            this.angerManagement.tick(p_376677_, this::canTargetEntity);
    ...
    @Override
    protected void addAdditionalSaveData(ValueOutput p_422360_) {
        super.addAdditionalSaveData(p_422360_);
        p_422360_.store("anger", AngerManagement.codec(this::canTargetEntity), this.angerManagement);
        p_422360_.store("listener", VibrationSystem.Data.CODEC, this.vibrationData);
    }

    @Override
    protected void readAdditionalSaveData(ValueInput p_422659_) {
        super.readAdditionalSaveData(p_422659_);
        this.angerManagement = p_422659_.read("anger", AngerManagement.codec(this::canTargetEntity))
            .orElseGet(() -> new AngerManagement(this::canTargetEntity, Collections.emptyList()));
        this.syncClientAngerLevel();
        this.vibrationData = p_422659_.read("listener", VibrationSystem.Data.CODEC).orElseGet(VibrationSystem.Data::new);
    }
    ...
    public @Nullable SpawnGroupData finalizeSpawn(
        ServerLevelAccessor p_219400_, DifficultyInstance p_219401_, EntitySpawnReason p_360351_, @Nullable SpawnGroupData p_219403_
    ) {
        this.getBrain().setMemoryWithExpiry(MemoryModuleType.DIG_COOLDOWN, Unit.INSTANCE, 1200L);
        if (p_360351_ == EntitySpawnReason.TRIGGERED) {
            this.setPose(Pose.EMERGING);
            this.getBrain().setMemoryWithExpiry(MemoryModuleType.IS_EMERGING, Unit.INSTANCE, WardenAi.EMERGE_DURATION);
            this.playSound(SoundEvents.WARDEN_AGITATED, 5.0F, 1.0F);
        }

        return super.finalizeSpawn(p_219400_, p_219401_, p_360351_, p_219403_);
    }
    ...
    public boolean hurtServer(ServerLevel p_376595_, DamageSource p_376181_, float p_376898_) {
        ...
        if (!this.isNoAi() && !this.isDiggingOrEmerging()) {
            Entity entity = p_376181_.getEntity();
            ...
            if (this.brain.getMemory(MemoryModuleType.ATTACK_TARGET).isEmpty()
                ...
                && (p_376181_.isDirect() || this.closerThan(livingentity, 5.0))) {
