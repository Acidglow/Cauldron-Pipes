...
public abstract class StoredUserList<K, V extends StoredUserEntry<K>> {
    ...
    public boolean add(V entry) {
        String s = this.getKeyForUser(entry.getUser());
        V v = this.map.get(s);
        if (entry.equals(v)) {
            return false;
        } else {
    ...
    public @Nullable V get(K obj) {
        this.removeExpired();
        return this.map.get(this.getKeyForUser(obj));
    }

    public boolean remove(K key) {
        V v = this.map.remove(this.getKeyForUser(key));
        if (v == null) {
            return false;
        } else {
            try {
    ...
    public void save() throws IOException {
        ...
        this.map.values().stream().map(p_466393_ -> Util.make(new JsonObject(), p_466393_::serialize)).forEach(jsonarray::add);
