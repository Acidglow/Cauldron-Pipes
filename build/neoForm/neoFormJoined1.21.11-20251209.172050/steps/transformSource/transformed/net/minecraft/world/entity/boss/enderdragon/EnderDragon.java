...
public class EnderDragon extends Mob implements Enemy {
                ...
                if (this.level() instanceof ServerLevel serverlevel1) {
                    ...
                    if (vec3 != null) {
                        ...
                        if (Math.abs(d0) > 1.0E-5F || Math.abs(d2) > 1.0E-5F) {
                            float f4 = Mth.clamp(Mth.wrapDegrees(180.0F - (float)Mth.atan2(d0, d2) * (180.0F / (float)Math.PI) - this.getYRot()), -50.0F, 50.0F);
                            this.yRotA *= 0.8F;
            ...
            for (int l1 = j; l1 <= i1; l1++) {
                for (int i2 = k; i2 <= j1; i2++) {
                    ...
                    if (!blockstate.isAir() && !blockstate.is(BlockTags.DRAGON_TRANSPARENT)) {
                        if (net.neoforged.neoforge.common.CommonHooks.canEntityDestroy(level, blockpos, this) && !blockstate.is(BlockTags.DRAGON_IMMUNE)) {
    ...
    public boolean hurt(ServerLevel level, EnderDragonPart part, DamageSource source, float amount) {
        if (this.phaseManager.getCurrentPhase().getPhase() == EnderDragonPhase.DYING) {
            ...
            if (amount < 0.01F) {
                ...
                if (source.getEntity() instanceof Player || source.is(DamageTypeTags.ALWAYS_HURTS_ENDER_DRAGONS)) {
                    ...
                    this.reallyHurt(level, source, amount);
    ...
    public boolean hurtServer(ServerLevel p_376883_, DamageSource p_376758_, float p_376328_) {
        return this.hurt(p_376883_, this.body, p_376758_, p_376328_);
    }
    ...
    @Override
    protected void addAdditionalSaveData(ValueOutput p_422338_) {
        super.addAdditionalSaveData(p_422338_);
        p_422338_.putInt("DragonPhase", this.phaseManager.getCurrentPhase().getPhase().getId());
        p_422338_.putInt("DragonDeathTime", this.dragonDeathTime);
    }

    @Override
    protected void readAdditionalSaveData(ValueInput p_421923_) {
        super.readAdditionalSaveData(p_421923_);
        p_421923_.getInt("DragonPhase").ifPresent(p_409657_ -> this.phaseManager.setPhase(EnderDragonPhase.getById(p_409657_)));
        this.dragonDeathTime = p_421923_.getIntOr("DragonDeathTime", 0);
    }
    ...
    public void onCrystalDestroyed(ServerLevel level, EndCrystal crystal, BlockPos pos, DamageSource damageSource) {
        Player player;
        if (damageSource.getEntity() instanceof Player player1) {
            player = player1;
        } else {
