...
public class AirAndWaterRandomPos {
    public static @Nullable Vec3 getPos(
        PathfinderMob mob, int maxDistance, int yRange, int y, double x, double z, double amplifier
    ) {
    ...
    public static @Nullable BlockPos generateRandomPos(
        ...
        BlockPos blockpos = RandomPos.generateRandomDirectionWithinRadians(
            mob.getRandom(), 0.0, maxDistance, yRange, y, x, z, amplifier
        );
        if (blockpos == null) {
            return null;
        } else {
            ...
            if (!GoalUtils.isOutsideLimits(blockpos1, mob) && !GoalUtils.isRestricted(shortCircuit, mob, blockpos1)) {
                blockpos1 = RandomPos.moveUpOutOfSolid(blockpos1, mob.level().getMaxY(), p_148376_ -> GoalUtils.isSolid(mob, p_148376_));
