...
public class WeatherAttributes {
    ...
    private static <Value> void addLayer(
        EnvironmentAttributeSystem.Builder builder, WeatherAttributes.WeatherAccess weatherAccess, EnvironmentAttribute<Value> attribute
    ) {
        ...
        builder.addTimeBasedLayer(attribute, (p_469894_, p_468621_) -> {
            float f = weatherAccess.thunderLevel();
            float f1 = weatherAccess.rainLevel() - f;
            if (entry != null && f1 > 0.0F) {
                Value value = entry.applyModifier(p_469894_);
                p_469894_ = attribute.type().stateChangeLerp().apply(f1, p_469894_, value);
            }

            if (entry1 != null && f > 0.0F) {
                Value value1 = entry1.applyModifier(p_469894_);
                p_469894_ = attribute.type().stateChangeLerp().apply(f, p_469894_, value1);
            }
