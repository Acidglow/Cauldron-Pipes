...
public class Dolphin extends AgeableWaterCreature {
    ...
    public static final Predicate<ItemEntity> ALLOWED_ITEMS = p_479413_ -> !p_479413_.hasPickUpDelay() && p_479413_.isAlive() && p_479413_.isInWater();
    ...
    public Dolphin(EntityType<? extends Dolphin> p_480497_, Level p_477978_) {
        super(p_480497_, p_477978_);
    ...
    public @Nullable SpawnGroupData finalizeSpawn(
        ServerLevelAccessor p_479150_, DifficultyInstance p_481882_, EntitySpawnReason p_478942_, @Nullable SpawnGroupData p_481012_
    ) {
        this.setAirSupply(this.getMaxAirSupply());
        this.setXRot(0.0F);
        SpawnGroupData spawngroupdata = Objects.requireNonNullElseGet(p_481012_, () -> new AgeableMob.AgeableMobGroupData(0.1F));
        return super.finalizeSpawn(p_479150_, p_481882_, p_478942_, spawngroupdata);
    }

    public @Nullable Dolphin getBreedOffspring(ServerLevel p_478908_, AgeableMob p_480898_) {
        return EntityType.DOLPHIN.create(p_478908_, EntitySpawnReason.BREEDING);
    }
    ...
    @Override
    protected void handleAirSupply(int p_478685_) {
    }
    ...
    @Override
    protected void defineSynchedData(SynchedEntityData.Builder p_478217_) {
        super.defineSynchedData(p_478217_);
        p_478217_.define(GOT_FISH, false);
        p_478217_.define(MOISTNESS_LEVEL, 2400);
    }

    @Override
    protected void addAdditionalSaveData(ValueOutput p_479354_) {
        super.addAdditionalSaveData(p_479354_);
        p_479354_.putBoolean("GotFish", this.gotFish());
        p_479354_.putInt("Moistness", this.getMoistnessLevel());
    }

    @Override
    protected void readAdditionalSaveData(ValueInput p_481249_) {
        super.readAdditionalSaveData(p_481249_);
        this.setGotFish(p_481249_.getBooleanOr("GotFish", false));
        this.setMoisntessLevel(p_481249_.getIntOr("Moistness", 2400));
    }
    ...
    @Override
    public boolean canAttack(LivingEntity p_478500_) {
        return !this.isBaby() && super.canAttack(p_478500_);
    }
    ...
    @Override
    protected boolean canDispenserEquipIntoSlot(EquipmentSlot p_481432_) {
        return p_481432_ == EquipmentSlot.MAINHAND && this.canPickUpLoot();
    }

    @Override
    protected void pickUpItem(ServerLevel p_480659_, ItemEntity p_480700_) {
        if (this.getItemBySlot(EquipmentSlot.MAINHAND).isEmpty()) {
            ItemStack itemstack = p_480700_.getItem();
            if (this.canHoldItem(itemstack)) {
                this.onItemPickup(p_480700_);
                this.setItemSlot(EquipmentSlot.MAINHAND, itemstack);
                this.setGuaranteedDrop(EquipmentSlot.MAINHAND);
                this.take(p_480700_, itemstack.getCount());
                p_480700_.discard();
            }
        }
    }
    ...
    public void tick() {
        ...
        if (this.isNoAi()) {
            ...
            if (this.level().isClientSide() && this.isInWater() && this.getDeltaMovement().lengthSqr() > 0.03) {
                ...
                for (int i = 0; i < 2; i++) {
                    this.level()
                        .addParticle(ParticleTypes.DOLPHIN, this.getX() - vec3.x * f2 + f, this.getY() - vec3.y, this.getZ() - vec3.z * f2 + f1, 0.0, 0.0, 0.0);
                    this.level()
                        .addParticle(ParticleTypes.DOLPHIN, this.getX() - vec3.x * f2 - f, this.getY() - vec3.y, this.getZ() - vec3.z * f2 - f1, 0.0, 0.0, 0.0);
                }
            }
    ...
    @Override
    public void handleEntityEvent(byte p_478221_) {
        if (p_478221_ == 38) {
            this.addParticlesAroundSelf(ParticleTypes.HAPPY_VILLAGER);
        } else {
            super.handleEntityEvent(p_478221_);
        }
    }
    ...
    @Override
    protected void travelInWater(Vec3 p_479281_, double p_481909_, boolean p_480975_, double p_481669_) {
        this.moveRelative(this.getSpeed(), p_479281_);
