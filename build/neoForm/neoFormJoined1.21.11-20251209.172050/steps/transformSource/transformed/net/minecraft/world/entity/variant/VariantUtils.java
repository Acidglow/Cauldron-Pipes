...
public class VariantUtils {
    ...
    public static <T> void writeVariant(ValueOutput output, Holder<T> variant) {
        variant.unwrapKey().ifPresent(p_466622_ -> output.store("variant", Identifier.CODEC, p_466622_.identifier()));
    }

    public static <T> Optional<Holder<T>> readVariant(ValueInput input, ResourceKey<? extends Registry<T>> registryKey) {
        return input.read("variant", Identifier.CODEC).map(p_466624_ -> ResourceKey.create(registryKey, p_466624_)).flatMap(input.lookup()::get);
    }
