...
public class BlazeModel extends EntityModel<LivingEntityRenderState> {
    ...
    public BlazeModel(ModelPart p_478253_) {
        super(p_478253_);
        this.head = p_478253_.getChild("head");
        this.upperBodyParts = new ModelPart[12];
        Arrays.setAll(this.upperBodyParts, p_478980_ -> p_478253_.getChild(getPartName(p_478980_)));
    }
    ...
    public void setupAnim(LivingEntityRenderState p_479077_) {
        super.setupAnim(p_479077_);
        float f = p_479077_.ageInTicks * (float) Math.PI * -0.1F;

        for (int i = 0; i < 4; i++) {
            this.upperBodyParts[i].y = -2.0F + Mth.cos((i * 2 + p_479077_.ageInTicks) * 0.25F);
            this.upperBodyParts[i].x = Mth.cos(f) * 9.0F;
            this.upperBodyParts[i].z = Mth.sin(f) * 9.0F;
            f++;
        }

        f = (float) (Math.PI / 4) + p_479077_.ageInTicks * (float) Math.PI * 0.03F;

        for (int j = 4; j < 8; j++) {
            this.upperBodyParts[j].y = 2.0F + Mth.cos((j * 2 + p_479077_.ageInTicks) * 0.25F);
            this.upperBodyParts[j].x = Mth.cos(f) * 7.0F;
            this.upperBodyParts[j].z = Mth.sin(f) * 7.0F;
            f++;
        }

        f = 0.47123894F + p_479077_.ageInTicks * (float) Math.PI * -0.05F;

        for (int k = 8; k < 12; k++) {
            this.upperBodyParts[k].y = 11.0F + Mth.cos((k * 1.5F + p_479077_.ageInTicks) * 0.5F);
            this.upperBodyParts[k].x = Mth.cos(f) * 5.0F;
            this.upperBodyParts[k].z = Mth.sin(f) * 5.0F;
            f++;
        }

        this.head.yRot = p_479077_.yRot * (float) (Math.PI / 180.0);
        this.head.xRot = p_479077_.xRot * (float) (Math.PI / 180.0);
    }
}
