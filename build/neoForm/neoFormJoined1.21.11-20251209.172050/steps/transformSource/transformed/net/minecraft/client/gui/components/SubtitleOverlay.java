...
public class SubtitleOverlay implements SoundEventListener {
            ...
            if (!this.audibleSubtitles.isEmpty()) {
                ...
                for (SubtitleOverlay.Subtitle subtitleoverlay$subtitle2 : this.audibleSubtitles) {
                    ...
                    if (subtitleoverlay$soundplayedat != null) {
                        ...
                        guiGraphics.pose().translate(guiGraphics.guiWidth() - j * 1.0F - 2.0F, guiGraphics.guiHeight() - 35 - l1 * (k + 1) * 1.0F);
                        ...
                        if (!flag) {
                            if (d1 > 0.0) {
                                guiGraphics.drawString(this.minecraft.font, ">", j - this.minecraft.font.width(">"), -l, k1);
                            } else if (d1 < 0.0) {
                                guiGraphics.drawString(this.minecraft.font, "<", -j, -l, k1);
                            }
                        }

                        guiGraphics.drawString(this.minecraft.font, component, -i1 / 2, -l, k1);
    ...
    public void onPlaySound(SoundInstance p_94645_, WeighedSoundEvents p_94646_, float p_314721_) {
        if (p_94646_.getSubtitle() != null) {
            ...
            this.subtitles.add(new SubtitleOverlay.Subtitle(component, p_314721_, new Vec3(p_94645_.getX(), p_94645_.getY(), p_94645_.getZ())));
        }
    }
    ...
    static class Subtitle {
        ...
        public SubtitleOverlay.@Nullable SoundPlayedAt getClosest(Vec3 location) {
        ...
        public boolean isAudibleFrom(Vec3 location) {
            if (Float.isInfinite(this.range)) {
                ...
                return subtitleoverlay$soundplayedat == null ? false : location.closerThan(subtitleoverlay$soundplayedat.location, this.range);
            }
        }

        public void purgeOldInstances(double displayTime) {
            long i = Util.getMillis();
            this.playedAt.removeIf(p_347590_ -> i - p_347590_.time() > displayTime);
        }
