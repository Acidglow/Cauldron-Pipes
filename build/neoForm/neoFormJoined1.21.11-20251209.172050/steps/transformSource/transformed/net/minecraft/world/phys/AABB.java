...
public class AABB {
    ...
    public double distanceToSqr(AABB box) {
        double d0 = Math.max(Math.max(this.minX - box.maxX, box.minX - this.maxX), 0.0);
        double d1 = Math.max(Math.max(this.minY - box.maxY, box.minY - this.maxY), 0.0);
        double d2 = Math.max(Math.max(this.minZ - box.maxZ, box.minZ - this.maxZ), 0.0);
        return Mth.lengthSquared(d0, d1, d2);
    }
