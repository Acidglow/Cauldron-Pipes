    ...
    public void slotsChanged(Container inventory) {
        if (inventory == this.enchantSlots) {
            ...
            if (!itemstack.isEmpty() && itemstack.isEnchantable()) {
                this.access.execute((p_460548_, p_460549_) -> {
                    IdMap<Holder<Enchantment>> idmap = p_460548_.registryAccess().lookupOrThrow(Registries.ENCHANTMENT).asHolderIdMap();
                    float j = 0;

                    for (BlockPos blockpos : EnchantingTableBlock.BOOKSHELF_OFFSETS) {
                        if (EnchantingTableBlock.isValidBookShelf(p_460548_, p_460549_, blockpos)) {
                            j += p_460548_.getBlockState(p_460549_.offset(blockpos)).getEnchantPowerBonus(p_460548_, p_460549_.offset(blockpos));
                        }
                    }
                    ...
                    for (int k = 0; k < 3; k++) {
                        ...
                        this.costs[k] = net.neoforged.neoforge.event.EventHooks.onEnchantmentLevelSet(p_460548_, p_460549_, k, (int)j, itemstack, costs[k]);
                    }

                    for (int l = 0; l < 3; l++) {
                        if (this.costs[l] > 0) {
                            List<EnchantmentInstance> list = this.getEnchantmentList(p_460548_.registryAccess(), itemstack, l, this.costs[l]);
                            if (!list.isEmpty()) {
                ...
                this.access
                    .execute(
                        (p_466630_, p_466631_) -> {
                            ...
                            List<EnchantmentInstance> list = this.getEnchantmentList(p_466630_.registryAccess(), itemstack, id, this.costs[id]);
                            if (!list.isEmpty()) {
                                ...
                                p_466630_.playSound(
                                    null, p_466631_, SoundEvents.ENCHANTMENT_TABLE_USE, SoundSource.BLOCKS, 1.0F, p_466630_.random.nextFloat() * 0.1F + 0.9F
                                );
                            }
                        }
                    );
