...
public class SpreadPlayersCommand {
                                            ...
                                            Commands.argument("respectTeams", BoolArgumentType.bool())
                                                .then(
                                                    Commands.argument("targets", EntityArgument.entities())
                                                        .executes(
                                                            p_457364_ -> spreadPlayers(
                                                                p_457364_.getSource(),
                                                                Vec2Argument.getVec2(p_457364_, "center"),
                                                                FloatArgumentType.getFloat(p_457364_, "spreadDistance"),
                                                                FloatArgumentType.getFloat(p_457364_, "maxRange"),
                                                                p_457364_.getSource().getLevel().getMaxY() + 1,
                                                                BoolArgumentType.getBool(p_457364_, "respectTeams"),
                                                                EntityArgument.getEntities(p_457364_, "targets")
                                                            )
                                                        )
                                                )
                                        )
    ...
    private static int spreadPlayers(
        ...
        if (maxHeight < i) {
            ...
            spreadPositions(center, spreadDistance, serverlevel, randomsource, d0, d1, d2, d3, maxHeight, aspreadplayerscommand$position, respectTeams);
    ...
    private static double setPlayerPositions(
        ...
        return targets.size() < 2 ? 0.0 : d0 / targets.size();
    }
    ...
    static class Position {
        ...
        public int getSpawnY(BlockGetter level, int y) {
            ...
            while (blockpos$mutableblockpos.getY() > level.getMinY()) {
                blockpos$mutableblockpos.move(Direction.DOWN);
