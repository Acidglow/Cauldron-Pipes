...
public class AllayModel extends EntityModel<AllayRenderState> implements ArmedModel<AllayRenderState> {
    ...
    public AllayModel(ModelPart p_478120_) {
        super(p_478120_.getChild("root"), RenderTypes::entityTranslucent);
    }
    ...
    public void setupAnim(AllayRenderState p_478884_) {
        super.setupAnim(p_478884_);
        float f = p_478884_.walkAnimationSpeed;
        float f1 = p_478884_.walkAnimationPos;
        float f2 = p_478884_.ageInTicks * 20.0F * (float) (Math.PI / 180.0) + f1;
        float f3 = Mth.cos(f2) * (float) Math.PI * 0.15F + f;
        float f4 = p_478884_.ageInTicks * 9.0F * (float) (Math.PI / 180.0);
        float f5 = Math.min(f / 0.3F, 1.0F);
        float f6 = 1.0F - f5;
        float f7 = p_478884_.holdingAnimationProgress;
        if (p_478884_.isDancing) {
            float f8 = p_478884_.ageInTicks * 8.0F * (float) (Math.PI / 180.0) + f;
            float f9 = Mth.cos(f8) * 16.0F * (float) (Math.PI / 180.0);
            float f10 = p_478884_.spinningProgress;
            float f11 = Mth.cos(f8) * 14.0F * (float) (Math.PI / 180.0);
            float f12 = Mth.cos(f8) * 30.0F * (float) (Math.PI / 180.0);
            this.root.yRot = p_478884_.isSpinning ? (float) (Math.PI * 4) * f10 : this.root.yRot;
            this.root.zRot = f9 * (1.0F - f10);
            this.head.yRot = f12 * (1.0F - f10);
            this.head.zRot = f11 * (1.0F - f10);
        } else {
            this.head.xRot = p_478884_.xRot * (float) (Math.PI / 180.0);
            this.head.yRot = p_478884_.yRot * (float) (Math.PI / 180.0);
        }
    ...
    public void translateToHand(AllayRenderState p_479667_, HumanoidArm p_481273_, PoseStack p_479241_) {
        float f = 1.0F;
        float f1 = 3.0F;
        this.root.translateAndRotate(p_479241_);
        this.body.translateAndRotate(p_479241_);
        p_479241_.translate(0.0F, 0.0625F, 0.1875F);
        p_479241_.mulPose(Axis.XP.rotation(this.right_arm.xRot));
        p_479241_.scale(0.7F, 0.7F, 0.7F);
        p_479241_.translate(0.0625F, 0.0F, 0.0F);
    }
}
