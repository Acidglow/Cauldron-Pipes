...
public class ThrownPotionSplitFix extends EntityRenameFix {
    ...
    protected Pair<String, Typed<?>> fix(String p_399987_, Typed<?> p_399615_) {
        if (!p_399987_.equals("minecraft:potion")) {
            return Pair.of(p_399987_, p_399615_);
        } else {
            String s = this.itemIdFinder.get().getItemId(p_399615_);
            return "minecraft:lingering_potion".equals(s) ? Pair.of("minecraft:lingering_potion", p_399615_) : Pair.of("minecraft:splash_potion", p_399615_);
        }
    }
    ...
    record ItemIdFinder(OpticFinder<?> itemFinder, OpticFinder<Pair<String, String>> itemIdFinder) {
        public String getItemId(Typed<?> data) {
            return data.getOptionalTyped(this.itemFinder)
                .flatMap(p_400219_ -> p_400219_.getOptional(this.itemIdFinder))
                .map(Pair::getSecond)
