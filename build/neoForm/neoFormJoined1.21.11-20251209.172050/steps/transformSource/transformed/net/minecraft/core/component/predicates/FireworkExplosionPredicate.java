...
public record FireworkExplosionPredicate(FireworkExplosionPredicate.FireworkPredicate predicate) implements SingleComponentItemPredicate<FireworkExplosion> {
    ...
    public boolean matches(FireworkExplosion p_400258_) {
        return this.predicate.test(p_400258_);
    }
    ...
    public record FireworkPredicate(Optional<FireworkExplosion.Shape> shape, Optional<Boolean> twinkle, Optional<Boolean> trail)
        ...
        public static final Codec<FireworkExplosionPredicate.FireworkPredicate> CODEC = RecordCodecBuilder.create(
            p_400098_ -> p_400098_.group(
                ...
                .apply(p_400098_, FireworkExplosionPredicate.FireworkPredicate::new)
        );

        public boolean test(FireworkExplosion p_399871_) {
            if (this.shape.isPresent() && this.shape.get() != p_399871_.shape()) {
                return false;
            } else {
                return this.twinkle.isPresent() && this.twinkle.get() != p_399871_.hasTwinkle()
                    ? false
                    : !this.trail.isPresent() || this.trail.get() == p_399871_.hasTrail();
            }
        }
    }
}
