...
public record AttackRange(float minRange, float maxRange, float minCreativeRange, float maxCreativeRange, float hitboxMargin, float mobFactor) {
    public static final Codec<AttackRange> CODEC = RecordCodecBuilder.create(
        p_482355_ -> p_482355_.group(
            ...
            .apply(p_482355_, AttackRange::new)
    );
    ...
    public HitResult getClosesetHit(Entity entity, float partialTick, Predicate<Entity> predicate) {
        ...
        if (either.left().isPresent()) {
            ...
            if (entityhitresult != null) {
                ...
                Vec3 vec31 = entity.getHeadLookAngle();
                Vec3 vec32 = entity.getEyePosition(partialTick).add(vec31);
    ...
    public boolean isInRange(LivingEntity entity, Vec3 pos) {
        return this.isInRange(entity, pos::distanceToSqr, 0.0);
    }

    public boolean isInRange(LivingEntity entity, AABB box, double range) {
        return this.isInRange(entity, box::distanceToSqr, range);
    }

    private boolean isInRange(LivingEntity entity, ToDoubleFunction<Vec3> distanceSqrGetter, double range) {
        double d0 = Math.sqrt(distanceSqrGetter.applyAsDouble(entity.getEyePosition()));
        double d1 = this.effectiveMinRange(entity) - this.hitboxMargin - range;
        double d2 = this.effectiveMaxRange(entity) + this.hitboxMargin + range;
        return d0 >= d1 && d0 <= d2;
    }
}
