    ...
    public static void grab(File gameDirectory, @Nullable String filename, RenderTarget renderTarget, int downscaleFactor, Consumer<Component> messageConsumer) {
        takeScreenshot(
            ...
            p_465362_ -> {
                ...
                net.neoforged.neoforge.client.event.ScreenshotEvent event = net.neoforged.neoforge.client.ClientHooks.onScreenshot(p_465362_, file2);
                if (event.isCanceled()) {
                ...
                Util.ioPool()
                    .execute(
                        () -> {
                            try {
                                NativeImage $$4x = p_465362_;

                                try {
                                    $$4x.writeToFile(target);
