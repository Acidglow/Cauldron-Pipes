...
public class EnderDragonModel extends EntityModel<EnderDragonRenderState> {
    ...
    public EnderDragonModel(ModelPart root) {
        ...
        for (int j = 0; j < this.tailParts.length; j++) {
            this.tailParts[j] = root.getChild(tailName(j));
        }
    ...
    public void setupAnim(EnderDragonRenderState p_481096_) {
        super.setupAnim(p_481096_);
        float f = p_481096_.flapTime * (float) (Math.PI * 2);
        ...
        float f5 = 1.5F;
        DragonFlightHistory.Sample dragonflighthistory$sample = p_481096_.getHistoricalPos(6);
        float f6 = Mth.wrapDegrees(p_481096_.getHistoricalPos(5).yRot() - p_481096_.getHistoricalPos(10).yRot());
        float f7 = Mth.wrapDegrees(p_481096_.getHistoricalPos(5).yRot() + f6 / 2.0F);

        for (int i = 0; i < 5; i++) {
            ModelPart modelpart = this.neckParts[i];
            DragonFlightHistory.Sample dragonflighthistory$sample1 = p_481096_.getHistoricalPos(5 - i);
            float f8 = Mth.cos(i * 0.45F + f) * 0.15F;
            modelpart.yRot = Mth.wrapDegrees(dragonflighthistory$sample1.yRot() - dragonflighthistory$sample.yRot()) * (float) (Math.PI / 180.0) * 1.5F;
            modelpart.xRot = f8
                + p_481096_.getHeadPartYOffset(i, dragonflighthistory$sample, dragonflighthistory$sample1) * (float) (Math.PI / 180.0) * 1.5F * 5.0F;
        ...
        this.head.z = f4;
        this.head.x = f2;
        DragonFlightHistory.Sample dragonflighthistory$sample2 = p_481096_.getHistoricalPos(0);
        this.head.yRot = Mth.wrapDegrees(dragonflighthistory$sample2.yRot() - dragonflighthistory$sample.yRot()) * (float) (Math.PI / 180.0);
        this.head.xRot = Mth.wrapDegrees(p_481096_.getHeadPartYOffset(6, dragonflighthistory$sample, dragonflighthistory$sample2))
            * (float) (Math.PI / 180.0)
            * 1.5F
            * 5.0F;
        ...
        dragonflighthistory$sample = p_481096_.getHistoricalPos(11);

        for (int j = 0; j < 12; j++) {
            DragonFlightHistory.Sample dragonflighthistory$sample3 = p_481096_.getHistoricalPos(12 + j);
            ...
            modelpart1.yRot = (Mth.wrapDegrees(dragonflighthistory$sample3.yRot() - dragonflighthistory$sample.yRot()) * 1.5F + 180.0F)
                * (float) (Math.PI / 180.0);
            modelpart1.xRot = f9 + (float)(dragonflighthistory$sample3.y() - dragonflighthistory$sample.y()) * (float) (Math.PI / 180.0) * 1.5F * 5.0F;
