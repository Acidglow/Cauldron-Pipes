...
public class Sniffing<E extends Warden> extends Behavior<E> {
    ...
    protected void stop(ServerLevel p_217672_, E p_217673_, long p_217674_) {
        ...
        p_217673_.getBrain().getMemory(MemoryModuleType.NEAREST_ATTACKABLE).filter(p_217673_::canTargetEntity).ifPresent(p_427042_ -> {
            if (p_217673_.closerThan(p_427042_, 6.0, 20.0)) {
                p_217673_.increaseAngerAt(p_427042_);
            }

            if (!p_217673_.getBrain().hasMemoryValue(MemoryModuleType.DISTURBANCE_LOCATION)) {
                WardenAi.setDisturbanceLocation(p_217673_, p_427042_.blockPosition());
            }
        });
    }
}
