...
public class VanillaRegistries {
    ...
    public static void validateThatAllBiomeFeaturesHaveBiomeFilter(HolderGetter<PlacedFeature> features, HolderLookup<Biome> biomes) {
        biomes.listElements().forEach(p_466065_ -> {
            Identifier identifier = p_466065_.key().identifier();
            List<HolderSet<PlacedFeature>> list = p_466065_.value().getGenerationSettings().features();
            list.stream().flatMap(HolderSet::stream).forEach(p_256657_ -> p_256657_.unwrap().ifLeft(p_466068_ -> {
                Holder.Reference<PlacedFeature> reference = features.getOrThrow((ResourceKey<PlacedFeature>)p_466068_);
                if (!validatePlacedFeature(reference.value())) {
                    Util.logAndPauseIfInIde("Placed feature " + p_466068_.identifier() + " in biome " + identifier + " is missing BiomeFilter.biome()");
                }
            }).ifRight(p_466070_ -> {
                if (!validatePlacedFeature(p_466070_)) {
                    Util.logAndPauseIfInIde("Placed inline feature in biome " + p_466065_ + " is missing BiomeFilter.biome()");
                }
            }));
        });
    }
