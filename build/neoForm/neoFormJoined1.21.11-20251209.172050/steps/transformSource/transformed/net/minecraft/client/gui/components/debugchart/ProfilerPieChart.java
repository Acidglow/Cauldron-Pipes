...
public class ProfilerPieChart {
    ...
    public void setPieChartResults(@Nullable ProfileResults pieChartResults) {
        this.profilerPieChartResults = pieChartResults;
    }
    ...
    public void render(GuiGraphics guiGraphics) {
        if (this.profilerPieChartResults != null) {
            ...
            int k = i + 105;
            int l = list.size() * 9;
            int i1 = guiGraphics.guiHeight() - this.bottomOffset - 5;
            int j1 = i1 - l;
            int k1 = 62;
            int l1 = j1 - 62 - 5;
            guiGraphics.fill(j - 5, l1 - 62 - 5, k + 5, i1 + 5, -1873784752);
            guiGraphics.submitProfilerChartRenderState(list, j, l1 - 62 + 10, k, l1 + 62);
            ...
            int i2 = -1;
            int j2 = l1 - 62;
            guiGraphics.drawString(this.font, s1, j, j2, -1);
            s1 = decimalformat.format(resultfield.globalPercentage) + "%";
            guiGraphics.drawString(this.font, s1, k - this.font.width(s1), j2, -1);
            ...
            for (int k2 = 0; k2 < list.size(); k2++) {
                ...
                int l2 = j1 + k2 * 9;
                guiGraphics.drawString(this.font, s2, j, l2, resultfield1.getColor());
                s2 = decimalformat.format(resultfield1.percentage) + "%";
                guiGraphics.drawString(this.font, s2, k - 50 - this.font.width(s2), l2, resultfield1.getColor());
                s2 = decimalformat.format(resultfield1.globalPercentage) + "%";
                guiGraphics.drawString(this.font, s2, k - this.font.width(s2), l2, resultfield1.getColor());
            }
        }
    }
            ...
            if (!list.isEmpty()) {
                ...
                if (index == 0) {
                    ...
                    if (index < list.size() && !"unspecified".equals(list.get(index).name)) {
                        ...
                        this.profilerTreePath = this.profilerTreePath + list.get(index).name;
                    }
                }
            }
        }
