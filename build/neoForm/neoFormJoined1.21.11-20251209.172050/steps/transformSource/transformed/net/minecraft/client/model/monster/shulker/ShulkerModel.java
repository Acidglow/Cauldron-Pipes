...
public class ShulkerModel extends EntityModel<ShulkerRenderState> {
    ...
    public ShulkerModel(ModelPart p_481516_) {
        super(p_481516_, RenderTypes::entityCutoutNoCullZOffset);
        this.lid = p_481516_.getChild("lid");
        this.head = p_481516_.getChild("head");
    }
    ...
    public void setupAnim(ShulkerRenderState p_479295_) {
        super.setupAnim(p_479295_);
        float f = (0.5F + p_479295_.peekAmount) * (float) Math.PI;
        float f1 = -1.0F + Mth.sin(f);
        float f2 = 0.0F;
        if (f > (float) Math.PI) {
            f2 = Mth.sin(p_479295_.ageInTicks * 0.1F) * 0.7F;
        }

        this.lid.setPos(0.0F, 16.0F + Mth.sin(f) * 8.0F + f2, 0.0F);
        if (p_479295_.peekAmount > 0.3F) {
            this.lid.yRot = f1 * f1 * f1 * f1 * (float) Math.PI * 0.125F;
        } else {
            this.lid.yRot = 0.0F;
        }

        this.head.xRot = p_479295_.xRot * (float) (Math.PI / 180.0);
        this.head.yRot = (p_479295_.yHeadRot - 180.0F - p_479295_.yBodyRot) * (float) (Math.PI / 180.0);
    }
}
