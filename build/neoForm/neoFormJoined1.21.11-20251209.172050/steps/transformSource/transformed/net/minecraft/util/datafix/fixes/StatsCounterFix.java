...
public class StatsCounterFix extends DataFix {
            ...
            if (s != null) {
                ...
                if (i < 0) {
                    ...
                    if ("stat.mineBlock".equals(s1)) {
                        String s6 = upgradeBlock(key.substring(i + 1).replace('.', ':'));
                        return new StatsCounterFix.StatType("minecraft:mined", s6);
                    } else {
                        String s2 = ITEM_KEYS.get(s1);
                        if (s2 != null) {
                            String s7 = key.substring(i + 1).replace('.', ':');
                            ...
                            if (s3 != null) {
                                String s4 = key.substring(i + 1).replace('.', ':');
    ...
    private TypeRewriteRule makeStatFixer() {
        ...
        return this.fixTypeEverywhereTyped("StatsCounterFix", type, type1, p_466477_ -> {
            Dynamic<?> dynamic = p_466477_.get(DSL.remainderFinder());
    ...
    private TypeRewriteRule makeObjectiveFixer() {
        ...
        return this.fixTypeEverywhereTyped(
            ...
            type1,
            p_466475_ -> {
                Dynamic<?> dynamic = p_466475_.get(DSL.remainderFinder());
                Dynamic<?> dynamic1 = dynamic.update(
                    "CriteriaName",
                    p_337666_ -> DataFixUtils.orElse(
                        p_337666_.asString()
                            .result()
                            .map(
                                p_293833_ -> {
                                    if (SPECIAL_OBJECTIVE_CRITERIA.contains(p_293833_)) {
                                        return (String)p_293833_;
                                    } else {
