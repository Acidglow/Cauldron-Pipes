...
public class ComponentUtils {
    ...
    @CheckReturnValue
    public static Component mergeStyles(Component component, Style p_style) {
        if (p_style.isEmpty()) {
            return component;
        } else {
            Style style = component.getStyle();
            if (style.isEmpty()) {
                return component.copy().setStyle(p_style);
            } else {
                return (Component)(style.equals(p_style) ? component : component.copy().setStyle(style.applyTo(p_style)));
            }
        }
    }
    ...
    private static Style resolveStyle(@Nullable CommandSourceStack commandSourceStack, Style style, @Nullable Entity entity, int recursionDepth) throws CommandSyntaxException {
        if (style.getHoverEvent() instanceof HoverEvent.ShowText(Component $$6)) {
            HoverEvent hoverevent = new HoverEvent.ShowText(updateForEntity(commandSourceStack, $$6, entity, recursionDepth + 1));
    ...
    public static MutableComponent copyOnClickText(String text) {
        return wrapInSquareBrackets(
            Component.literal(text)
                .withStyle(
                    p_392614_ -> p_392614_.withColor(ChatFormatting.GREEN)
