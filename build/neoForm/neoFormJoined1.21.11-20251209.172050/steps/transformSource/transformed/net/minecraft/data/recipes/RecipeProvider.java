...
public abstract class RecipeProvider {
    ...
    protected void trimSmithing(Item template, ResourceKey<TrimPattern> pattern, ResourceKey<Recipe<?>> recipe) {
        Holder.Reference<TrimPattern> reference = this.registries.lookupOrThrow(Registries.TRIM_PATTERN).getOrThrow(pattern);
        SmithingTrimRecipeBuilder.smithingTrim(
                Ingredient.of(template), this.tag(ItemTags.TRIMMABLE_ARMOR), this.tag(ItemTags.TRIM_MATERIALS), reference, RecipeCategory.MISC
            )
    ...
    protected void shelf(ItemLike shelf, ItemLike strippedLog) {
        this.shaped(RecipeCategory.DECORATIONS, shelf, 6)
            ...
            .pattern("###")
            .group("shelf")
            .unlockedBy(getHasName(strippedLog), this.has(strippedLog))
            .save(this.output);
    }
    ...
    protected void waxRecipes(FeatureFlagSet requiredFeatures) {
        net.neoforged.neoforge.common.DataMapHooks.INVERSE_WAXABLES_DATAMAP
            .forEach(
                (p_470522_, p_470523_) -> {
                    if (p_470523_.requiredFeatures().isSubsetOf(requiredFeatures)) {
                        Pair<RecipeCategory, String> pair = HoneycombItem.WAXED_RECIPES
                            .getOrDefault(p_470523_, Pair.of(RecipeCategory.BUILDING_BLOCKS, getItemName(p_470523_)));
                        RecipeCategory recipecategory = pair.getFirst();
                        String s = pair.getSecond();
                        this.shapeless(recipecategory, p_470523_)
                            .requires(p_470522_)
                            .requires(Items.HONEYCOMB)
                            .group(s)
                            .unlockedBy(getHasName(p_470522_), this.has(p_470522_))
                            .save(this.output, getConversionRecipeName(p_470523_, Items.HONEYCOMB));
                    }
                }
            );
    }
    ...
    protected void waxedChiseled(Block chiseledBlock, Block cutBlock) {
        this.shaped(RecipeCategory.BUILDING_BLOCKS, chiseledBlock)
            .define('M', cutBlock)
            .pattern(" M ")
            .pattern(" M ")
            .group(getItemName(chiseledBlock))
            .unlockedBy(getHasName(cutBlock), this.has(cutBlock))
            .save(this.output);
    }
    ...
    protected static Criterion<InventoryChangeTrigger.TriggerInstance> inventoryTrigger(ItemPredicate... items) {
                        ...
                        RecipeOutput recipeoutput = new RecipeOutput() {
                            ...
                            public void accept(ResourceKey<Recipe<?>> p_380360_, Recipe<?> p_362882_, @Nullable AdvancementHolder p_364507_, net.neoforged.neoforge.common.conditions.ICondition... conditions) {
                                if (!set.add(p_380360_)) {
                                    throw new IllegalStateException("Duplicate recipe " + p_380360_.identifier());
                                } else {
                            ...
                            private void saveRecipe(ResourceKey<Recipe<?>> p_380099_, Recipe<?> p_364792_, net.neoforged.neoforge.common.conditions.ICondition... conditions) {
                                list.add(
                                    DataProvider.saveStable(p_364823_, p_364211_, Recipe.CONDITIONAL_CODEC, Optional.of(new net.neoforged.neoforge.common.conditions.WithConditions<>(p_364792_, conditions)), packoutput$pathprovider.json(p_380099_.identifier()))
                                );
                            }
