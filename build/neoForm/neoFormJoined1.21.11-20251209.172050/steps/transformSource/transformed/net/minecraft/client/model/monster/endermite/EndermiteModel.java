...
public class EndermiteModel extends EntityModel<EntityRenderState> {
    ...
    public EndermiteModel(ModelPart p_479245_) {
        super(p_479245_);

        for (int i = 0; i < 4; i++) {
            this.bodyParts[i] = p_479245_.getChild(createSegmentName(i));
        }
    }
    ...
    public void setupAnim(EntityRenderState p_479806_) {
        super.setupAnim(p_479806_);

        for (int i = 0; i < this.bodyParts.length; i++) {
            this.bodyParts[i].yRot = Mth.cos(p_479806_.ageInTicks * 0.9F + i * 0.15F * (float) Math.PI) * (float) Math.PI * 0.01F * (1 + Math.abs(i - 2));
            this.bodyParts[i].x = Mth.sin(p_479806_.ageInTicks * 0.9F + i * 0.15F * (float) Math.PI) * (float) Math.PI * 0.1F * Math.abs(i - 2);
        }
    }
}
