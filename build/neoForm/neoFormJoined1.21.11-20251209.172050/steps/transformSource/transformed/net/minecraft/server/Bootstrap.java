...
public class Bootstrap {
    ...
    private static <T> void checkTranslations(Iterable<T> objects, Function<T, String> objectToKeyFunction, Set<String> translationSet) {
        ...
        objects.forEach(p_135883_ -> {
            String s = objectToKeyFunction.apply((T)p_135883_);
            if (!language.has(s)) {
    ...
    private static void checkGameruleTranslations(final Set<String> translations) {
        ...
        gamerules.visitGameRuleTypes(new GameRuleTypeVisitor() {
            @Override
            public <T> void visit(GameRule<T> p_461214_) {
                if (!language.has(p_461214_.getDescriptionId())) {
                    translations.add(p_461214_.id());
                }
            }
        });
    }
    ...
    public static Set<String> getMissingTranslations() {
        ...
        checkTranslations(BuiltInRegistries.CUSTOM_STAT, p_466140_ -> "stat." + p_466140_.toString().replace(':', '.'), set);
