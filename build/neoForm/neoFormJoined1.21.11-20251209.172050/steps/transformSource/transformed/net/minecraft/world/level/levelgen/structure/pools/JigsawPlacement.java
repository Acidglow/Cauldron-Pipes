...
public class JigsawPlacement {
    ...
    public static Optional<Structure.GenerationStub> addPieces(
        ...
        if (structurepoolelement == EmptyPoolElement.INSTANCE) {
            ...
            if (startJigsawName.isPresent()) {
                ...
                if (optional.isEmpty()) {
                    LOGGER.error(
                        ...
                        identifier,
                        startPool.unwrapKey().map(p_466726_ -> p_466726_.identifier().toString()).orElse("<unregistered>")
                    );
                    return Optional.empty();
                }
            ...
            int k = projectStartToHeightmap.isEmpty()
                ? blockpos1.getY()
                : pos.getY() + chunkgenerator.getFirstFreeHeight(i, j, projectStartToHeightmap.get(), levelheightaccessor, context.randomState());
                        ...
                        p_352014_ -> {
                            ...
                            if (maxDepth > 0) {
                                AABB aabb = new AABB(
                                    ...
                                    Math.max(i1 - maxDistanceFromCenter.vertical(), levelheightaccessor.getMinY() + dimensionPadding.bottom()),
                                    j - maxDistanceFromCenter.horizontal(),
                                    i + maxDistanceFromCenter.horizontal() + 1,
                                    Math.min(i1 + maxDistanceFromCenter.vertical() + 1, levelheightaccessor.getMaxY() + 1 - dimensionPadding.top()),
                                    j + maxDistanceFromCenter.horizontal() + 1
                                );
    ...
    private static Optional<BlockPos> getRandomNamedJigsaw(
        ...
        for (StructureTemplate.JigsawBlockInfo structuretemplate$jigsawblockinfo : element.getShuffledJigsawBlocks(structureTemplateManager, pos, rotation, random)) {
            if (startJigsawName.equals(structuretemplate$jigsawblockinfo.name())) {
    ...
    private static void addPieces(
        ...
        JigsawPlacement.Placer jigsawplacement$placer = new JigsawPlacement.Placer(pools, maxDepth, chunkGenerator, structureTemplateManager, pieces, random);
    ...
    static final class Placer {
        ...
        void tryPlacingChildren(
            ...
            for (StructureTemplate.JigsawBlockInfo structuretemplate$jigsawblockinfo : structurepoolelement.getShuffledJigsawBlocks(
                ...
                ResourceKey<StructureTemplatePool> resourcekey = poolAliasLookup.lookup(structuretemplate$jigsawblockinfo.pool());
                        ...
                        if (holder1.value().size() == 0 && !holder1.is(Pools.EMPTY)) {
                            JigsawPlacement.LOGGER
                                .warn(
                                    ...
                                    holder1.unwrapKey().map(p_466727_ -> p_466727_.identifier().toString()).orElse("<unregistered>")
                                );
                        } else {
                                ...
                                for (Rotation rotation1 : Rotation.getShuffled(this.random)) {
                                    ...
                                    if (useExpansionHack && boundingbox1.getYSpan() <= 16) {
                                        i1 = list1.stream()
                                            .mapToInt(
                                                p_379284_ -> {
                                                    StructureTemplate.StructureBlockInfo structuretemplate$structureblockinfo1 = p_379284_.info();
                                                    if (!boundingbox1.isInside(
                                                        ...
                                                        ResourceKey<StructureTemplatePool> resourcekey1 = poolAliasLookup.lookup(p_379284_.pool());
