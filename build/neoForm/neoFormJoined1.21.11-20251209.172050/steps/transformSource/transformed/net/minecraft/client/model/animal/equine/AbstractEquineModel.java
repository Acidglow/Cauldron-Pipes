...
public abstract class AbstractEquineModel<T extends EquineRenderState> extends EntityModel<T> {
    ...
    public void setupAnim(T p_481147_) {
        super.setupAnim(p_481147_);
        float f = Mth.clamp(p_481147_.yRot, -20.0F, 20.0F);
        float f1 = p_481147_.xRot * (float) (Math.PI / 180.0);
        float f2 = p_481147_.walkAnimationSpeed;
        float f3 = p_481147_.walkAnimationPos;
        if (f2 > 0.2F) {
            f1 += Mth.cos(f3 * 0.8F) * 0.15F * f2;
        }

        float f4 = p_481147_.eatAnimation;
        float f5 = p_481147_.standAnimation;
        float f6 = 1.0F - f5;
        float f7 = p_481147_.feedingAnimation;
        boolean flag = p_481147_.animateTail;
        this.headParts.xRot = (float) (Math.PI / 6) + f1;
        this.headParts.yRot = f * (float) (Math.PI / 180.0);
        float f8 = p_481147_.isInWater ? 0.2F : 1.0F;
        float f9 = Mth.cos(f8 * f3 * 0.6662F + (float) Math.PI);
        float f10 = f9 * 0.8F * f2;
        float f11 = (1.0F - Math.max(f5, f4)) * ((float) (Math.PI / 6) + f1 + f7 * Mth.sin(p_481147_.ageInTicks) * 0.05F);
        this.headParts.xRot = f5 * ((float) (Math.PI / 12) + f1) + f4 * (2.1816616F + Mth.sin(p_481147_.ageInTicks) * 0.05F) + f11;
        this.headParts.yRot = f5 * f * (float) (Math.PI / 180.0) + (1.0F - Math.max(f5, f4)) * this.headParts.yRot;
        float f12 = p_481147_.ageScale;
        ...
        float f14 = Mth.cos(p_481147_.ageInTicks * 0.6F + (float) Math.PI);
        ...
        if (flag) {
            this.tail.yRot = Mth.cos(p_481147_.ageInTicks * 0.7F);
        } else {
            this.tail.yRot = 0.0F;
        }
    }
