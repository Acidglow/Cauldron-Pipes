...
public record VanillaChargedCreeperExplosionLoot(HolderLookup.Provider registries) implements LootTableSubProvider {
    ...
    @Override
    public void generate(BiConsumer<ResourceKey<LootTable>, LootTable.Builder> p_433530_) {
        ...
        for (VanillaChargedCreeperExplosionLoot.Entry vanillachargedcreeperexplosionloot$entry : ENTRIES) {
            p_433530_.accept(
                ...
                LootTable.lootTable()
                    .withPool(
                        LootPool.lootPool().setRolls(ConstantValue.exactly(1.0F)).add(LootItem.lootTableItem(vanillachargedcreeperexplosionloot$entry.item))
                    )
            );
