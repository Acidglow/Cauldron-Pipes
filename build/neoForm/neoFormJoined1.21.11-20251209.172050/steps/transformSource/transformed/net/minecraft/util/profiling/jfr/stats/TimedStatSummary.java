...
public record TimedStatSummary<T extends TimedStat>(
    ...
    public static <T extends TimedStat> Optional<TimedStatSummary<T>> summary(List<T> items) {
        if (items.isEmpty()) {
            return Optional.empty();
        } else {
            List<T> list = items.stream().sorted(Comparator.comparing(TimedStat::duration)).toList();
