...
public class PlayerTabOverlay {
    ...
    private static final Comparator<PlayerInfo> PLAYER_COMPARATOR = Comparator.<PlayerInfo>comparingInt(p_359084_ -> -p_359084_.getTabListOrder())
        ...
        .thenComparing(p_442308_ -> p_442308_.getProfile().name(), String::compareToIgnoreCase);
    ...
    public void render(GuiGraphics guiGraphics, int width, Scoreboard scoreboard, @Nullable Objective objective) {
        ...
        if (!this.healthStates.isEmpty()) {
            Set<UUID> set = list.stream().map(p_442309_ -> p_442309_.getProfile().id()).collect(Collectors.toSet());
        ...
        for (int k4 = 0; k4 < j2; k4++) {
            ...
            if (k4 < list.size()) {
                ...
                if (flag1) {
                    ...
                    PlayerFaceRenderer.draw(guiGraphics, playerinfo1.getSkin().body().texturePath(), l1, i2, 8, playerinfo1.showHat(), flag, -1);
                    l1 += 9;
                }
                ...
                if (objective != null && playerinfo1.getGameMode() != GameType.SPECTATOR) {
                    ...
                    int k5 = j5 + i3;
                    if (k5 - j5 > 5) {
                        this.renderTablistScore(objective, i2, playertaboverlay$scoredisplayentry, j5, k5, gameprofile.id(), guiGraphics);
                    }
                }

                this.renderPingIcon(guiGraphics, j3, l1 - (flag1 ? 9 : 0), i2, playerinfo1);
            }
        }
    ...
    protected void renderPingIcon(GuiGraphics guiGraphics, int width, int x, int y, PlayerInfo playerInfo) {
        ...
        guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, identifier, x + width - 11, y, 10, 8);
    }
    ...
    private void renderTablistHearts(int y, int minX, int maxX, UUID playerUuid, GuiGraphics guiGraphics, int health) {
        ...
        playertaboverlay$healthstate.update(health, this.gui.getGuiTicks());
        ...
        if (i > 0) {
            ...
            if (k <= 3) {
                float f1 = Mth.clamp(health / 20.0F, 0.0F, 1.0F);
                ...
                guiGraphics.drawString(
                    this.minecraft.font, component1, (maxX + minX - this.minecraft.font.width(component1)) / 2, y, ARGB.opaque(j1)
                );
            } else {
                Identifier identifier = flag ? HEART_CONTAINER_BLINKING_SPRITE : HEART_CONTAINER_SPRITE;

                for (int l = i; l < j; l++) {
                    guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, identifier, minX + l * k, y, 9, 9);
                }

                for (int i1 = 0; i1 < i; i1++) {
                    guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, identifier, minX + i1 * k, y, 9, 9);
                    if (flag) {
                        if (i1 * 2 + 1 < playertaboverlay$healthstate.displayedValue()) {
                            guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, HEART_FULL_BLINKING_SPRITE, minX + i1 * k, y, 9, 9);
                        }

                        if (i1 * 2 + 1 == playertaboverlay$healthstate.displayedValue()) {
                            guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, HEART_HALF_BLINKING_SPRITE, minX + i1 * k, y, 9, 9);
                        }
                    }
                    ...
                    if (i1 * 2 + 1 < health) {
                        guiGraphics.blitSprite(
                            ...
                            i1 >= 10 ? HEART_ABSORBING_FULL_BLINKING_SPRITE : HEART_FULL_SPRITE,
                            minX + i1 * k,
                            y,
                            9,
                            9
                    ...
                    if (i1 * 2 + 1 == health) {
                        guiGraphics.blitSprite(
                            RenderPipelines.GUI_TEXTURED,
                            i1 >= 10 ? HEART_ABSORBING_HALF_BLINKING_SPRITE : HEART_HALF_SPRITE,
                            minX + i1 * k,
                            y,
                            9,
                            9
