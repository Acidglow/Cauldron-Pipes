...
public class ConfirmScreen extends Screen {
    ...
    protected void addButtons(LinearLayout layout) {
        this.yesButton = layout.addChild(Button.builder(this.yesButtonComponent, p_169259_ -> this.callback.accept(true)).build());
        this.noButton = layout.addChild(Button.builder(this.noButtonComponent, p_169257_ -> this.callback.accept(false)).build());
    }
    ...
    @Override
    public boolean keyPressed(KeyEvent p_446643_) {
        if (this.delayTicker <= 0 && p_446643_.key() == 256) {
            this.callback.accept(false);
            return true;
        } else {
            return super.keyPressed(p_446643_);
        }
    }
}
