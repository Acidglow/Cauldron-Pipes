...
public class ServerStatusPinger {
    ...
    /**
     * A list of Connections that have pending pings
     */
    private final List<Connection> connections = Collections.synchronizedList(Lists.newArrayList());

    public void pingServer(final ServerData serverData, final Runnable serverListUpdater, final Runnable stateUpdater, final EventLoopGroupHolder group) throws UnknownHostException {
        ...
        if (optional.isEmpty()) {
            ...
            final Connection connection = Connection.connectToServer(inetsocketaddress, group, null);
            ...
            ClientStatusPacketListener clientstatuspacketlistener = new ClientStatusPacketListener() {
                ...
                public void handleStatusResponse(ClientboundStatusResponsePacket p_105489_) {
                    if (this.receivedPing) {
                        ...
                        serverstatus.players().ifPresentOrElse(p_445196_ -> {
                            serverData.status = ServerStatusPinger.formatPlayerCount(p_445196_.online(), p_445196_.max());
                            serverData.players = p_445196_;
                            if (!p_445196_.sample().isEmpty()) {
                                List<Component> list = new ArrayList<>(p_445196_.sample().size());

                                for (NameAndId nameandid : p_445196_.sample()) {
                                    Component component;
                                ...
                                if (p_445196_.sample().size() < p_445196_.online()) {
                                    list.add(Component.translatable("multiplayer.status.and_more", p_445196_.online() - p_445196_.sample().size()));
                                }
                ...
                public void onDisconnect(DisconnectionDetails p_350468_) {
                    if (!this.success) {
                        ServerStatusPinger.this.onPingFailed(p_350468_.reason(), serverData);
                        ServerStatusPinger.this.pingLegacyServer(inetsocketaddress, serveraddress, serverData, group);
                    }
                }
    ...
    void pingLegacyServer(InetSocketAddress resolvedAddress, final ServerAddress serverAddress, final ServerData serverData, EventLoopGroupHolder group) {
        new Bootstrap().group(group.eventLoopGroup()).handler(new ChannelInitializer<Channel>() {
            @Override
        ...
        }).channel(group.channelCls()).connect(resolvedAddress.getAddress(), resolvedAddress.getPort());
    }
