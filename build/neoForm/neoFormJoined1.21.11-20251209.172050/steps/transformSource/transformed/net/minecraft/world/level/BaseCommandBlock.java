...
public abstract class BaseCommandBlock {
        ...
        if (level.getGameTime() == this.lastExecution) {
            ...
            if (level.isCommandBlockEnabled() && !StringUtil.isNullOrEmpty(this.command)) {
                try {
                    ...
                    try (BaseCommandBlock.CloseableCommandBlockSource basecommandblock$closeablecommandblocksource = this.createSource(level)) {
                        CommandSource commandsource = Objects.requireNonNullElse(basecommandblock$closeablecommandblocksource, CommandSource.NULL);
                        CommandSourceStack commandsourcestack = this.createCommandSourceStack(level, commandsource).withCallback((p_45418_, p_45419_) -> {
                            if (p_45418_) {
                                this.successCount++;
                            }
                        });
    ...
    public abstract CommandSourceStack createCommandSourceStack(ServerLevel level, CommandSource commandSource);
