...
public class CreativeModeInventoryScreen extends AbstractContainerScreen<CreativeModeInventoryScreen.ItemPickerMenu> {
    ...
    @Override
    public boolean charTyped(CharacterEvent p_445421_) {
        if (this.ignoreTextInput) {
            ...
            String s = this.searchBox.getValue();
            if (this.searchBox.charTyped(p_445421_)) {
    ...
    @Override
    public boolean keyPressed(KeyEvent p_446023_) {
        this.ignoreTextInput = false;
        if (!selectedTab.hasSearchBar()) {
            if (this.minecraft.options.keyChat.matches(p_446023_)) {
                this.ignoreTextInput = true;
                this.selectTab(CreativeModeTabs.searchTab());
                return true;
            } else {
                return super.keyPressed(p_446023_);
            }
        } else {
            boolean flag = !this.isCreativeSlot(this.hoveredSlot) || this.hoveredSlot.hasItem();
            boolean flag1 = InputConstants.getKey(p_446023_).getNumericKeyValue().isPresent();
            if (flag && flag1 && this.checkHotbarKeyPressed(p_446023_)) {
                this.ignoreTextInput = true;
                return true;
            } else {
                String s = this.searchBox.getValue();
                if (this.searchBox.keyPressed(p_446023_)) {
                    if (!Objects.equals(s, this.searchBox.getValue())) {
                        this.refreshSearchResults();
                    }
                    ...
                    return this.searchBox.isFocused() && this.searchBox.isVisible() && !p_446023_.isEscape() ? true : super.keyPressed(p_446023_);
                }
            }
        }
    }
    ...
    @Override
    public boolean keyReleased(KeyEvent p_445555_) {
        this.ignoreTextInput = false;
        return super.keyReleased(p_445555_);
    }
    ...
    private void updateVisibleTags(String search) {
        ...
        if (i == -1) {
            predicate = p_465531_ -> p_465531_.getPath().contains(search);
        } else {
            String s = search.substring(0, i).trim();
            String s1 = search.substring(i + 1).trim();
            predicate = p_465524_ -> p_465524_.getNamespace().contains(s) && p_465524_.getPath().contains(s1);
        }

        BuiltInRegistries.ITEM.getTags().map(HolderSet.Named::key).filter(p_465529_ -> predicate.test(p_465529_.location())).forEach(this.visibleTags::add);
    }
    ...
    @Override
    public boolean mouseClicked(MouseButtonEvent p_447250_, boolean p_433638_) {
        if (p_447250_.button() == 0) {
            double d0 = p_447250_.x() - this.leftPos;
            double d1 = p_447250_.y() - this.topPos;
            ...
            if (selectedTab.getType() != CreativeModeTab.Type.INVENTORY && this.insideScrollbar(p_447250_.x(), p_447250_.y())) {
                this.scrolling = this.canScroll();
                return true;
            }
        }

        return super.mouseClicked(p_447250_, p_433638_);
    }

    @Override
    public boolean mouseReleased(MouseButtonEvent p_445386_) {
        if (p_445386_.button() == 0) {
            double d0 = p_445386_.x() - this.leftPos;
            double d1 = p_445386_.y() - this.topPos;
        ...
        return super.mouseReleased(p_445386_);
    }
    ...
    @Override
    public boolean mouseDragged(MouseButtonEvent p_445545_, double p_98535_, double p_98536_) {
        if (this.scrolling) {
            int i = this.topPos + 18;
            int j = i + 112;
            this.scrollOffs = ((float)p_445545_.y() - i - 7.5F) / (j - i - 15.0F);
            this.scrollOffs = Mth.clamp(this.scrollOffs, 0.0F, 1.0F);
            this.menu.scrollTo(this.scrollOffs);
            return true;
        } else {
            return super.mouseDragged(p_445545_, p_98535_, p_98536_);
        }
    }
    ...
    public List<Component> getTooltipFromContainerItem(ItemStack p_281769_) {
        ...
        if (list.isEmpty()) {
            ...
            if (flag2 && flag) {
                this.visibleTags.forEach(p_465527_ -> {
                    if (p_281769_.is((TagKey<Item>)p_465527_)) {
                        list1.add(1, Component.literal("#" + p_465527_.location()).withStyle(ChatFormatting.DARK_PURPLE));
                    }
                });
            }
    ...
    protected void renderBg(GuiGraphics p_282663_, float p_282504_, int p_282089_, int p_282249_) {
        ...
        if (selectedTab.getType() == CreativeModeTab.Type.INVENTORY) {
            InventoryScreen.renderEntityInInventoryFollowsMouse(
                p_282663_, this.leftPos + 73, this.topPos + 6, this.leftPos + 105, this.topPos + 49, 20, 0.0625F, p_282089_, p_282249_, this.minecraft.player
            );
        }
    }
    ...
    protected boolean checkTabHovering(GuiGraphics guiGraphics, CreativeModeTab creativeModeTab, int mouseX, int mouseY) {
        ...
        if (this.isHovering(i + 3, j + 3, 21, 27, mouseX, mouseY)) {
            guiGraphics.setTooltipForNextFrame(this.font, creativeModeTab.getDisplayName(), mouseX, mouseY);
            return true;
        } else {
            return false;
        }
    ...
    protected void renderTabButton(GuiGraphics guiGraphics, int mouseX, int mouseY, CreativeModeTab tab) {
        boolean flag = tab == selectedTab;
        boolean flag1 = currentPage.isTop(tab);
        int i = currentPage.getColumn(tab);
        int j = this.leftPos + this.getTabX(tab);
        ...
        if (!flag && mouseX > j && mouseY > k && mouseX < j + 26 && mouseY < k + 32) {
            guiGraphics.requestCursor(CursorTypes.POINTING_HAND);
        }

        // TODO 1.20.2: Deal with custom tab backgrounds, and deal with transparency.
        guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, aidentifier[Mth.clamp(i, 0, aidentifier.length)], j, k, 26, 32);
        int l = j + 13 - 8;
        int i1 = k + 16 - 8 + (flag1 ? 1 : -1);
        guiGraphics.renderItem(tab.getIconItem(), l, i1);
    }
    ...
    public static void handleHotbarLoadOrSave(Minecraft client, int index, boolean load, boolean save) {
        ...
        if (load) {
            ...
            client.getNarrator().saySystemNow(component2);
            hotbarmanager.save();
        }
    }
