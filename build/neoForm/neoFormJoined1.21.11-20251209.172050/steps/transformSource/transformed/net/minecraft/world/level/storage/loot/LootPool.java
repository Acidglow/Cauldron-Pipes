...
public class LootPool {
    ...
    public void validate(ValidationContext context) {
        ...
        for (int j = 0; j < this.functions.size(); j++) {
            this.functions.get(j).validate(context.forChild(new ProblemReporter.IndexedFieldPathElement("functions", j)));
        }

        for (int k = 0; k < this.entries.size(); k++) {
            this.entries.get(k).validate(context.forChild(new ProblemReporter.IndexedFieldPathElement("entries", k)));
        }
