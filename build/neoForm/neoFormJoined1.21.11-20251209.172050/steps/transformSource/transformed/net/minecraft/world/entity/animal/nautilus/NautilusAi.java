...
public class NautilusAi {
    ...
    protected static final TargetingConditions ATTACK_TARGET_CONDITIONS = TargetingConditions.forCombat()
        .selector(
            (p_460540_, p_460541_) -> (p_460541_.getGameRules().get(GameRules.MOB_GRIEFING) || !p_460540_.getType().equals(EntityType.ARMOR_STAND))
                && p_460541_.getWorldBorder().isWithinBounds(p_460540_.getBoundingBox())
        );
    ...
    private static void initIdleActivity(Brain<Nautilus> brain) {
        brain.addActivity(
            ...
            ImmutableList.of(
                ...
                Pair.of(2, new FollowTemptation(p_454902_ -> 1.3F, p_455464_ -> p_455464_.isBaby() ? 2.5 : 3.5)),
    ...
    protected static Optional<? extends LivingEntity> findNearestValidAttackTarget(ServerLevel level, AbstractNautilus nautilus) {
        if (!BehaviorUtils.isBreeding(nautilus) && nautilus.isInWater() && !nautilus.isBaby() && !nautilus.isTame()) {
            Optional<LivingEntity> optional = BehaviorUtils.getLivingEntityFromUUIDMemory(nautilus, MemoryModuleType.ANGRY_AT)
                .filter(p_455307_ -> p_455307_.isInWater() && Sensor.isEntityAttackableIgnoringLineOfSight(level, nautilus, p_455307_));
    ...
    private static boolean isHostileTarget(LivingEntity entity) {
        return entity.isInWater() && entity.getType().is(EntityTypeTags.NAUTILUS_HOSTILES);
    }
    ...
    public static Predicate<ItemStack> getTemptations() {
        return p_456045_ -> p_456045_.is(ItemTags.NAUTILUS_FOOD);
    }
}
