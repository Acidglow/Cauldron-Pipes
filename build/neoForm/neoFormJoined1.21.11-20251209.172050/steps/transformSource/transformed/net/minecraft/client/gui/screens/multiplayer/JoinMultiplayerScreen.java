...
public class JoinMultiplayerScreen extends Screen {
    ...
    protected void init() {
        ...
        this.selectButton = linearlayout1.addChild(Button.builder(Component.translatable("selectServer.select"), p_442313_ -> {
        ...
        linearlayout1.addChild(Button.builder(Component.translatable("selectServer.direct"), p_293604_ -> {
        ...
        linearlayout2.addChild(Button.builder(CommonComponents.GUI_BACK, p_315824_ -> this.onClose()).width(74).build());
        this.layout.visitWidgets(p_438743_ -> {
            AbstractWidget abstractwidget = this.addRenderableWidget(p_438743_);
        });
    ...
    private void deleteCallback(boolean confirmed) {
        ServerSelectionList.Entry serverselectionlist$entry = this.serverSelectionList.getSelected();
        if (confirmed && serverselectionlist$entry instanceof ServerSelectionList.OnlineServerEntry) {
    ...
    @Override
    public boolean keyPressed(KeyEvent p_445969_) {
        if (super.keyPressed(p_445969_)) {
            return true;
        } else if (p_445969_.key() == 294) {
