...
public class PiglinAi {
    ...
    private static void initIdleActivity(Brain<Piglin> brain) {
        brain.addActivity(
            ...
            ImmutableList.of(
                ...
                StartAttacking.<Piglin>create((p_432552_, p_432553_) -> p_432553_.isAdult(), PiglinAi::findNearestValidAttackTarget),
    ...
    private static void initCelebrateActivity(Brain<Piglin> brain) {
        brain.addActivityAndRemoveMemoryWhenStopped(
            ...
            ImmutableList.<net.minecraft.world.entity.ai.behavior.BehaviorControl<? super Piglin>>of(
                avoidRepellent(),
                SetEntityLookTarget.create(PiglinAi::isPlayerHoldingLovedItem, 14.0F),
                StartAttacking.<Piglin>create((p_432554_, p_432555_) -> p_432555_.isAdult(), PiglinAi::findNearestValidAttackTarget),
    ...
    public static void angerNearbyPiglins(ServerLevel level, Player player, boolean requireLineOfSight) {
        ...
        list.stream().filter(PiglinAi::isIdle).filter(p_34881_ -> !requireLineOfSight || BehaviorUtils.canSee(p_34881_, player)).forEach(p_460546_ -> {
            if (level.getGameRules().get(GameRules.UNIVERSAL_ANGER)) {
                setAngerTargetToNearestTargetablePlayerIfFound(level, p_460546_, player);
            } else {
                setAngerTarget(level, p_460546_, player);
            }
        });
    }
    ...
    protected static void wasHurtBy(ServerLevel level, Piglin piglin, LivingEntity entity) {
        if (!(entity instanceof Piglin)) {
            ...
            getAvoidTarget(piglin).ifPresent(p_423349_ -> {
                if (p_423349_.getType() != entity.getType()) {
    ...
    public static boolean isWearingSafeArmor(LivingEntity entity) {
        for (EquipmentSlot equipmentslot : EquipmentSlotGroup.ARMOR) {
            if (entity.getItemBySlot(equipmentslot).makesPiglinsNeutral(entity)) {
                return true;
            }
        }
    ...
    private static BehaviorControl<LivingEntity> babySometimesRideBabyHoglin() {
        ...
        return CopyMemoryWithExpiry.create(
            p_423351_ -> p_423351_.isBaby() && setentitylooktargetsometimes$ticker.tickDownAndCheck(p_423351_.level().random),
    ...
    protected static void broadcastAngerTarget(ServerLevel level, AbstractPiglin piglin, LivingEntity angerTarget) {
        getAdultPiglins(piglin).forEach(p_423356_ -> {
            if (angerTarget.getType() != EntityType.HOGLIN || p_423356_.canHunt() && ((Hoglin)angerTarget).canBeHunted()) {
                setAngerTargetIfCloserThanCurrent(level, p_423356_, angerTarget);
            }
        });
    }
