...
public class ScoreboardCommand {
                                            ...
                                            Commands.argument("criteria", ObjectiveCriteriaArgument.criteria())
                                                ...
                                                .then(
                                                    Commands.argument("displayName", ComponentArgument.textComponent(context))
                                                        .executes(
                                                            p_400888_ -> addObjective(
                                                                p_400888_.getSource(),
                                                                StringArgumentType.getString(p_400888_, "objective"),
                                                                ObjectiveCriteriaArgument.getCriteria(p_400888_, "criteria"),
                                                                ComponentArgument.getResolvedComponent(p_400888_, "displayName")
                                                            )
                                                        )
                                                )
                                        )
                                        ...
                                        .then(
                                            Commands.literal("displayname")
                                                .then(
                                                    Commands.argument("displayName", ComponentArgument.textComponent(context))
                                                        .executes(
                                                            p_400890_ -> setDisplayName(
                                                                p_400890_.getSource(),
                                                                ObjectiveArgument.getObjective(p_400890_, "objective"),
                                                                ComponentArgument.getResolvedComponent(p_400890_, "displayName")
                                                            )
                                                        )
                                                )
                                        )
                                                    ...
                                                    Commands.argument("objective", ObjectiveArgument.objective())
                                                        .then(
                                                            Commands.argument("name", ComponentArgument.textComponent(context))
                                                                .executes(
                                                                    p_400889_ -> setScoreDisplay(
                                                                        p_400889_.getSource(),
                                                                        ScoreHolderArgument.getNamesWithDefaultWildcard(p_400889_, "targets"),
                                                                        ObjectiveArgument.getObjective(p_400889_, "objective"),
                                                                        ComponentArgument.getResolvedComponent(p_400889_, "name")
                                                                    )
                                                                )
                                                        )
                                                        .executes(
    ...
    private static ArgumentBuilder<CommandSourceStack, ?> addNumberFormats(
        ...
        return argumentBuilder.then(Commands.literal("blank").executes(p_313547_ -> executor.run(p_313547_, BlankFormat.INSTANCE)))
            .then(Commands.literal("fixed").then(Commands.argument("contents", ComponentArgument.textComponent(context)).executes(p_400892_ -> {
                Component component = ComponentArgument.getResolvedComponent(p_400892_, "contents");
                return executor.run(p_400892_, new FixedFormat(component));
            })))
        ...
        if (object2intmap.isEmpty()) {
            ...
            Object2IntMaps.fastForEach(
                ...
                p_313504_ -> source.sendSuccess(
                    () -> Component.translatable(
                        "commands.scoreboard.players.list.entity.entry", ((Objective)p_313504_.getKey()).getFormattedDisplayName(), p_313504_.getIntValue()
                    ),
                    false
                )
            );
