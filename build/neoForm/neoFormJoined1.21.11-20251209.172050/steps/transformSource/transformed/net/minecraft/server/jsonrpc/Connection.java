...
public class Connection extends SimpleChannelInboundHandler<JsonElement> {
    ...
    public void tick() {
        ...
        this.pendingRequests
            ...
            .removeIf(
                p_466327_ -> {
                    boolean flag = p_466327_.getValue().timedOut(i);
                    if (flag) {
                        p_466327_.getValue()
                            .resultFuture()
                            .completeExceptionally(
                                new ReadTimeoutException("RPC method " + p_466327_.getValue().method().key().identifier() + " timed out waiting for response")
                            );
                    }
    ...
    private @Nullable JsonObject handleIncomingRequest(@Nullable JsonElement requestId, String methodName, @Nullable JsonElement params) {
    ...
    public @Nullable JsonElement dispatchIncomingRequest(String methodName, @Nullable JsonElement params) {
        Identifier identifier = Identifier.tryParse(methodName);
        if (identifier == null) {
