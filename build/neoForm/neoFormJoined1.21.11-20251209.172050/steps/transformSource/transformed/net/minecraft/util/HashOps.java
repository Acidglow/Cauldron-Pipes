...
public class HashOps implements DynamicOps<HashCode> {
    ...
    private static Hasher hashMap(Hasher hasher, Map<HashCode, HashCode> map) {
        ...
        map.entrySet()
            ...
            .forEach(p_412485_ -> hasher.putBytes(p_412485_.getKey().asBytes()).putBytes(p_412485_.getValue().asBytes()));
    ...
    static Hasher hashMap(Hasher hasher, Stream<Pair<HashCode, HashCode>> map) {
        hasher.putByte((byte)2);
        map.sorted(MAPLIKE_ENTRY_ORDER)
            .forEach(p_412404_ -> hasher.putBytes(p_412404_.getFirst().asBytes()).putBytes(p_412404_.getSecond().asBytes()));
        hasher.putByte((byte)3);
        return hasher;
    }
    ...
    public HashCode createList(Stream<HashCode> input) {
        ...
        input.forEach(p_412370_ -> hasher.putBytes(p_412370_.asBytes()));
    ...
    public <U> U convertTo(DynamicOps<U> ops, HashCode hashCode) {
    ...
    public HashCode update(HashCode input, String key, Function<HashCode, HashCode> updater) {
        return input;
    }

    public HashCode updateGeneric(HashCode input, HashCode key, Function<HashCode, HashCode> updater) {
        return input;
    }
    ...
    public DataResult<HashCode> mergeToMap(HashCode list, HashCode key, HashCode value) {
        return this.isEmpty(list) ? DataResult.success(this.createMap(Map.of(key, value))) : unsupported();
    }

    public DataResult<HashCode> mergeToMap(HashCode list, Map<HashCode, HashCode> values) {
        return this.isEmpty(list) ? DataResult.success(this.createMap(values)) : unsupported();
    }

    public DataResult<HashCode> mergeToMap(HashCode list, MapLike<HashCode> values) {
        return this.isEmpty(list) ? DataResult.success(this.createMap(values.entries())) : unsupported();
    }
    ...
    class ListHashBuilder extends AbstractListBuilder<HashCode, Hasher> {
        ...
        protected DataResult<HashCode> build(Hasher p_412335_, HashCode p_412267_) {
            assert p_412267_.equals(HashOps.this.empty);

            p_412335_.putByte((byte)5);
            return DataResult.success(p_412335_.hash());
        }
    }
    ...
    final class MapHashBuilder extends AbstractUniversalBuilder<HashCode, List<Pair<HashCode, HashCode>>> {
        ...
        protected List<Pair<HashCode, HashCode>> append(HashCode p_412594_, HashCode p_412246_, List<Pair<HashCode, HashCode>> p_412564_) {
            p_412564_.add(Pair.of(p_412594_, p_412246_));
            return p_412564_;
        }

        protected DataResult<HashCode> build(List<Pair<HashCode, HashCode>> p_412648_, HashCode p_412523_) {
            assert HashOps.this.isEmpty(p_412523_);

            return DataResult.success(HashOps.hashMap(HashOps.this.hashFunction.newHasher(), p_412648_.stream()).hash());
        }
    }
}
