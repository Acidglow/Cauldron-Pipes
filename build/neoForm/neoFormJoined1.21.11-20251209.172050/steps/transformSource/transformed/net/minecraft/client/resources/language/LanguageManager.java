...
public class LanguageManager implements ResourceManagerReloadListener {
    ...
    private static Map<String, LanguageInfo> extractLanguages(Stream<PackResources> packResources) {
        ...
        packResources.forEach(p_389370_ -> {
            try {
                LanguageMetadataSection languagemetadatasection = p_389370_.getMetadataSection(LanguageMetadataSection.TYPE);
                if (languagemetadatasection != null) {
                    languagemetadatasection.languages().forEach(map::putIfAbsent);
                }
            } catch (IOException | RuntimeException runtimeexception) {
                LOGGER.warn("Unable to parse language metadata section of resourcepack: {}", p_389370_.packId(), runtimeexception);
            }
        });
