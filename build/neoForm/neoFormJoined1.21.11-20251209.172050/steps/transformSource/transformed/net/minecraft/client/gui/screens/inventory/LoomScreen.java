...
public class LoomScreen extends AbstractContainerScreen<LoomMenu> {
            ...
            for (int l1 = 0; l1 < 4; l1++) {
                for (int i2 = 0; i2 < 4; i2++) {
                    ...
                    if (k2 == this.menu.getSelectedBannerPatternIndex()) {
                        ...
                        p_282870_.setTooltipForNextFrame(
                            Component.translatable(holder.value().translationKey() + "." + dyecolor1.getName()), p_283331_, p_283087_
                        );
    ...
    @Override
    public boolean mouseClicked(MouseButtonEvent p_445823_, boolean p_433366_) {
        if (this.displayPatterns) {
            ...
            for (int k = 0; k < 4; k++) {
                for (int l = 0; l < 4; l++) {
                    double d0 = p_445823_.x() - (i + l * 14);
                    double d1 = p_445823_.y() - (j + k * 14);
            ...
            i = this.leftPos + 119;
            j = this.topPos + 9;
            if (p_445823_.x() >= i && p_445823_.x() < i + 12 && p_445823_.y() >= j && p_445823_.y() < j + 56) {
                this.scrolling = true;
            }
        }

        return super.mouseClicked(p_445823_, p_433366_);
    }

    @Override
    public boolean mouseDragged(MouseButtonEvent p_446758_, double p_99087_, double p_99088_) {
        int i = this.totalRowCount() - 4;
        if (this.scrolling && this.displayPatterns && i > 0) {
            int j = this.topPos + 13;
            int k = j + 56;
            this.scrollOffs = ((float)p_446758_.y() - j - 7.5F) / (k - j - 15.0F);
            this.scrollOffs = Mth.clamp(this.scrollOffs, 0.0F, 1.0F);
            this.startRow = Math.max((int)(this.scrollOffs * i + 0.5), 0);
            return true;
        } else {
            return super.mouseDragged(p_446758_, p_99087_, p_99088_);
        }
    }

    @Override
    public boolean mouseReleased(MouseButtonEvent p_470627_) {
        this.scrolling = false;
        return super.mouseReleased(p_470627_);
    }

    @Override
    public boolean mouseScrolled(double p_99079_, double p_99080_, double p_99081_, double p_295430_) {
        if (super.mouseScrolled(p_99079_, p_99080_, p_99081_, p_295430_)) {
            return true;
        } else {
            int i = this.totalRowCount() - 4;
            if (this.displayPatterns && i > 0) {
                float f = (float)p_295430_ / i;
