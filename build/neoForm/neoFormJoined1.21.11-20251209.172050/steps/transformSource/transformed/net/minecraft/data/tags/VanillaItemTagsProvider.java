...
public class VanillaItemTagsProvider extends IntrinsicHolderTagsProvider<Item> {
    ...
    protected void addTags(HolderLookup.Provider p_255639_) {
        (new BlockItemTagsProvider() {
            @Override
            protected TagAppender<Block, Block> tag(TagKey<Block> p_422507_, TagKey<Item> p_421704_) {
                return new VanillaItemTagsProvider.BlockToItemConverter(VanillaItemTagsProvider.this.tag(p_421704_));
            }
        }).run();
        ...
        this.tag(ItemTags.TRIM_MATERIALS)
            .addAll(
                p_255639_.lookupOrThrow(Registries.ITEM)
                    .listElements()
                    .filter(p_399387_ -> p_399387_.value().components().has(DataComponents.PROVIDES_TRIM_MATERIAL))
                    .sorted(Comparator.comparing(p_466089_ -> p_466089_.key().identifier()))
