...
public class SplashManager extends SimplePreparableReloadListener<List<Component>> {
     ...
     */
    protected List<Component> prepare(ResourceManager resourceManager, ProfilerFiller profiler) {
        try {
            ...
            if (splashes.sourcePackId().equals("vanilla"))
                return net.neoforged.neoforge.client.resources.NeoForgeSplashHooks.loadSplashes(resourceManager);

            List list;
            try (BufferedReader bufferedreader = splashes.openAsReader()) {
                list = bufferedreader.lines()
                    .map(String::trim)
                    .filter(p_118876_ -> p_118876_.hashCode() != 125780783)
    ...
    protected void apply(List<Component> object, ResourceManager resourceManager, ProfilerFiller profiler) {
        this.splashes = List.copyOf(object);
    }
