...
public record VanillaShearingLoot(HolderLookup.Provider registries) implements LootTableSubProvider {
    @Override
    public void generate(BiConsumer<ResourceKey<LootTable>, LootTable.Builder> p_332018_) {
        p_332018_.accept(
        ...
        LootData.WOOL_ITEM_BY_DYE
            .forEach(
                (p_361524_, p_361723_) -> p_332018_.accept(
                    BuiltInLootTables.SHEAR_SHEEP_BY_DYE.get(p_361524_),
                    LootTable.lootTable().withPool(LootPool.lootPool().setRolls(UniformGenerator.between(1.0F, 3.0F)).add(LootItem.lootTableItem(p_361723_)))
                )
            );
        p_332018_.accept(
