...
public record KeyframeTrack<T>(List<Keyframe<T>> keyframes, EasingType easingType) {
    ...
    public static <T> MapCodec<KeyframeTrack<T>> mapCodec(Codec<T> valueCodec) {
        ...
        return RecordCodecBuilder.mapCodec(
            p_469037_ -> p_469037_.group(
                ...
                .apply(p_469037_, KeyframeTrack::new)
        );
    }
    ...
    public static DataResult<KeyframeTrack<?>> validatePeriod(KeyframeTrack<?> track, int max) {
        for (Keyframe<?> keyframe : track.keyframes()) {
            int i = keyframe.ticks();
            if (i < 0 || i > max) {
                return DataResult.error(() -> "Keyframe at tick " + keyframe.ticks() + " must be in range [0; " + max + "]");
            }
        }
