...
public class DragonSittingFlamingPhase extends AbstractDragonSittingPhase {
            ...
            for (int i = 0; i < 8; i++) {
                ...
                for (int j = 0; j < 6; j++) {
                    this.dragon
                        .level()
                        .addParticle(
                            PowerParticleOption.create(ParticleTypes.DRAGON_BREATH, 1.0F), d3, d4, d5, -vec3.x * 0.08F * j, -vec3.y * 0.6F, -vec3.z * 0.08F * j
                        );
                }
    ...
    @Override
    public void doServerTick(ServerLevel p_376561_) {
        this.flameTicks++;
        if (this.flameTicks >= 200) {
            ...
            while (p_376561_.isEmptyBlock(blockpos$mutableblockpos)) {
                if (--d3 < 0.0) {
                    d3 = d2;
                    break;
                }
            ...
            d3 = Mth.floor(d3) + 1;
            this.flame = new AreaEffectCloud(p_376561_, d0, d3, d1);
            ...
            p_376561_.addFreshEntity(this.flame);
        }
    }
