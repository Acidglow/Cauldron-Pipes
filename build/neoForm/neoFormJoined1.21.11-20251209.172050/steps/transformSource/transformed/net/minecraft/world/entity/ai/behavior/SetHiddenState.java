    ...
    public static BehaviorControl<LivingEntity> create(int stayHiddenSeconds, int closeEnoughDist) {
        ...
        return BehaviorBuilder.create(
            p_259055_ -> p_259055_.group(p_259055_.present(MemoryModuleType.HIDING_PLACE), p_259055_.present(MemoryModuleType.HEARD_BELL_TIME))
                .apply(p_259055_, (p_260296_, p_260145_) -> (p_466585_, p_466586_, p_466587_) -> {
                    long j = p_259055_.<Long>get(p_260145_);
                    boolean flag = j + 300L <= p_466587_;
                    if (mutableint.intValue() <= i && !flag) {
                        BlockPos blockpos = p_259055_.get(p_260296_).pos();
                        if (blockpos.closerThan(p_466586_.blockPosition(), closeEnoughDist)) {
                        ...
                        p_260145_.erase();
                        p_260296_.erase();
                        p_466586_.getBrain().updateActivityFromSchedule(p_466585_.environmentAttributes(), p_466585_.getGameTime(), p_466586_.position());
