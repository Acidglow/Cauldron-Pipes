...
public class Mth {
    ...
    private static final float[] SIN = Util.make(new float[65536], p_466443_ -> {
        for (int i = 0; i < p_466443_.length; i++) {
            p_466443_[i] = (float)Math.sin(i / 10430.378350470453);
        }
    });
    ...
    public static IntStream outFromOrigin(int input, int lowerBound, int upperBound, int steps) {
        if (lowerBound > upperBound) {
            ...
            return IntStream.iterate(i, p_216282_ -> {
                ...
                return i - j >= lowerBound || i + j <= upperBound;
            }, p_216260_ -> {
                boolean flag = p_216260_ <= i;
                int j = Math.abs(i - p_216260_);
                boolean flag1 = i + j + steps <= upperBound;
                if (!flag || !flag1) {
                    int k = i - j - (flag ? steps : 0);
                    if (k >= lowerBound) {
                        return k;
                    }
                }
