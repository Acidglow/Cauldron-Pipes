...
public class GameRuleRegistryFix extends DataFix {
    ...
    protected TypeRewriteRule makeRule() {
        return this.fixTypeEverywhereTyped(
            ...
            p_460872_ -> p_460872_.update(
                DSL.remainderFinder(),
                p_460907_ -> p_460907_.renameAndFixField(
                    "GameRules",
                    "game_rules",
                    p_466460_ -> {
                        boolean flag = Boolean.parseBoolean(p_466460_.get("doFireTick").asString("true"));
                        boolean flag1 = Boolean.parseBoolean(p_466460_.get("allowFireTicksAwayFromPlayer").asString("false"));
                        int i;
                        if (!flag) {
                            i = 0;
                        } else if (!flag1) {
                        ...
                        if (i != 128) {
                            p_466460_ = p_466460_.set("minecraft:fire_spread_radius_around_player", p_466460_.createInt(i));
                        }

                        return p_466460_.remove("spawnChunkRadius")
                            ...
                            .renameAndFixField("commandModificationBlockLimit", "minecraft:max_block_modifications", p_461159_ -> convertInteger(p_461159_, 1))
                            ...
                            .renameAndFixField("maxCommandChainLength", "minecraft:max_command_sequence_length", p_460920_ -> convertInteger(p_460920_, 0))
                            .renameAndFixField("maxCommandForkCount", "minecraft:max_command_forks", p_460781_ -> convertInteger(p_460781_, 0))
                            .renameAndFixField("maxEntityCramming", "minecraft:max_entity_cramming", p_460988_ -> convertInteger(p_460988_, 0))
                            ...
                            .renameAndFixField(
                                "playersNetherPortalCreativeDelay", "minecraft:players_nether_portal_creative_delay", p_460783_ -> convertInteger(p_460783_, 0)
                            )
                            .renameAndFixField(
                                "playersNetherPortalDefaultDelay", "minecraft:players_nether_portal_default_delay", p_460630_ -> convertInteger(p_460630_, 0)
                            )
                            .renameAndFixField("playersSleepingPercentage", "minecraft:players_sleeping_percentage", p_460948_ -> convertInteger(p_460948_, 0))
                            ...
                            .renameAndFixField("randomTickSpeed", "minecraft:random_tick_speed", p_466461_ -> convertInteger(p_466461_, 0))
                            ...
                            .renameAndFixField("snowAccumulationHeight", "minecraft:max_snow_accumulation_height", p_461139_ -> convertInteger(p_461139_, 0, 8))
    ...
    private static Dynamic<?> convertInteger(Dynamic<?> data) {
        return convertInteger(data, Integer.MIN_VALUE, Integer.MAX_VALUE);
    }

    private static Dynamic<?> convertInteger(Dynamic<?> data, int minValue) {
        return convertInteger(data, minValue, Integer.MAX_VALUE);
    }

    private static Dynamic<?> convertInteger(Dynamic<?> data, int minValue, int maxValue) {
        String s = data.asString("");

        try {
            int i = Integer.parseInt(s);
            return data.createInt(Mth.clamp(i, minValue, maxValue));
        } catch (NumberFormatException numberformatexception) {
            return data;
        }
    }

    private static Dynamic<?> convertBoolean(Dynamic<?> data) {
        return data.createBoolean(Boolean.parseBoolean(data.asString("")));
    }

    private static Dynamic<?> convertBooleanInverted(Dynamic<?> data) {
        return data.createBoolean(!Boolean.parseBoolean(data.asString("")));
    }
}
