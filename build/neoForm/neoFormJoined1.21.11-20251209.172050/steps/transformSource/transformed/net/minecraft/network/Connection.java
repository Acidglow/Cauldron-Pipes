...
public class Connection extends SimpleChannelInboundHandler<Packet<?>> {
        ...
        if (serverboundProtocol.id() != clientboundProtocol.id()) {
            ...
            this.runOnceConnected(
                p_428098_ -> {
                    ...
                    p_428098_.sendPacket(
                        new ClientIntentionPacket(SharedConstants.getCurrentVersion().protocolVersion(), hostName, port, intention), null, true
                    );
    ...
    public void send(Packet<?> packet, @Nullable ChannelFutureListener sendListener, boolean flush) {
        if (this.isConnected()) {
            ...
            this.pendingActions.add(p_428102_ -> p_428102_.sendPacket(packet, sendListener, flush));
        }
    }
    ...
    public static Connection connectToServer(InetSocketAddress address, EventLoopGroupHolder group, @Nullable LocalSampleLogger bandwidthLogger) {
        Connection connection = new Connection(PacketFlow.CLIENTBOUND);
        if (bandwidthLogger != null) {
            connection.setBandwidthLogger(bandwidthLogger);
        }

        ChannelFuture channelfuture = connect(address, group, connection);
        channelfuture.syncUninterruptibly();
        return connection;
    }

    public static ChannelFuture connect(InetSocketAddress address, EventLoopGroupHolder group, final Connection connection) {
        net.neoforged.neoforge.network.DualStackUtils.checkIPv6(address.getAddress());
        return new Bootstrap().group(group.eventLoopGroup()).handler(new ChannelInitializer<Channel>() {
            @Override
        ...
        }).channel(group.channelCls()).connect(address.getAddress(), address.getPort());
    }
