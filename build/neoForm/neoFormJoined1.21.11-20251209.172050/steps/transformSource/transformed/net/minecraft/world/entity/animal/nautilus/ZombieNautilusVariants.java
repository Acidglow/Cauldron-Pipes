...
public class ZombieNautilusVariants {
    ...
    private static ResourceKey<ZombieNautilusVariant> createKey(Identifier name) {
        return ResourceKey.create(Registries.ZOMBIE_NAUTILUS_VARIANT, name);
    }
    ...
    private static void register(
        ...
        ZombieNautilusVariant.ModelType model,
        String name,
        TagKey<Biome> spawnBiomes
    ) {
        HolderSet<Biome> holderset = context.lookup(Registries.BIOME).getOrThrow(spawnBiomes);
        register(context, key, model, name, SpawnPrioritySelectors.single(new BiomeCheck(holderset), 1));
    }
    ...
    private static void register(
        ...
        Identifier identifier = Identifier.withDefaultNamespace("entity/nautilus/" + name);
        context.register(key, new ZombieNautilusVariant(new ModelAndTexture<>(model, identifier), spawnConditions));
    }
}
