...
public class DiscoveryService {
    public static DiscoveryService.DiscoverResponse discover(List<SchemaComponent<?>> schemaRegistry) {
        ...
        BuiltInRegistries.INCOMING_RPC_METHOD.listElements().forEach(p_466332_ -> {
            if (p_466332_.value().attributes().discoverable()) {
                list.add(p_466332_.value().info().named(p_466332_.key().identifier()));
            }
        });
        BuiltInRegistries.OUTGOING_RPC_METHOD.listElements().forEach(p_466330_ -> {
            if (p_466330_.value().attributes().discoverable()) {
                list.add(p_466330_.value().info().named(p_466330_.key().identifier()));
            }
        });
