...
public class BiomeAmbientSoundsHandler implements AmbientSoundHandler {
    ...
    public void tick() {
        ...
        if (!Objects.equals(holder, this.previousLoopSound)) {
            ...
            if (holder != null) {
                this.loopSounds.compute(holder, (p_457909_, p_426941_) -> {
                    if (p_426941_ == null) {
                        p_426941_ = new BiomeAmbientSoundsHandler.LoopSoundInstance(holder.value());
                        this.soundManager.play(p_426941_);
                    }

                    p_426941_.fadeIn();
                    return (BiomeAmbientSoundsHandler.LoopSoundInstance)p_426941_;
                });
            }
        }
        ...
        ambientsounds.mood()
            .ifPresent(
                p_457356_ -> {
                    int i = p_457356_.blockSearchExtent() * 2 + 1;
                    BlockPos blockpos = BlockPos.containing(
                        this.player.getX() + this.random.nextInt(i) - p_457356_.blockSearchExtent(),
                        this.player.getEyeY() + this.random.nextInt(i) - p_457356_.blockSearchExtent(),
                        this.player.getZ() + this.random.nextInt(i) - p_457356_.blockSearchExtent()
                    );
                    int j = level.getBrightness(LightLayer.SKY, blockpos);
                    if (j > 0) {
                        this.moodiness -= j / 15.0F * 0.001F;
                    } else {
                        this.moodiness = this.moodiness - (float)(level.getBrightness(LightLayer.BLOCK, blockpos) - 1) / p_457356_.tickDelay();
                    }
