...
public class Evoker extends SpellcasterIllager {
    ...
    public Evoker(EntityType<? extends Evoker> p_481404_, Level p_480983_) {
        super(p_481404_, p_480983_);
        this.xpReward = 10;
    }
    ...
    @Override
    protected boolean considersEntityAsAlly(Entity p_481079_) {
        if (p_481079_ == this) {
            return true;
        } else if (super.considersEntityAsAlly(p_481079_)) {
            return true;
        } else {
            return p_481079_ instanceof Vex vex && vex.getOwner() != null ? this.considersEntityAsAlly(vex.getOwner()) : false;
        }
    }
    ...
    @Override
    public void applyRaidBuffs(ServerLevel p_482095_, int p_479035_, boolean p_481642_) {
    }
    ...
    class EvokerAttackSpellGoal extends SpellcasterIllager.SpellcasterUseSpellGoal {
        ...
        private void createSpellEntity(double x, double z, double minY, double maxY, float yRot, int warmupDelay) {
            ...
            if (flag) {
                Evoker.this.level()
                    .addFreshEntity(new EvokerFangs(Evoker.this.level(), x, blockpos.getY() + d0, z, yRot, warmupDelay, Evoker.this));
                Evoker.this.level().gameEvent(GameEvent.ENTITY_PLACE, new Vec3(x, blockpos.getY() + d0, z), GameEvent.Context.of(Evoker.this));
            }
        }
    ...
    public class EvokerWololoSpellGoal extends SpellcasterIllager.SpellcasterUseSpellGoal {
        private final TargetingConditions wololoTargeting = TargetingConditions.forNonCombat()
            .range(16.0)
            .selector((p_480681_, p_480339_) -> ((Sheep)p_480681_).getColor() == DyeColor.BLUE);
