...
public final class EnvironmentAttributeMap {
    ...
    public static final Codec<EnvironmentAttributeMap> CODEC = Codec.lazyInitialized(
        () -> Codec.dispatchedMap(EnvironmentAttributes.CODEC, Util.memoize((java.util.function.Function<EnvironmentAttribute<?>, Codec<? extends Entry<?, ?>>>) EnvironmentAttributeMap.Entry::createCodec))
            .xmap(EnvironmentAttributeMap::new, p_457769_ -> p_457769_.entries)
    );
    ...
    public static final Codec<EnvironmentAttributeMap> CODEC_ONLY_POSITIONAL = CODEC.validate(p_457772_ -> {
        List<EnvironmentAttribute<?>> list = p_457772_.keySet().stream().filter(p_457601_ -> !p_457601_.isPositional()).toList();
        return !list.isEmpty() ? DataResult.error(() -> "The following attributes cannot be positional: " + list) : DataResult.success(p_457772_);
    });
    final Map<EnvironmentAttribute<?>, EnvironmentAttributeMap.Entry<?, ?>> entries;

    private static EnvironmentAttributeMap filterSyncable(EnvironmentAttributeMap map) {
        return new EnvironmentAttributeMap(Map.copyOf(Maps.filterKeys(map.entries, EnvironmentAttribute::isSyncable)));
    }
    ...
    public <Value> Value applyModifier(EnvironmentAttribute<Value> attribute, Value oldValue) {
        EnvironmentAttributeMap.Entry<Value, ?> entry = this.get(attribute);
        return entry != null ? entry.applyModifier(oldValue) : oldValue;
    }
    ...
    public boolean equals(Object other) {
        return other == this
            ? true
            : other instanceof EnvironmentAttributeMap environmentattributemap && this.entries.equals(environmentattributemap.entries);
    }
    ...
    public static class Builder {
        ...
        public <Value, Parameter> EnvironmentAttributeMap.Builder modify(
            EnvironmentAttribute<Value> attribute, AttributeModifier<Value, Parameter> modifier, Parameter paramater
        ) {
            attribute.type().checkAllowedModifier(modifier);
            this.entries.put(attribute, new EnvironmentAttributeMap.Entry<>(paramater, modifier));
            return this;
        }
    ...
    public record Entry<Value, Argument>(Argument argument, AttributeModifier<Value, Argument> modifier) {
        private static <Value> Codec<EnvironmentAttributeMap.Entry<Value, ?>> createCodec(EnvironmentAttribute<Value> attribute) {
            Codec<EnvironmentAttributeMap.Entry<Value, ?>> codec = attribute.type()
                ...
                .dispatch(
                    ...
                    Util.memoize(p_457747_ -> createFullCodec(attribute, (AttributeModifier<Value, ?>)p_457747_))
                );
            return Codec.either(attribute.valueCodec(), codec)
                .xmap(
                    p_457896_ -> p_457896_.map(
                        p_457523_ -> new EnvironmentAttributeMap.Entry<>(p_457523_, AttributeModifier.override()), p_457679_ -> p_457679_
                    ),
                    p_457999_ -> p_457999_.modifier == AttributeModifier.override()
                        ? Either.left((Value)p_457999_.argument())
                        : Either.right((EnvironmentAttributeMap.Entry<Value, ?>)p_457999_)
                );
        }
        ...
        private static <Value, Argument> MapCodec<EnvironmentAttributeMap.Entry<Value, Argument>> createFullCodec(
            ...
            return RecordCodecBuilder.mapCodec(
                p_458215_ -> p_458215_.group(modifier.argumentCodec(attribute).fieldOf("argument").forGetter(EnvironmentAttributeMap.Entry::argument))
                    .apply(p_458215_, p_458234_ -> new EnvironmentAttributeMap.Entry<>(p_458234_, modifier))
            );
        }
