...
public class ExtraDataFixUtils {
    ...
    public static <T> Typed<T> cast(Type<T> type, Object data, DynamicOps<?> ios) {
        return new Typed<>(type, ios, (T)data);
    }
    ...
    private static <A, B> TypeRewriteRule typePatcher(Type<A> oldType, Type<B> newType) {
        RewriteResult<A, B> rewriteresult = RewriteResult.create(View.create("Patcher", oldType, newType, p_359667_ -> p_359670_ -> {
    ...
    public static Dynamic<?> fixStringField(Dynamic<?> data, String fieldName, UnaryOperator<String> fixer) {
        return data.update(
            fieldName, p_359675_ -> DataFixUtils.orElse(p_359675_.asString().map(fixer).map(data::createString).result(), p_359675_)
        );
    }
