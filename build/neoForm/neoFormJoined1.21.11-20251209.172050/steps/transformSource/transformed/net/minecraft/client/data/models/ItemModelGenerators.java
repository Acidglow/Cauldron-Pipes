...
public class ItemModelGenerators {
    ...
    public static Identifier prefixForSlotTrim(String name) {
        return Identifier.withDefaultNamespace("trims/items/" + name + "_trim");
    }
    ...
    public Identifier createFlatItemModel(Item item, Item layerZeroItem, ModelTemplate modelTemplate) {
        return modelTemplate.create(ModelLocationUtils.getModelLocation(item), TextureMapping.layer0(layerZeroItem), this.modelOutput);
    }

    public void generateFlatItem(Item item, Item layerZeroItem, ModelTemplate modelTemplate) {
        this.itemModelOutput.accept(item, ItemModelUtils.plainModel(this.createFlatItemModel(item, layerZeroItem, modelTemplate)));
    }

    public void generateItemWithTintedOverlay(Item item, ItemTintSource tintSource) {
        this.generateItemWithTintedOverlay(item, "_overlay", tintSource);
    }

    public void generateItemWithTintedOverlay(Item item, String suffix, ItemTintSource tintSource) {
        Identifier identifier = this.generateLayeredItem(
            item, TextureMapping.getItemTexture(item), TextureMapping.getItemTexture(item, suffix)
        );
        this.itemModelOutput.accept(item, ItemModelUtils.tintedModel(identifier, BLANK_LAYER, tintSource));
    }

    public void generateItemWithTintedBaseLayer(Item item, int color) {
        Identifier identifier = TextureMapping.getItemTexture(item);
        Identifier identifier1 = TextureMapping.getItemTexture(item, "_overlay");
        ...
        this.itemModelOutput.accept(item, ItemModelUtils.tintedModel(identifier2, new Dye(color)));
    }
    ...
    public void generateRecoveryCompassItem(Item item) {
        this.itemModelOutput
            .accept(
                item,
                ItemModelUtils.rangeSelect(new CompassAngle(true, CompassAngleState.CompassTarget.RECOVERY), 32.0F, this.createCompassModels(item))
            );
    }

    public void generateClockItem(Item item) {
        List<RangeSelectItemModel.Entry> list = new ArrayList<>();
        ItemModel.Unbaked itemmodel$unbaked = ItemModelUtils.plainModel(this.createFlatItemModel(item, "_00", ModelTemplates.FLAT_ITEM));
        list.add(ItemModelUtils.override(itemmodel$unbaked, 0.0F));
    ...
    public Identifier generateLayeredItem(Item item, Identifier layer0, Identifier layer1) {
        return ModelTemplates.TWO_LAYERED_ITEM.create(item, TextureMapping.layered(layer0, layer1), this.modelOutput);
    }

    public Identifier generateLayeredItem(Identifier modelLocation, Identifier layer0, Identifier layer1) {
        return ModelTemplates.TWO_LAYERED_ITEM.create(modelLocation, TextureMapping.layered(layer0, layer1), this.modelOutput);
    }

    public void generateLayeredItem(Identifier modelLocation, Identifier layer0, Identifier layer1, Identifier layer2) {
        ModelTemplates.THREE_LAYERED_ITEM.create(modelLocation, TextureMapping.layered(layer0, layer1, layer2), this.modelOutput);
    }
    ...
    public void generateTrimmableItem(Item item, ResourceKey<EquipmentAsset> equipmentAsset, Identifier modelId, boolean usesSecondLayer) {
        ...
        for (ItemModelGenerators.TrimMaterialData itemmodelgenerators$trimmaterialdata : TRIM_MATERIAL_MODELS) {
            ...
            Identifier identifier4 = modelId.withSuffix("_" + itemmodelgenerators$trimmaterialdata.assets().assetId(equipmentAsset).suffix());
    ...
    public void generateBundleModels(Item bundleItem) {
        ItemModel.Unbaked itemmodel$unbaked = ItemModelUtils.plainModel(this.createFlatItemModel(bundleItem, ModelTemplates.FLAT_ITEM));
        Identifier identifier = this.generateBundleCoverModel(bundleItem, ModelTemplates.BUNDLE_OPEN_BACK_INVENTORY, "_open_back");
        Identifier identifier1 = this.generateBundleCoverModel(bundleItem, ModelTemplates.BUNDLE_OPEN_FRONT_INVENTORY, "_open_front");
        ...
        this.itemModelOutput
            .accept(bundleItem, ItemModelUtils.select(new DisplayContext(), itemmodel$unbaked, ItemModelUtils.when(ItemDisplayContext.GUI, itemmodel$unbaked2)));
    }

    public Identifier generateBundleCoverModel(Item bundleItem, ModelTemplate modelTemplate, String suffix) {
        Identifier identifier = TextureMapping.getItemTexture(bundleItem, suffix);
        return modelTemplate.create(bundleItem, TextureMapping.layer0(identifier), this.modelOutput);
    }
    ...
    public void generateBooleanDispatch(Item item, ConditionalItemModelProperty property, ItemModel.Unbaked trueModel, ItemModel.Unbaked falseModel) {
        this.itemModelOutput.accept(item, ItemModelUtils.conditional(property, trueModel, falseModel));
    }

    public void generateElytra(Item elytraItem) {
        ItemModel.Unbaked itemmodel$unbaked = ItemModelUtils.plainModel(this.createFlatItemModel(elytraItem, ModelTemplates.FLAT_ITEM));
        ItemModel.Unbaked itemmodel$unbaked1 = ItemModelUtils.plainModel(this.createFlatItemModel(elytraItem, "_broken", ModelTemplates.FLAT_ITEM));
        this.generateBooleanDispatch(elytraItem, new Broken(), itemmodel$unbaked1, itemmodel$unbaked);
    }

    public void generateBrush(Item brushItem) {
        ItemModel.Unbaked itemmodel$unbaked = ItemModelUtils.plainModel(ModelLocationUtils.getModelLocation(brushItem));
        ItemModel.Unbaked itemmodel$unbaked1 = ItemModelUtils.plainModel(ModelLocationUtils.getModelLocation(brushItem, "_brushing_0"));
        ItemModel.Unbaked itemmodel$unbaked2 = ItemModelUtils.plainModel(ModelLocationUtils.getModelLocation(brushItem, "_brushing_1"));
        ItemModel.Unbaked itemmodel$unbaked3 = ItemModelUtils.plainModel(ModelLocationUtils.getModelLocation(brushItem, "_brushing_2"));
        this.itemModelOutput
            .accept(
                brushItem,
                ItemModelUtils.rangeSelect(
    ...
    public void generateFishingRod(Item fishingRodItem) {
        ItemModel.Unbaked itemmodel$unbaked = ItemModelUtils.plainModel(this.createFlatItemModel(fishingRodItem, ModelTemplates.FLAT_HANDHELD_ROD_ITEM));
        ItemModel.Unbaked itemmodel$unbaked1 = ItemModelUtils.plainModel(this.createFlatItemModel(fishingRodItem, "_cast", ModelTemplates.FLAT_HANDHELD_ROD_ITEM));
        this.generateBooleanDispatch(fishingRodItem, new FishingRodCast(), itemmodel$unbaked1, itemmodel$unbaked);
    }

    public void generateGoatHorn(Item goatHornItem) {
        ItemModel.Unbaked itemmodel$unbaked = ItemModelUtils.plainModel(ModelLocationUtils.getModelLocation(goatHornItem));
        ...
        this.generateBooleanDispatch(goatHornItem, ItemModelUtils.isUsingItem(), itemmodel$unbaked1, itemmodel$unbaked);
    }

    public void generateShield(Item shieldItem) {
        ItemModel.Unbaked itemmodel$unbaked = ItemModelUtils.specialModel(ModelLocationUtils.getModelLocation(shieldItem), new ShieldSpecialRenderer.Unbaked());
        ItemModel.Unbaked itemmodel$unbaked1 = ItemModelUtils.specialModel(
            ModelLocationUtils.getModelLocation(shieldItem, "_blocking"), new ShieldSpecialRenderer.Unbaked()
        );
        this.generateBooleanDispatch(shieldItem, ItemModelUtils.isUsingItem(), itemmodel$unbaked1, itemmodel$unbaked);
    }
    ...
    public static ItemModel.Unbaked createFlatModelDispatch(ItemModel.Unbaked itemModel, ItemModel.Unbaked holdingModel) {
        return ItemModelUtils.select(
            new DisplayContext(),
            holdingModel,
            ItemModelUtils.when(List.of(ItemDisplayContext.GUI, ItemDisplayContext.GROUND, ItemDisplayContext.FIXED, ItemDisplayContext.ON_SHELF), itemModel)
        );
    }

    public void generateSpyglass(Item spyglassItem) {
        ItemModel.Unbaked itemmodel$unbaked = ItemModelUtils.plainModel(this.createFlatItemModel(spyglassItem, ModelTemplates.FLAT_ITEM));
        ItemModel.Unbaked itemmodel$unbaked1 = ItemModelUtils.plainModel(ModelLocationUtils.getModelLocation(spyglassItem, "_in_hand"));
        this.itemModelOutput.accept(spyglassItem, createFlatModelDispatch(itemmodel$unbaked, itemmodel$unbaked1));
    }

    public void generateTrident(Item tridentItem) {
        ItemModel.Unbaked itemmodel$unbaked = ItemModelUtils.plainModel(this.createFlatItemModel(tridentItem, ModelTemplates.FLAT_ITEM));
        ItemModel.Unbaked itemmodel$unbaked1 = ItemModelUtils.specialModel(
        ...
        this.itemModelOutput.accept(tridentItem, createFlatModelDispatch(itemmodel$unbaked, itemmodel$unbaked3));
    }

    public void generateSpear(Item spearItem) {
        ItemModel.Unbaked itemmodel$unbaked = ItemModelUtils.plainModel(this.createFlatItemModel(spearItem, ModelTemplates.FLAT_ITEM));
        ItemModel.Unbaked itemmodel$unbaked1 = ItemModelUtils.plainModel(
            ModelTemplates.SPEAR_IN_HAND.create(spearItem, TextureMapping.layer0(TextureMapping.getItemTexture(spearItem, "_in_hand")), this.modelOutput)
        );
        this.itemModelOutput.accept(spearItem, createFlatModelDispatch(itemmodel$unbaked, itemmodel$unbaked1), new ClientItem.Properties(true, false, 1.95F));
    }

    public void addPotionTint(Item potionItem, Identifier model) {
        this.itemModelOutput.accept(potionItem, ItemModelUtils.tintedModel(model, new Potion()));
    }

    public void generatePotion(Item potionItem) {
        Identifier identifier = this.generateLayeredItem(
            potionItem, ModelLocationUtils.decorateItemModelLocation("potion_overlay"), ModelLocationUtils.getModelLocation(potionItem)
        );
        this.addPotionTint(potionItem, identifier);
    }
    ...
    public void generateTippedArrow(Item arrowItem) {
        ...
        this.addPotionTint(arrowItem, identifier);
    }
    ...
    public void generateDyedItem(Item item, int color) {
        ...
        this.itemModelOutput.accept(item, ItemModelUtils.tintedModel(identifier, new Dye(color)));
    }

    public void generateTwoLayerDyedItem(Item item) {
        Identifier identifier = TextureMapping.getItemTexture(item);
        Identifier identifier1 = TextureMapping.getItemTexture(item, "_overlay");
        Identifier identifier2 = ModelTemplates.FLAT_ITEM.create(item, TextureMapping.layer0(identifier), this.modelOutput);
        Identifier identifier3 = ModelLocationUtils.getModelLocation(item, "_dyed");
