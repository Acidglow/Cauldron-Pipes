...
public class SilverfishModel extends EntityModel<EntityRenderState> {
    ...
    public SilverfishModel(ModelPart p_478212_) {
        super(p_478212_);
        Arrays.setAll(this.bodyParts, p_480427_ -> p_478212_.getChild(getSegmentName(p_480427_)));
        Arrays.setAll(this.bodyLayers, p_480365_ -> p_478212_.getChild(getLayerName(p_480365_)));
    }
    ...
    public void setupAnim(EntityRenderState p_481340_) {
        super.setupAnim(p_481340_);

        for (int i = 0; i < this.bodyParts.length; i++) {
            this.bodyParts[i].yRot = Mth.cos(p_481340_.ageInTicks * 0.9F + i * 0.15F * (float) Math.PI) * (float) Math.PI * 0.05F * (1 + Math.abs(i - 2));
            this.bodyParts[i].x = Mth.sin(p_481340_.ageInTicks * 0.9F + i * 0.15F * (float) Math.PI) * (float) Math.PI * 0.2F * Math.abs(i - 2);
        }
