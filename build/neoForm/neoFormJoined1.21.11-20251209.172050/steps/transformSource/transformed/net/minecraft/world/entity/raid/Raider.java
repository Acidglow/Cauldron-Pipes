...
public abstract class Raider extends PatrollingMonster {
    ...
    static final Predicate<ItemEntity> ALLOWED_ITEMS = p_427143_ -> !p_427143_.hasPickUpDelay()
        && p_427143_.isAlive()
        && ItemStack.matches(p_427143_.getItem(), Raid.getOminousBannerInstance(p_427143_.registryAccess().lookupOrThrow(Registries.BANNER_PATTERN)));
    ...
    protected void readAdditionalSaveData(ValueInput p_422705_) {
        ...
        if (this.level() instanceof ServerLevel serverlevel) {
            p_422705_.getInt("RaidId").ifPresent(p_432559_ -> {
                this.raid = serverlevel.getRaids().get(p_432559_);
                if (this.raid != null) {
    ...
    public @Nullable SpawnGroupData finalizeSpawn(
        ServerLevelAccessor p_37856_, DifficultyInstance p_37857_, EntitySpawnReason p_362502_, @Nullable SpawnGroupData p_37859_
    ) {
        ...
        public boolean canUse() {
            if (this.cannotPickUpBanner()) {
                ...
                for (ItemEntity itementity : this.mob
                    .level()
                    .getEntitiesOfClass(ItemEntity.class, this.mob.getBoundingBox().inflate(d0, 8.0, d0), Raider.ALLOWED_ITEMS)) {
