...
public class EquipmentLayerRenderer {
    ...
    public EquipmentLayerRenderer(EquipmentAssetManager equipmentAssets, TextureAtlas atlas) {
        ...
        this.layerTextureLookup = Util.memoize(p_465655_ -> p_465655_.layer.getTextureLocation(p_465655_.layerType));
        this.trimSpriteLookup = Util.memoize(p_465654_ -> atlas.getSprite(p_465654_.spriteId()));
    }
    ...
    public <S> void renderLayers(
        ...
        if (!list.isEmpty()) {
            ...
            for (EquipmentClientInfo.Layer equipmentclientinfo$layer : list) {
                ...
                if (k != 0) {
                    Identifier identifier = equipmentclientinfo$layer.usePlayerTexture() && texture != null
                        ? texture
                        : this.layerTextureLookup.apply(new EquipmentLayerRenderer.LayerTextureKey(layerType, equipmentclientinfo$layer));
                    identifier = net.neoforged.neoforge.client.ClientHooks.getArmorTexture(item, layerType, equipmentclientinfo$layer, identifier);
                    ...
                    if (flag) {
                        nodeCollector.order(j++)
                            .submitModel(
                                armorModel, renderState, poseStack, RenderTypes.armorEntityGlint(), packedLight, OverlayTexture.NO_OVERLAY, k, null, outlineColor, null
                            );
                    }
