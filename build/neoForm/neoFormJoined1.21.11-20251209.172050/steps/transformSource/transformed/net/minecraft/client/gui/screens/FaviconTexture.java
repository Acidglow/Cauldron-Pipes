    ...
    public static FaviconTexture forWorld(TextureManager textureManager, String worldName) {
        return new FaviconTexture(
            ...
            Identifier.withDefaultNamespace(
                "worlds/" + Util.sanitizeName(worldName, Identifier::validPathChar) + "/" + Hashing.sha1().hashUnencodedChars(worldName) + "/icon"
            )
        );
    }

    public static FaviconTexture forServer(TextureManager textureManager, String worldName) {
        return new FaviconTexture(textureManager, Identifier.withDefaultNamespace("servers/" + Hashing.sha1().hashUnencodedChars(worldName) + "/icon"));
    }
