...
public class GameProtocols {
    public static final CodecModifier<RegistryFriendlyByteBuf, ServerboundSetCreativeModeSlotPacket, GameProtocols.Context> HAS_INFINITE_MATERIALS = (p_412030_, p_412031_) -> new StreamCodec<RegistryFriendlyByteBuf, ServerboundSetCreativeModeSlotPacket>(
        
    ) {
        public ServerboundSetCreativeModeSlotPacket decode(RegistryFriendlyByteBuf p_412408_) {
            if (!p_412031_.hasInfiniteMaterials()) {
                throw new SkipPacketDecoderException("Not in creative mode");
            } else {
                return (ServerboundSetCreativeModeSlotPacket)p_412030_.decode(p_412408_);
            }
        }

        public void encode(RegistryFriendlyByteBuf p_412197_, ServerboundSetCreativeModeSlotPacket p_412074_) {
            if (!p_412031_.hasInfiniteMaterials()) {
                throw new SkipPacketEncoderException("Not in creative mode");
            } else {
                p_412030_.encode(p_412197_, p_412074_);
            }
        }
    };
    public static final UnboundProtocol<ServerGamePacketListener, RegistryFriendlyByteBuf, GameProtocols.Context> SERVERBOUND_TEMPLATE = ProtocolInfoBuilder.contextServerboundProtocol(
        ...
        p_448814_ -> p_448814_.addPacket(GamePacketTypes.SERVERBOUND_ACCEPT_TELEPORTATION, ServerboundAcceptTeleportationPacket.STREAM_CODEC)
    ...
    public static final SimpleUnboundProtocol<ClientGamePacketListener, RegistryFriendlyByteBuf> CLIENTBOUND_TEMPLATE = ProtocolInfoBuilder.clientboundProtocol(
        ConnectionProtocol.PLAY,
        p_470524_ -> p_470524_.withBundlePacket(GamePacketTypes.CLIENTBOUND_BUNDLE, ClientboundBundlePacket::new, new ClientboundBundleDelimiterPacket())
