...
public class StructureGridSpawner implements GameTestRunner.StructureSpawner {
    ...
    public void onBatchStart(ServerLevel p_352155_) {
        if (this.clearOnBatch) {
            this.testInLastBatch.forEach(p_396410_ -> {
                BoundingBox boundingbox = p_396410_.getTestInstanceBlockEntity().getStructureBoundingBox();
    ...
    public Optional<GameTestInfo> spawnStructure(GameTestInfo p_320875_) {
        ...
        p_320875_.setTestBlockPos(blockpos);
        GameTestInfo gametestinfo = p_320875_.prepareTestStructure();
        if (gametestinfo == null) {
            return Optional.empty();
        } else {
            gametestinfo.startExecution(1);
            AABB aabb = p_320875_.getTestInstanceBlockEntity().getStructureBounds();
