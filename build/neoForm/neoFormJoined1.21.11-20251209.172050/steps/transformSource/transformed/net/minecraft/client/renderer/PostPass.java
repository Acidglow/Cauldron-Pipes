...
public class PostPass implements AutoCloseable {
    ...
    public PostPass(RenderPipeline pipeline, Identifier outputTargetId, Map<String, List<UniformValue>> uniforms, List<PostPass.Input> inputs) {
    ...
    public void addToFrame(FrameGraphBuilder frameGraphBuilder, Map<Identifier, ResourceHandle<RenderTarget>> targets, GpuBufferSlice projectionMatrixBuffer) {
        ...
        ResourceHandle<RenderTarget> resourcehandle = targets.computeIfPresent(
            this.outputTargetId, (p_468653_, p_363989_) -> framepass.readsAndWrites((ResourceHandle<RenderTarget>)p_363989_)
        );
            ...
            framepass.executes(
                () -> {
                    ...
                    List<PostPass.InputTexture> list = this.inputs
                        .stream()
                        .map(
                            p_454209_ -> new PostPass.InputTexture(
                                p_454209_.samplerName(),
                                p_454209_.texture(targets),
                                samplercache.getClampToEdge(p_454209_.bilinear() ? FilterMode.LINEAR : FilterMode.NEAREST)
                            )
                        )
                        .toList();
    ...
    public interface Input {
        ...
        GpuTextureView texture(Map<Identifier, ResourceHandle<RenderTarget>> targets);
    ...
    public record TargetInput(String samplerName, Identifier targetId, boolean depthBuffer, boolean bilinear) implements PostPass.Input {
        ...
        @Override
        public void addToPass(FramePass p_364579_, Map<Identifier, ResourceHandle<RenderTarget>> p_360589_) {
            p_364579_.reads(this.getHandle(p_360589_));
        }

        @Override
        public GpuTextureView texture(Map<Identifier, ResourceHandle<RenderTarget>> p_361239_) {
    ...
    public record TextureInput(String samplerName, AbstractTexture texture, int width, int height, boolean bilinear) implements PostPass.Input {
        @Override
        public void addToPass(FramePass p_361843_, Map<Identifier, ResourceHandle<RenderTarget>> p_362022_) {
        }

        @Override
        public GpuTextureView texture(Map<Identifier, ResourceHandle<RenderTarget>> p_418145_) {
