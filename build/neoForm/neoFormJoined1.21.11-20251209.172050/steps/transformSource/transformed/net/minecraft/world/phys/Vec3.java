...
public class Vec3 implements Position {
    public static final Codec<Vec3> CODEC = Codec.DOUBLE
        .listOf()
        .comapFlatMap(
            p_466813_ -> Util.fixedSize((List<Double>)p_466813_, 3).map(p_231081_ -> new Vec3(p_231081_.get(0), p_231081_.get(1), p_231081_.get(2))),
    ...
    public Vec3 align(EnumSet<Direction.Axis> axes) {
        double d0 = axes.contains(Direction.Axis.X) ? Mth.floor(this.x) : this.x;
    ...
    public static Vec3 applyLocalCoordinatesToRotation(Vec2 rotation, Vec3 localCoordinates) {
        float f = Mth.cos((rotation.y + 90.0F) * (float) (Math.PI / 180.0));
        float f1 = Mth.sin((rotation.y + 90.0F) * (float) (Math.PI / 180.0));
        float f2 = Mth.cos(-rotation.x * (float) (Math.PI / 180.0));
        float f3 = Mth.sin(-rotation.x * (float) (Math.PI / 180.0));
        float f4 = Mth.cos((-rotation.x + 90.0F) * (float) (Math.PI / 180.0));
        float f5 = Mth.sin((-rotation.x + 90.0F) * (float) (Math.PI / 180.0));
        Vec3 vec3 = new Vec3(f * f2, f3, f1 * f2);
        Vec3 vec31 = new Vec3(f * f4, f5, f1 * f4);
        Vec3 vec32 = vec3.cross(vec31).scale(-1.0);
        double d0 = vec3.x * localCoordinates.z + vec31.x * localCoordinates.y + vec32.x * localCoordinates.x;
        double d1 = vec3.y * localCoordinates.z + vec31.y * localCoordinates.y + vec32.y * localCoordinates.x;
        double d2 = vec3.z * localCoordinates.z + vec31.z * localCoordinates.y + vec32.z * localCoordinates.x;
        return new Vec3(d0, d1, d2);
    }

    public Vec3 addLocalCoordinates(Vec3 localCoordinates) {
        return applyLocalCoordinatesToRotation(this.rotation(), localCoordinates);
    }
