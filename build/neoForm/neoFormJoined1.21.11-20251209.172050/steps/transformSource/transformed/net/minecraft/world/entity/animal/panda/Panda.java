...
public class Panda extends Animal {
    ...
    public Panda(EntityType<? extends Panda> p_481872_, Level p_481733_) {
        super(p_481872_, p_481733_);
    ...
    @Override
    protected boolean canDispenserEquipIntoSlot(EquipmentSlot p_479639_) {
        return p_479639_ == EquipmentSlot.MAINHAND && this.canPickUpLoot();
    }
    ...
    public void setMainGene(Panda.Gene mainGene) {
        ...
        this.entityData.set(MAIN_GENE_ID, (byte)mainGene.getId());
    }
    ...
    public void setHiddenGene(Panda.Gene hiddenGene) {
        ...
        this.entityData.set(HIDDEN_GENE_ID, (byte)hiddenGene.getId());
    }
    ...
    @Override
    protected void defineSynchedData(SynchedEntityData.Builder p_481005_) {
        super.defineSynchedData(p_481005_);
        p_481005_.define(UNHAPPY_COUNTER, 0);
        p_481005_.define(SNEEZE_COUNTER, 0);
        p_481005_.define(MAIN_GENE_ID, (byte)0);
        p_481005_.define(HIDDEN_GENE_ID, (byte)0);
        p_481005_.define(DATA_ID_FLAGS, (byte)0);
        p_481005_.define(EAT_COUNTER, 0);
    }
    ...
    @Override
    protected void addAdditionalSaveData(ValueOutput p_479557_) {
        super.addAdditionalSaveData(p_479557_);
        p_479557_.store("MainGene", Panda.Gene.CODEC, this.getMainGene());
        p_479557_.store("HiddenGene", Panda.Gene.CODEC, this.getHiddenGene());
    }

    @Override
    protected void readAdditionalSaveData(ValueInput p_479760_) {
        super.readAdditionalSaveData(p_479760_);
        this.setMainGene(p_479760_.read("MainGene", Panda.Gene.CODEC).orElse(Panda.Gene.NORMAL));
        this.setHiddenGene(p_479760_.read("HiddenGene", Panda.Gene.CODEC).orElse(Panda.Gene.NORMAL));
    }

    @Override
    public @Nullable AgeableMob getBreedOffspring(ServerLevel p_480560_, AgeableMob p_478965_) {
        Panda panda = EntityType.PANDA.create(p_480560_, EntitySpawnReason.BREEDING);
        if (panda != null) {
            if (p_478965_ instanceof Panda panda1) {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(4, new TemptGoal(this, 1.0, p_481749_ -> p_481749_.is(ItemTags.PANDA_FOOD), false));
    ...
    @Override
    public boolean doHurtTarget(ServerLevel p_479020_, Entity p_478005_) {
        if (!this.isAggressive()) {
            this.didBite = true;
        }

        return super.doHurtTarget(p_479020_, p_478005_);
    }
    ...
    @Override
    protected void pickUpItem(ServerLevel p_480080_, ItemEntity p_478063_) {
        if (this.getItemBySlot(EquipmentSlot.MAINHAND).isEmpty() && canPickUpAndEat(p_478063_)) {
            this.onItemPickup(p_478063_);
            ItemStack itemstack = p_478063_.getItem();
            this.setItemSlot(EquipmentSlot.MAINHAND, itemstack);
            this.setGuaranteedDrop(EquipmentSlot.MAINHAND);
            this.take(p_478063_, itemstack.getCount());
            p_478063_.discard();
        }
    }

    @Override
    public boolean hurtServer(ServerLevel p_480184_, DamageSource p_479799_, float p_478083_) {
        this.sit(false);
    ...
    public @Nullable SpawnGroupData finalizeSpawn(
        ServerLevelAccessor p_482023_, DifficultyInstance p_480729_, EntitySpawnReason p_477927_, @Nullable SpawnGroupData p_478134_
    ) {
        RandomSource randomsource = p_482023_.getRandom();
        this.setMainGene(Panda.Gene.getRandom(randomsource));
        this.setHiddenGene(Panda.Gene.getRandom(randomsource));
        this.setAttributes();
        if (p_478134_ == null) {
            p_478134_ = new AgeableMob.AgeableMobGroupData(0.2F);
        }

        return super.finalizeSpawn(p_482023_, p_480729_, p_477927_, p_478134_);
    }

    public void setGeneFromParents(Panda panda, @Nullable Panda mate) {
        if (mate == null) {
            ...
            this.setMainGene(panda.getOneOfGenesRandomly());
            this.setHiddenGene(mate.getOneOfGenesRandomly());
        } else {
            this.setMainGene(mate.getOneOfGenesRandomly());
            this.setHiddenGene(panda.getOneOfGenesRandomly());
        }
    ...
    @Override
    public EntityDimensions getDefaultDimensions(Pose p_478519_) {
        return this.isBaby() ? BABY_DIMENSIONS : super.getDefaultDimensions(p_478519_);
    }
    ...
    static class PandaAttackGoal extends MeleeAttackGoal {
        ...
        public PandaAttackGoal(Panda panda, double speedModifier, boolean followTargetEvenIfNotSeen) {
            super(panda, speedModifier, followTargetEvenIfNotSeen);
            this.panda = panda;
        }
    ...
    static class PandaAvoidGoal<T extends LivingEntity> extends AvoidEntityGoal<T> {
        ...
        public PandaAvoidGoal(Panda panda, Class<T> avoidClass, float maxDistance, double walkSpeedModifier, double sprintSpeedModifier) {
            super(panda, avoidClass, maxDistance, walkSpeedModifier, sprintSpeedModifier, EntitySelector.NO_SPECTATORS);
            this.panda = panda;
        }
                    ...
                    if (this.lookAtType == Player.class) {
                        ...
                        this.lookAt = serverlevel.getNearestEntity(
                            this.mob
                                .level()
                                .getEntitiesOfClass(
                                    this.lookAtType, this.mob.getBoundingBox().inflate(this.lookDistance, 3.0, this.lookDistance), p_479988_ -> true
                                ),
                            this.lookAtContext,
                            this.mob,
                            this.mob.getX(),
