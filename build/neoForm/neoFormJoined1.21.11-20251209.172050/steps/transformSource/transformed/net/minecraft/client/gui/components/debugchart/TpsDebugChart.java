...
public class TpsDebugChart extends AbstractDebugChart {
    ...
    protected void renderAdditionalLinesAndLabels(GuiGraphics p_298653_, int p_298791_, int p_298387_, int p_298869_) {
        ...
        this.drawStringWithShade(p_298653_, String.format(Locale.ROOT, "%.1f TPS", f), p_298791_ + 1, p_298869_ - 60 + 1);
    }
    ...
    protected void drawAdditionalDimensions(GuiGraphics p_321511_, int p_321489_, int p_321791_, int p_321685_) {
        ...
        p_321511_.fill(p_321791_, p_321489_ - j, p_321791_ + 1, p_321489_, -6745839);
        long k = this.sampleStorage.get(p_321685_, TpsDebugDimensions.SCHEDULED_TASKS.ordinal());
        int l = this.getSampleHeight(k);
        p_321511_.fill(p_321791_, p_321489_ - j - l, p_321791_ + 1, p_321489_ - j, -4548257);
        ...
        p_321511_.fill(p_321791_, p_321489_ - j1 - l - j, p_321791_ + 1, p_321489_ - l - j, -10547572);
    }
    ...
    protected int getSampleHeight(double p_299161_) {
        return (int)Math.round(toMilliseconds(p_299161_) * 60.0 / this.msptSupplier.get().floatValue());
    }

    @Override
    protected int getSampleColor(long p_299243_) {
        float f = this.msptSupplier.get();
        return this.getSampleColor(toMilliseconds(p_299243_), f, -16711936, f * 1.125, -256, f * 1.25, -65536);
    }
