...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (!optionset.has(optionspec) && optionset.hasOptions()) {
            ...
            net.neoforged.neoforge.data.loading.DatagenModLoader.begin(mods, path, collection, existingPacks, flag2, flag3, validate, isFlat, () -> {},
                    net.neoforged.neoforge.data.event.GatherDataEvent.Server::new, datagenerator, packConsumer -> {});
        } else {
    ...
    public static void addServerProviders(DataGenerator dataGenerator, Collection<Path> paths, boolean server, boolean dev, boolean reports) {
        DataGenerator.PackGenerator datagenerator$packgenerator = dataGenerator.getVanillaPack(server);
        datagenerator$packgenerator.addProvider(p_253388_ -> new SnbtToNbt(p_253388_, paths).addFilter(new StructureUpdater()));
        ...
        DataGenerator.PackGenerator datagenerator$packgenerator1 = dataGenerator.getVanillaPack(server);
        ...
        datagenerator$packgenerator1 = dataGenerator.getVanillaPack(dev);
        datagenerator$packgenerator1.addProvider(p_253386_ -> new NbtToSnbt(p_253386_, paths));
        datagenerator$packgenerator1 = dataGenerator.getVanillaPack(reports);
        ...
        DataGenerator.PackGenerator datagenerator$packgenerator2 = dataGenerator.getBuiltinDatapack(server, "trade_rebalance");
        ...
        datagenerator$packgenerator1 = dataGenerator.getBuiltinDatapack(server, "redstone_experiments");
        datagenerator$packgenerator1.addProvider(
            p_359371_ -> PackMetadataGenerator.forFeaturePack(
                p_359371_, Component.translatable("dataPack.redstone_experiments.description"), FeatureFlagSet.of(FeatureFlags.REDSTONE_EXPERIMENTS)
            )
        );
        datagenerator$packgenerator1 = dataGenerator.getBuiltinDatapack(server, "minecart_improvements");
        datagenerator$packgenerator1.addProvider(
            p_359372_ -> PackMetadataGenerator.forFeaturePack(
                p_359372_, Component.translatable("dataPack.minecart_improvements.description"), FeatureFlagSet.of(FeatureFlags.MINECART_IMPROVEMENTS)
            )
        );
    }
}
