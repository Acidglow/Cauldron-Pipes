...
public class PoolAliasBindings {
    ...
    public static void registerTargetsAsPools(
        ...
        poolAliasBindings.stream()
            ...
            .map(p_466745_ -> p_466745_.identifier().getPath())
            .forEach(
                p_321475_ -> Pools.register(
                    context,
                    p_321475_,
                    new StructureTemplatePool(pool, List.of(Pair.of(StructurePoolElement.single(p_321475_), 1)), StructureTemplatePool.Projection.RIGID)
                )
            );
    }
}
