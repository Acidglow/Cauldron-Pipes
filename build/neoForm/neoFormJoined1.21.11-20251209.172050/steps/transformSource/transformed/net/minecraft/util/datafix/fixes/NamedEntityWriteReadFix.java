...
public abstract class NamedEntityWriteReadFix extends DataFix {
    ...
    private <S, T, A> TypeRewriteRule fix(Type<S> inputType, Type<T> outputType, Type<?> type, OpticFinder<A> optic) {
        return this.fixTypeEverywhereTyped(this.name, inputType, outputType, p_466467_ -> {
            if (p_466467_.getOptional(optic).isEmpty()) {
                return ExtraDataFixUtils.cast(outputType, p_466467_);
            } else {
                Typed<?> typed = ExtraDataFixUtils.cast(type, p_466467_);
