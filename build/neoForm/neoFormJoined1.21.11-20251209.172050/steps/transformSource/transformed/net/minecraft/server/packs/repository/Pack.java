...
public class Pack {
    ...
    public static @Nullable Pack readMetaAndCreate(
        ...
        Pack.Metadata pack$metadata = readPackMetadata(location, resources, packformat, packType);
    ...
    public static Pack.@Nullable Metadata readPackMetadata(
        PackLocationInfo location, Pack.ResourcesSupplier resources, PackFormat format, PackType packType
    ) {
        try {
            ...
            try (PackResources packresources = resources.openPrimary(location)) {
                ...
                PackCompatibility packcompatibility = PackCompatibility.forVersion(packmetadatasection.supportedFormats(), format);
                ...
                List<String> list = overlaymetadatasection != null ? overlaymetadatasection.overlaysForVersion(format) : List.of();
                // Neo: add `neoforge:overlays` overlays as well
                var neoOverlays = packresources.getMetadataSection(OverlayMetadataSection.forPackTypeNeoForge(packType));
                if (neoOverlays != null) {
                    list = new java.util.ArrayList<>(list);
                    list.addAll(neoOverlays.overlaysForVersion(format));
