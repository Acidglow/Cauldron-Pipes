...
public class BossBarCommands {
                                ...
                                .then(
                                    Commands.argument("name", ComponentArgument.textComponent(context))
                                        .executes(
                                            p_466201_ -> createBar(
                                                p_466201_.getSource(),
                                                IdentifierArgument.getId(p_466201_, "id"),
                                                ComponentArgument.getResolvedComponent(p_466201_, "name")
                                            )
                                        )
                                )
                        )
                                    ...
                                    Commands.literal("name")
                                        .then(
                                            Commands.argument("name", ComponentArgument.textComponent(context))
                                                .executes(
                                                    p_400886_ -> setName(
                                                        p_400886_.getSource(), getBossBar(p_400886_), ComponentArgument.getResolvedComponent(p_400886_, "name")
                                                    )
                                                )
                                        )
                                )
    ...
    public static CustomBossEvent getBossBar(CommandContext<CommandSourceStack> source) throws CommandSyntaxException {
        Identifier identifier = IdentifierArgument.getId(source, "id");
        CustomBossEvent custombossevent = source.getSource().getServer().getCustomBossEvents().get(identifier);
