...
public class MacroFunction<T extends ExecutionCommandSource<T>> implements CommandFunction<T> {
    private static final DecimalFormat DECIMAL_FORMAT = Util.make(
        new DecimalFormat("#", DecimalFormatSymbols.getInstance(Locale.ROOT)), p_458819_ -> p_458819_.setMaximumFractionDigits(15)
    );
    ...
    static class MacroEntry<T extends ExecutionCommandSource<T>> implements MacroFunction.Entry<T> {
        ...
        @Override
        public UnboundEntryAction<T> instantiate(List<String> p_306061_, CommandDispatcher<T> p_306059_, Identifier p_469200_) throws FunctionInstantiationException {
            ...
            try {
                ...
                throw new FunctionInstantiationException(
                    Component.translatable("commands.function.error.parse", Component.translationArg(p_469200_), s, commandsyntaxexception.getMessage())
                );
            }
        }
    }
    ...
    static class PlainTextEntry<T> implements MacroFunction.Entry<T> {
        ...
        @Override
        public UnboundEntryAction<T> instantiate(List<String> p_306111_, CommandDispatcher<T> p_305941_, Identifier p_469774_) {
