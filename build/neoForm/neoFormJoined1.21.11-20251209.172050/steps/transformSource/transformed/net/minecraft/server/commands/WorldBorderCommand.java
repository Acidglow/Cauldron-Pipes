...
public class WorldBorderCommand {
                        ...
                        .then(
                            Commands.argument("distance", DoubleArgumentType.doubleArg(-5.999997E7F, 5.999997E7F))
                                .executes(
                                    p_432445_ -> setSize(
                                        p_432445_.getSource(),
                                        p_432445_.getSource().getLevel().getWorldBorder().getSize() + DoubleArgumentType.getDouble(p_432445_, "distance"),
                                        0L
                                    )
                                )
                                .then(
                                    Commands.argument("time", TimeArgument.time(0))
                                        .executes(
                                            p_458835_ -> setSize(
                                                p_458835_.getSource(),
                                                p_458835_.getSource().getLevel().getWorldBorder().getSize()
                                                    + DoubleArgumentType.getDouble(p_458835_, "distance"),
                                                p_458835_.getSource().getLevel().getWorldBorder().getLerpTime()
                                                    + IntegerArgumentType.getInteger(p_458835_, "time")
                                            )
                                        )
                                )
                        )
                                ...
                                .then(
                                    Commands.argument("time", TimeArgument.time(0))
                                        .executes(
                                            p_458836_ -> setSize(
                                                p_458836_.getSource(),
                                                DoubleArgumentType.getDouble(p_458836_, "distance"),
                                                IntegerArgumentType.getInteger(p_458836_, "time")
                                            )
                                        )
                                )
                        )
    ...
    private static int setSize(CommandSourceStack source, double newSize, long time) throws CommandSyntaxException {
        ...
        if (d0 == newSize) {
            ...
            String s = String.format(Locale.ROOT, "%.1f", newSize);
            if (time > 0L) {
                worldborder.lerpSizeBetween(d0, newSize, time, serverlevel.getGameTime());
                if (newSize > d0) {
                    source.sendSuccess(() -> Component.translatable("commands.worldborder.set.grow", s, formatTicksToSeconds(time)), true);
                } else {
                    source.sendSuccess(() -> Component.translatable("commands.worldborder.set.shrink", s, formatTicksToSeconds(time)), true);
                }
            } else {
                worldborder.setSize(newSize);
                source.sendSuccess(() -> Component.translatable("commands.worldborder.set.immediate", s), true);
            }
