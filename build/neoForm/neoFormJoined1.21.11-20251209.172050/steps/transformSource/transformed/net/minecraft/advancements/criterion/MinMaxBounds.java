...
public interface MinMaxBounds<T extends Number & Comparable<T>> {
    ...
    public record Bounds<T extends Number & Comparable<T>>(Optional<T> min, Optional<T> max) {
        ...
        static <T extends Number & Comparable<T>> Codec<MinMaxBounds.Bounds<T>> createCodec(Codec<T> valueCodec) {
            ...
            return Codec.either(codec, valueCodec).xmap(p_467854_ -> p_467854_.map(p_469720_ -> p_469720_, p_467164_ -> exactly((T)p_467164_)), p_467787_ -> {
        ...
        static <B extends ByteBuf, T extends Number & Comparable<T>> StreamCodec<B, MinMaxBounds.Bounds<T>> createStreamCodec(final StreamCodec<B, T> valueCodec) {
            return new StreamCodec<B, MinMaxBounds.Bounds<T>>() {
                ...
                public MinMaxBounds.Bounds<T> decode(B p_467201_) {
                    ...
                    Optional<T> optional = (b0 & 1) != 0 ? Optional.of(valueCodec.decode(p_467201_)) : Optional.empty();
                    Optional<T> optional1 = (b0 & 2) != 0 ? Optional.of(valueCodec.decode(p_467201_)) : Optional.empty();
                ...
                public void encode(B p_467330_, MinMaxBounds.Bounds<T> p_468206_) {
                    ...
                    optional.ifPresent(p_468905_ -> valueCodec.encode(p_467330_, (T)p_468905_));
                    optional1.ifPresent(p_467650_ -> valueCodec.encode(p_467330_, (T)p_467650_));
                }
            };
        }
