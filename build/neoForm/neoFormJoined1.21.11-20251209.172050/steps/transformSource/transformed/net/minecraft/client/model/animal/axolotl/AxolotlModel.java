...
public class AxolotlModel extends EntityModel<AxolotlRenderState> {
    ...
    public void setupAnim(AxolotlRenderState p_478616_) {
        super.setupAnim(p_478616_);
        float f = p_478616_.playingDeadFactor;
        float f1 = p_478616_.inWaterFactor;
        float f2 = p_478616_.onGroundFactor;
        float f3 = p_478616_.movingFactor;
        float f4 = 1.0F - f3;
        float f5 = 1.0F - Math.min(f2, f3);
        this.body.yRot = this.body.yRot + p_478616_.yRot * (float) (Math.PI / 180.0);
        this.setupSwimmingAnimation(p_478616_.ageInTicks, p_478616_.xRot, Math.min(f3, f1));
        this.setupWaterHoveringAnimation(p_478616_.ageInTicks, Math.min(f4, f1));
        this.setupGroundCrawlingAnimation(p_478616_.ageInTicks, Math.min(f3, f2));
        this.setupLayStillOnGroundAnimation(p_478616_.ageInTicks, Math.min(f4, f2));
