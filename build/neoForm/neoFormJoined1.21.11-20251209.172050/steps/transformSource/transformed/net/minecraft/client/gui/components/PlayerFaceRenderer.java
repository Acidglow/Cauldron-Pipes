...
public class PlayerFaceRenderer {
    ...
    public static void draw(
        GuiGraphics guiGraphics, Identifier skinTexture, int x, int y, int size, boolean p_drawHat, boolean upsideDown, int color
    ) {
    ...
    private static void drawHat(GuiGraphics guiGraphics, Identifier skinTexture, int x, int y, int size, boolean upsideDown, int color) {
        int i = 8 + (upsideDown ? 8 : 0);
        int j = 8 * (upsideDown ? -1 : 1);
