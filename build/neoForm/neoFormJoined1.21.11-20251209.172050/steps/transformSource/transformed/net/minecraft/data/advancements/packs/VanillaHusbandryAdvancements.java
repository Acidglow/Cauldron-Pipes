...
public class VanillaHusbandryAdvancements implements AdvancementSubProvider {
    ...
    private static final Comparator<Holder.Reference<?>> HOLDER_KEY_COMPARATOR = Comparator.comparing(p_465980_ -> p_465980_.key().identifier());
    ...
    private static Advancement.Builder addLeashedFrogVariants(
        ...
        sortedVariants(frogVariants)
            .forEach(
                p_465986_ -> builder.addCriterion(
                    p_465986_.key().identifier().toString(),
                    PlayerInteractTrigger.TriggerInstance.itemUsedOnEntity(
                        ...
                        Optional.of(
                            EntityPredicate.wrap(
                                EntityPredicate.Builder.entity()
                                    ...
                                    .components(
                                        DataComponentMatchers.Builder.components()
                                            .exact(DataComponentExactPredicate.expect(DataComponents.FROG_VARIANT, p_465986_))
                                            .build()
                                    )
                            )
                        )
    ...
    private static Advancement.Builder addBreedable(
        ...
        breedableAnimals.forEach(
            p_465989_ -> builder.addCriterion(
                EntityType.getKey((EntityType<?>)p_465989_).toString(),
                BredAnimalsTrigger.TriggerInstance.bredAnimals(EntityPredicate.Builder.entity().of(entityTypeRegistry, (EntityType<?>)p_465989_))
            )
        );
        indirectlyBreedableAnimals.forEach(
            p_465992_ -> builder.addCriterion(
                EntityType.getKey((EntityType<?>)p_465992_).toString(),
                BredAnimalsTrigger.TriggerInstance.bredAnimals(
                    Optional.of(EntityPredicate.Builder.entity().of(entityTypeRegistry, (EntityType<?>)p_465992_).build()),
                    Optional.of(EntityPredicate.Builder.entity().of(entityTypeRegistry, (EntityType<?>)p_465992_).build()),
                    Optional.empty()
                )
            )
    ...
    private static Advancement.Builder addCatVariants(Advancement.Builder builder, HolderLookup<CatVariant> variantRegistry) {
        sortedVariants(variantRegistry)
            .forEach(
                p_465982_ -> builder.addCriterion(
                    p_465982_.key().identifier().toString(),
                    TameAnimalTrigger.TriggerInstance.tamedAnimal(
                        EntityPredicate.Builder.entity()
                            .components(
                                DataComponentMatchers.Builder.components()
                                    .exact(DataComponentExactPredicate.expect(DataComponents.CAT_VARIANT, p_465982_))
                                    .build()
                            )
                    )
                )
    ...
    private static Advancement.Builder addTamedWolfVariants(Advancement.Builder builder, HolderLookup<WolfVariant> variantRegistry) {
        sortedVariants(variantRegistry)
            .forEach(
                p_465979_ -> builder.addCriterion(
                    p_465979_.key().identifier().toString(),
                    TameAnimalTrigger.TriggerInstance.tamedAnimal(
                        EntityPredicate.Builder.entity()
                            .components(
                                DataComponentMatchers.Builder.components()
                                    .exact(DataComponentExactPredicate.expect(DataComponents.WOLF_VARIANT, p_465979_))
                                    .build()
                            )
                    )
                )
