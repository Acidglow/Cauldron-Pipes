    ...
    static <C, I extends BoundedFloatFunction<C>> Codec<CubicSpline<C, I>> codec(Codec<I> valueCodec) {
        ...
        Codec<Point<C, I>> codec = RecordCodecBuilder.create(
            p_460352_ -> p_460352_.group(
                ...
                .apply(p_460352_, (p_184242_, p_184243_, p_184244_) -> new Point<>(p_184242_, p_184243_, p_184244_))
        );
        Codec<CubicSpline.Multipoint<C, I>> codec1 = RecordCodecBuilder.create(
            p_184267_ -> p_184267_.group(
                ...
                .apply(p_184267_, (p_445302_, p_445303_) -> {
                    float[] afloat = new float[p_445303_.size()];
                    ImmutableList.Builder<CubicSpline<C, I>> builder = ImmutableList.builder();
                    float[] afloat1 = new float[p_445303_.size()];

                    for (int i = 0; i < p_445303_.size(); i++) {
                        Point<C, I> point = p_445303_.get(i);
                        afloat[i] = point.location();
                        builder.add(point.value());
                        afloat1[i] = point.derivative();
                    }

                    return CubicSpline.Multipoint.create(p_445302_, afloat, builder.build(), afloat1);
                })
        );
        ...
        private String toString(float[] locations) {
            return "["
                + IntStream.range(0, locations.length)
                    .mapToDouble(p_184338_ -> locations[p_184338_])
