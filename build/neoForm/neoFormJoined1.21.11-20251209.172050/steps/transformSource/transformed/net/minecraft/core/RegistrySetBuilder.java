...
public class RegistrySetBuilder {
    ...
    private static HolderLookup.Provider buildProviderWithContext(
        ...
        registryAccess.registries().forEach(p_367805_ -> map.put(p_367805_.key(), Entry.createForContextRegistry(p_367805_.value())));
    ...
    private <T> HolderLookup.RegistryLookup<T> createLazyFullPatchedRegistries(
        ...
        if (cloner == null) {
            ...
            registrylookup.listElements().forEach(p_311483_ -> {
                ...
                lazyholder.supplier = () -> cloner.clone((T)p_311483_.value(), registryLookupProvider, object.get());
                map.put(resourcekey, lazyholder);
            });
            Optional<? extends HolderLookup.RegistryLookup<T>> lookup = lookupProvider.lookup(registryKey);
            Lifecycle lifecycle;
            if (lookup.isPresent()) {
    ...
    record BuildState(
        ...
        public static RegistrySetBuilder.BuildState create(RegistryAccess registryAccess, Stream<ResourceKey<? extends Registry<?>>> registries) {
            ...
            registryAccess.registries().forEach(p_367807_ -> builder.put(p_367807_.key().identifier(), net.neoforged.neoforge.common.CommonHooks.wrapRegistryLookup(p_367807_.value())));
            registries.forEach(p_465906_ -> builder.put(p_465906_.identifier(), registrysetbuilder$universallookup));
    ...
    static class UniversalLookup extends RegistrySetBuilder.EmptyTagLookup<Object> implements net.minecraft.core.HolderLookup<java.lang.Object> {
