    ...
    public static BehaviorControl<LivingEntity> create() {
        ...
        return BehaviorBuilder.create(
            p_258474_ -> p_258474_.group(
                ...
                .apply(
                    p_258474_,
                    (p_258460_, p_258461_, p_258462_) -> (p_460431_, p_460432_, p_460433_) -> {
                        ...
                        if (!path.notStarted() && !path.isDone()) {
                            ...
                            BlockState blockstate = p_460431_.getBlockState(blockpos);
                            if (blockstate.is(BlockTags.MOB_INTERACTABLE_DOORS, p_201959_ -> p_201959_.getBlock() instanceof DoorBlock)) {
                                DoorBlock doorblock = (DoorBlock)blockstate.getBlock();
                                if (!doorblock.isOpen(blockstate)) {
                                    doorblock.setOpen(p_460432_, p_460431_, blockstate, blockpos, true);
                                }

                                optional = rememberDoorToClose(p_258461_, optional, p_460431_, blockpos);
                            }

                            BlockPos blockpos1 = node1.asBlockPos();
                            BlockState blockstate1 = p_460431_.getBlockState(blockpos1);
                            if (blockstate1.is(BlockTags.MOB_INTERACTABLE_DOORS, p_201957_ -> p_201957_.getBlock() instanceof DoorBlock)) {
                                DoorBlock doorblock1 = (DoorBlock)blockstate1.getBlock();
                                if (!doorblock1.isOpen(blockstate1)) {
                                    doorblock1.setOpen(p_460432_, p_460431_, blockstate1, blockpos1, true);
                                    optional = rememberDoorToClose(p_258461_, optional, p_460431_, blockpos1);
                                }
                            }

                            optional.ifPresent(
                                p_258452_ -> closeDoorsThatIHaveOpenedOrPassedThrough(
                                    p_460431_, p_460432_, node, node1, (Set<GlobalPos>)p_258452_, p_258474_.tryGet(p_258462_)
                                )
                            );
                            return true;
                        } else {
    ...
    private static boolean areOtherMobsComingThroughDoor(LivingEntity entity, BlockPos pos, Optional<List<LivingEntity>> nearestLivingEntities) {
        return nearestLivingEntities.isEmpty()
            ...
            : nearestLivingEntities.get()
                .stream()
                .filter(p_423239_ -> p_423239_.getType() == entity.getType())
                .filter(p_426998_ -> pos.closerToCenterThan(p_426998_.position(), 2.0))
