...
public class SoundEngine {
    ...
    private final Object2FloatMap<SoundSource> gainBySource = Util.make(new Object2FloatOpenHashMap<>(), p_465803_ -> p_465803_.defaultReturnValue(1.0F));
    /**
    ...
    public void refreshCategoryVolume(SoundSource source) {
        if (this.loaded) {
            this.instanceToChannel.forEach((p_465801_, p_465802_) -> {
                if (source == p_465801_.getSource() || source == SoundSource.MASTER) {
                    float f = this.calculateVolume(p_465801_);
                    p_465802_.execute(p_425183_ -> p_425183_.setVolume(f));
                }
            });
        }
    }
    ...
    public void updateCategoryVolume(SoundSource source, float value) {
        this.gainBySource.put(source, Mth.clamp(value, 0.0F, 1.0F));
    ...
    public SoundEngine.PlayResult play(SoundInstance p_sound) {
        if (!this.loaded) {
        ...
        if (p_sound == null || !p_sound.canPlaySound()) {
            ...
            Identifier identifier = p_sound.getIdentifier();
            ...
            if (sound == SoundManager.INTENTIONALLY_EMPTY_SOUND) {
                ...
                boolean flag2 = false;
                if (f2 == 0.0F) {
                    if (!p_sound.canStartSilent() && soundsource != SoundSource.MUSIC) {
                ...
                boolean flag3 = shouldLoopAutomatically(p_sound);
                ...
                if (channelaccess$channelhandle == null) {
                    ...
                    if (!flag1) {
                        this.soundBuffers.getCompleteBuffer(sound.getPath()).thenAccept(p_446164_ -> channelaccess$channelhandle.execute(p_194495_ -> {
                            p_194495_.attachStaticBuffer(p_446164_);
                            p_194495_.play();
                        ...
                        soundinstance.getStream(this.soundBuffers, sound, flag3).thenAccept(p_446207_ -> channelaccess$channelhandle.execute(p_194498_ -> {
                            p_194498_.attachBufferStream(p_446207_);
                            p_194498_.play();
    ...
    private float calculateVolume(float volumeMultiplier, SoundSource source) {
        return Mth.clamp(volumeMultiplier, 0.0F, 1.0F)
    ...
    public void updateSource(Camera renderInfo) {
        if (this.loaded && renderInfo.isInitialized()) {
            ListenerTransform listenertransform = new ListenerTransform(
                renderInfo.position(), new Vec3(renderInfo.forwardVector()), new Vec3(renderInfo.upVector())
            );
            this.executor.execute(() -> this.listener.setTransform(listenertransform));
        }
    }

    public void stop(@Nullable Identifier soundName, @Nullable SoundSource category) {
        if (category != null) {
            for (SoundInstance soundinstance : this.instanceBySource.get(category)) {
                if (soundName == null || soundinstance.getIdentifier().equals(soundName)) {
            ...
            for (SoundInstance soundinstance1 : this.instanceToChannel.keySet()) {
                if (soundinstance1.getIdentifier().equals(soundName)) {
