...
public class WanderingTraderSpawner implements CustomSpawner {
    ...
    @Override
    public void tick(ServerLevel p_480006_, boolean p_480449_) {
        if (p_480006_.getGameRules().get(GameRules.SPAWN_WANDERING_TRADERS)) {
            if (--this.tickDelay <= 0) {
                this.tickDelay = 1200;
    ...
    private boolean spawn(ServerLevel serverLevel) {
        ...
        if (player == null) {
            ...
            Optional<BlockPos> optional = poimanager.find(
                p_477957_ -> p_477957_.is(PoiTypes.MEETING), p_479947_ -> true, blockpos, 48, PoiManager.Occupancy.ANY
            );
