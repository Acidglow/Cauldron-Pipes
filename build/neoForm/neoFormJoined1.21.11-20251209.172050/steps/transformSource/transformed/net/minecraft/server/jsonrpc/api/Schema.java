...
public record Schema<T>(
    ...
    public static final Codec<? extends Schema<?>> CODEC = Codec.<Schema<?>>recursive(
            "Schema",
            p_451634_ -> RecordCodecBuilder.create(
                p_457376_ -> p_457376_.group(
                        ...
                        p_451634_.optionalFieldOf("items").forGetter(Schema::items),
                        Codec.unboundedMap(Codec.STRING, p_451634_)
                    ...
                    .apply(p_457376_, (p_457377_, p_457378_, p_457379_, p_457380_, p_457381_) -> null)
            )
        )
        .validate(p_457382_ -> p_457382_ == null ? DataResult.error(() -> "Should not deserialize schema") : DataResult.success(p_457382_));
    ...
    public Schema<T> info() {
        return new Schema<>(
            ...
            this.properties.entrySet().stream().collect(Collectors.toMap(Entry::getKey, p_457374_ -> p_457374_.getValue().info())),
            this.enumValues,
            this.codec
        );
    }
    ...
    public static <E extends Enum<E> & StringRepresentable> Schema<E> ofEnum(Supplier<E[]> enumValues) {
        return ofEnum(enumValues, StringRepresentable.fromEnum(enumValues));
    }

    public static <E extends Enum<E> & StringRepresentable> Schema<E> ofEnum(Supplier<E[]> enumValues, Codec<E> codec) {
        List<String> list = Stream.<Enum>of((Enum[])enumValues.get()).map(p_442864_ -> ((StringRepresentable)p_442864_).getSerializedName()).toList();
