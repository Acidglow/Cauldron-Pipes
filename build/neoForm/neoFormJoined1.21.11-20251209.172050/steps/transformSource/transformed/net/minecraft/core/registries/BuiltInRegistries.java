...
public class BuiltInRegistries {
    ...
    public static final Registry<Identifier> CUSTOM_STAT = registerSimple(Registries.CUSTOM_STAT, p_465951_ -> Stats.JUMP);
    ...
    public static final Registry<ConsumeEffect.Type<?>> CONSUME_EFFECT_TYPE = registerSimple(
        Registries.CONSUME_EFFECT_TYPE, p_366371_ -> ConsumeEffect.Type.APPLY_EFFECTS
    );
    ...
    public static final Registry<TicketType> TICKET_TYPE = registerSimple(Registries.TICKET_TYPE, p_392595_ -> TicketType.UNKNOWN);
    ...
    public static final Registry<OutgoingRpcMethod<?, ?>> OUTGOING_RPC_METHOD = registerSimple(
        Registries.OUTGOING_RPC_METHOD, p_450766_ -> OutgoingRpcMethods.SERVER_STARTED
    );
    ...
    private static <T> Registry<T> registerSimpleWithIntrusiveHolders(
        ResourceKey<? extends Registry<T>> key, BuiltInRegistries.RegistryBootstrap<T> bootstrap
    ) {
        return internalRegister(key, new MappedRegistry<>(key, Lifecycle.stable(), true), bootstrap);
    }

    private static <T> DefaultedRegistry<T> registerDefaulted(
        ResourceKey<? extends Registry<T>> key, String defaultKey, BuiltInRegistries.RegistryBootstrap<T> bootstrap
    ) {
        return internalRegister(key, new DefaultedMappedRegistry<>(defaultKey, key, Lifecycle.stable(), false), bootstrap);
    }

    private static <T> DefaultedRegistry<T> registerDefaultedWithIntrusiveHolders(
        ResourceKey<? extends Registry<T>> key, String defaultKey, BuiltInRegistries.RegistryBootstrap<T> bootstrap
    ) {
        return internalRegister(key, new DefaultedMappedRegistry<>(defaultKey, key, Lifecycle.stable(), true), bootstrap);
    }
    ...
    private static void createContents() {
        LOADERS.forEach((p_469261_, p_259387_) -> {
            if (p_259387_.get() == null) {
                LOGGER.error("Unable to bootstrap registry '{}'", p_469261_);
            }
        });
    }
    ...
    private static <T extends Registry<?>> void validate(Registry<T> registry) {
        registry.forEach(p_465954_ -> {
            if (p_465954_.keySet().isEmpty()) {
                Util.logAndPauseIfInIde("Registry '" + registry.getKey((T)p_465954_) + "' was empty after loading");
            }

            if (p_465954_ instanceof DefaultedRegistry) {
                Identifier identifier = ((DefaultedRegistry)p_465954_).getDefaultKey();
                Objects.requireNonNull(p_465954_.getValue(identifier), "Missing default of DefaultedMappedRegistry: " + identifier);
            }
        });
    }
