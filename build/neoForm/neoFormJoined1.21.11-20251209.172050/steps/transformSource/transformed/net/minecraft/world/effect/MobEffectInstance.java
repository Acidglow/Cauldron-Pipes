...
public class MobEffectInstance implements Comparable<MobEffectInstance> {
    ...
    public MobEffectInstance withScaledDuration(float scale) {
        ...
        mobeffectinstance.duration = mobeffectinstance.mapDuration(p_392957_ -> Math.max(Mth.floor(p_392957_ * scale), 1));
    ...
    public boolean tickServer(ServerLevel level, LivingEntity entity, Runnable onEffectUpdated) {
        if (!this.hasRemainingDuration()) {
            ...
            if (this.effect.value().shouldApplyEffectTickThisTick(i, this.amplifier)
                && !this.effect.value().applyEffectTick(level, entity, this.amplifier)) {
                return false;
            } else {
