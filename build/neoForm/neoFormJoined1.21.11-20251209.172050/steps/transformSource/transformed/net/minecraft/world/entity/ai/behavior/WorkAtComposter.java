...
public class WorkAtComposter extends WorkAtPoi {
    ...
    @Override
    protected void useWorkstation(ServerLevel p_24790_, Villager p_479014_) {
        Optional<GlobalPos> optional = p_479014_.getBrain().getMemory(MemoryModuleType.JOB_SITE);
        if (!optional.isEmpty()) {
            GlobalPos globalpos = optional.get();
            BlockState blockstate = p_24790_.getBlockState(globalpos.pos());
            if (blockstate.is(Blocks.COMPOSTER)) {
                this.makeBread(p_24790_, p_479014_);
                this.compostItems(p_24790_, p_479014_, globalpos, blockstate);
            }
        }
    }

    private void compostItems(ServerLevel level, Villager villager, GlobalPos pos, BlockState state) {
