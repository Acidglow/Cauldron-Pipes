...
public class BellModel extends Model<BellModel.State> {
    ...
    public void setupAnim(BellModel.State p_478655_) {
        super.setupAnim(p_478655_);
        float f = 0.0F;
        float f1 = 0.0F;
        if (p_478655_.shakeDirection != null) {
            float f2 = Mth.sin(p_478655_.ticks / (float) Math.PI) / (4.0F + p_478655_.ticks / 3.0F);
            switch (p_478655_.shakeDirection) {
                case NORTH:
                    f = -f2;
                    break;
                case SOUTH:
