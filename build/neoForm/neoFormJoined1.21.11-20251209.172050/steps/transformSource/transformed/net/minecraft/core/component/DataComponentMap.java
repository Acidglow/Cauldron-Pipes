...
public interface DataComponentMap extends Iterable<TypedDataComponent<?>>, DataComponentGetter {
    DataComponentMap EMPTY = new DataComponentMap() {
        @Override
        public <T> @Nullable T get(DataComponentType<? extends T> p_331168_) {
            return null;
        }
    ...
    static DataComponentMap composite(final DataComponentMap map1, final DataComponentMap map2) {
        return new DataComponentMap() {
            @Override
            public <T> @Nullable T get(DataComponentType<? extends T> p_330291_) {
    ...
    default Iterator<TypedDataComponent<?>> iterator() {
        return Iterators.transform(this.keySet().iterator(), p_396325_ -> Objects.requireNonNull(this.getTyped((DataComponentType<?>)p_396325_)));
    }
    ...
    default DataComponentMap filter(final Predicate<DataComponentType<?>> predicate) {
        return new DataComponentMap() {
            @Override
            public <T> @Nullable T get(DataComponentType<? extends T> p_341052_) {
    ...
    public static class Builder implements net.neoforged.neoforge.common.extensions.IDataComponentMapBuilderExtensions {
        ...
        record SimpleMap(Reference2ObjectMap<DataComponentType<?>, Object> map) implements DataComponentMap {
            @Override
            public <T> @Nullable T get(DataComponentType<? extends T> p_331063_) {
