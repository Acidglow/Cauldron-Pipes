...
public class JumpableVehicleBarRenderer implements ContextualBarRenderer {
    ...
    public void renderBackground(GuiGraphics p_415798_, DeltaTracker p_415750_) {
        ...
        p_415798_.blitSprite(RenderPipelines.GUI_TEXTURED, JUMP_BAR_BACKGROUND_SPRITE, i, j, 182, 5);
        if (this.playerJumpableVehicle.getJumpCooldown() > 0) {
            p_415798_.blitSprite(RenderPipelines.GUI_TEXTURED, JUMP_BAR_COOLDOWN_SPRITE, i, j, 182, 5);
        } else {
            int k = Mth.lerpDiscrete(this.minecraft.player.getJumpRidingScale(), 0, 182);
            if (k > 0) {
                p_415798_.blitSprite(RenderPipelines.GUI_TEXTURED, JUMP_BAR_PROGRESS_SPRITE, 182, 5, 0, 0, i, j, k, 5);
            }
        }
    }
