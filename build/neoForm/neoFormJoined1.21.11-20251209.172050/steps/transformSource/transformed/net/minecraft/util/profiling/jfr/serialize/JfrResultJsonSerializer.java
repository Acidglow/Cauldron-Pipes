...
public class JfrResultJsonSerializer {
    ...
    private JsonElement structureGen(List<StructureGenStat> stats) {
        ...
        if (optional.isEmpty()) {
            ...
            stats.stream()
                ...
                .forEach(
                    (p_466502_, p_466503_) -> {
                        Optional<TimedStatSummary<StructureGenStat>> optional1 = TimedStatSummary.summary((List<StructureGenStat>)p_466503_);
                        if (!optional1.isEmpty()) {
                            TimedStatSummary<StructureGenStat> timedstatsummary1 = optional1.get();
                            JsonObject jsonobject1 = new JsonObject();
                            jsonarray.add(jsonobject1);
                            jsonobject1.addProperty("name", p_466502_);
                            ...
                            JsonObject jsonobject2 = Util.make(new JsonObject(), p_382693_ -> jsonobject1.add("durationNanosPercentiles", p_382693_));
                            timedstatsummary1.percentilesNanos().forEach((p_454454_, p_454455_) -> jsonobject2.addProperty("p" + p_454454_, p_454455_));
                            Function<StructureGenStat, JsonElement> function = p_382691_ -> {
                                JsonObject jsonobject3 = new JsonObject();
                                jsonobject3.addProperty("durationNanos", p_382691_.duration().toNanos());
                                jsonobject3.addProperty("chunkPosX", p_382691_.chunkPos().x);
                                jsonobject3.addProperty("chunkPosZ", p_382691_.chunkPos().z);
                                jsonobject3.addProperty("structureName", p_382691_.structureName());
                                jsonobject3.addProperty("level", p_382691_.level());
                                jsonobject3.addProperty("success", p_382691_.success());
                                return jsonobject3;
                            };
    ...
    private JsonElement chunkGen(List<Pair<ChunkStatus, TimedStatSummary<ChunkGenStat>>> summary) {
        ...
        if (summary.isEmpty()) {
            ...
            jsonobject.addProperty("durationNanosTotal", summary.stream().mapToDouble(p_185567_ -> p_185567_.getSecond().totalDuration().toNanos()).sum());
    ...
    private JsonElement threadAllocations(ThreadAllocationStat.Summary summary) {
        JsonArray jsonarray = new JsonArray();
        summary.allocationsPerSecondByThread().forEach((p_466497_, p_466498_) -> jsonarray.add(Util.make(new JsonObject(), p_185571_ -> {
            p_185571_.addProperty("thread", p_466497_);
            p_185571_.addProperty("bytesPerSecond", p_466498_);
        })));
        return jsonarray;
    }
    ...
    private JsonElement serverTicks(List<TickTimeStat> stats) {
        if (stats.isEmpty()) {
            ...
            JsonObject jsonobject = new JsonObject();
            double[] adouble = stats.stream().mapToDouble(p_185548_ -> p_185548_.currentAverage().toNanos() / 1000000.0).toArray();
            ...
            map.forEach((p_382695_, p_382696_) -> jsonobject.addProperty("p" + p_382695_, p_382696_));
            return jsonobject;
        }
    }
    ...
    private JsonElement fps(List<FpsStat> stats) {
        if (stats.isEmpty()) {
            ...
            JsonObject jsonobject = new JsonObject();
            int[] aint = stats.stream().mapToInt(FpsStat::fps).toArray();
