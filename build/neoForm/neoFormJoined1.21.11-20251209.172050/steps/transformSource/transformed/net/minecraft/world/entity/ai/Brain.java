...
public class Brain<E extends LivingEntity> {
                        ...
                        return memoryTypes.stream()
                            .flatMap(
                                p_22020_ -> p_22020_.getCodec()
                                    .map(p_466546_ -> BuiltInRegistries.MEMORY_MODULE_TYPE.getKey((MemoryModuleType<?>)p_22020_))
                                    .stream()
                            )
                            .map(p_466544_ -> ops.createString(p_466544_.toString()));
                    }
                            ...
                            .forEach(
                                p_460407_ -> {
                                    DataResult<MemoryModuleType<?>> dataresult = BuiltInRegistries.MEMORY_MODULE_TYPE
                                        .byNameCodec()
                                        .parse(ops, p_460407_.getFirst());
                                    DataResult<? extends Brain.MemoryValue<?>> dataresult1 = dataresult.flatMap(
                                        p_147350_ -> this.captureRead((MemoryModuleType<T>)p_147350_, ops, (T)p_460407_.getSecond())
                                    );
    ...
    public void updateActivityFromSchedule(EnvironmentAttributeSystem attributes, long time, Vec3 pos) {
        if (time - this.lastScheduleUpdate > 20L) {
            this.lastScheduleUpdate = time;
            Activity activity = this.schedule != null ? attributes.getValue(this.schedule, pos) : Activity.IDLE;
