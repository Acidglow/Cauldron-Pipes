...
public class LocatorBarRenderer implements ContextualBarRenderer {
    ...
    public void render(GuiGraphics p_415579_, DeltaTracker p_416560_) {
        ...
        if (entity != null) {
            ...
            PartialTickSupplier partialticksupplier = p_437152_ -> p_416560_.getGameTimeDeltaPartialTick(!tickratemanager.isEntityFrozen(p_437152_));
            this.minecraft
                .player
                .connection
                .getWaypointManager()
                .forEachWaypoint(
                    entity,
                    p_465465_ -> {
                        if (!p_465465_.id().left().map(p_437154_ -> p_437154_.equals(entity.getUUID())).orElse(false)) {
                            double d0 = p_465465_.yawAngleToCamera(level, this.minecraft.gameRenderer.getMainCamera(), partialticksupplier);
                            if (!(d0 <= -60.0) && !(d0 > 60.0)) {
                                int j = Mth.ceil((p_415579_.guiWidth() - 9) / 2.0F);
                                Waypoint.Icon waypoint$icon = p_465465_.icon();
                                WaypointStyle waypointstyle = this.minecraft.getWaypointStyles().get(waypoint$icon.style);
                                float f = Mth.sqrt((float)p_465465_.distanceSquared(entity));
                                ...
                                p_415579_.blitSprite(RenderPipelines.GUI_TEXTURED, identifier, j + l, i - 2, 9, 9, k);
                                TrackedWaypoint.PitchDirection trackedwaypoint$pitchdirection = p_465465_.pitchDirectionToCamera(
                                ...
                                if (trackedwaypoint$pitchdirection != TrackedWaypoint.PitchDirection.NONE) {
                                    ...
                                    p_415579_.blitSprite(RenderPipelines.GUI_TEXTURED, identifier1, j + l + 1, i + i1, 7, 5);
                                }
                            }
                        }
                    }
