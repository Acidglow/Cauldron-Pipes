...
public class SoundBufferLibrary {
    ...
    public CompletableFuture<SoundBuffer> getCompleteBuffer(Identifier soundID) {
        return this.cache.computeIfAbsent(soundID, p_465795_ -> CompletableFuture.supplyAsync(() -> {
            try {
                SoundBuffer soundbuffer;
                try (
                    InputStream inputstream = this.resourceManager.open(p_465795_);
    ...
    public CompletableFuture<?> preload(Collection<Sound> sounds) {
        return CompletableFuture.allOf(sounds.stream().map(p_465799_ -> this.getCompleteBuffer(p_465799_.getPath())).toArray(CompletableFuture[]::new));
    }
}
