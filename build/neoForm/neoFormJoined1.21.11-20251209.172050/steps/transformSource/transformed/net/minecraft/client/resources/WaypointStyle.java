...
public record WaypointStyle(int nearDistance, int farDistance, List<Identifier> sprites, List<Identifier> spriteLocations) {
    ...
    public static final Codec<WaypointStyle> CODEC = RecordCodecBuilder.<WaypointStyle>create(
            p_465757_ -> p_465757_.group(
                ...
                .apply(p_465757_, WaypointStyle::new)
        )
        .validate(WaypointStyle::validate);

    public WaypointStyle(int p_419759_, int p_419537_, List<Identifier> p_420059_) {
        this(p_419759_, p_419537_, p_420059_, p_420059_.stream().map(p_465756_ -> p_465756_.withPrefix("hud/locator_bar_dot/")).toList());
    }
    ...
    public Identifier sprite(float distance) {
        if (distance < this.nearDistance) {
            ...
            int i = Mth.lerpInt((distance - this.nearDistance) / (this.farDistance - this.nearDistance), 1, this.spriteLocations.size() - 1);
