...
public class ObjectiveCriteria {
    ...
    public static final Codec<ObjectiveCriteria> CODEC = Codec.STRING
        .comapFlatMap(
            p_401000_ -> byName(p_401000_).map(DataResult::success).orElse(DataResult.error(() -> "No scoreboard criteria with name: " + p_401000_)),
        ...
        if (objectivecriteria != null) {
            ...
            return i < 0
                ...
                : BuiltInRegistries.STAT_TYPE
                    .getOptional(Identifier.bySeparator(name.substring(0, i), '.'))
                    .flatMap(p_466816_ -> getStat((StatType<?>)p_466816_, Identifier.bySeparator(name.substring(i + 1), '.')));
        }
    }

    private static <T> Optional<ObjectiveCriteria> getStat(StatType<T> statType, Identifier resourceLocation) {
