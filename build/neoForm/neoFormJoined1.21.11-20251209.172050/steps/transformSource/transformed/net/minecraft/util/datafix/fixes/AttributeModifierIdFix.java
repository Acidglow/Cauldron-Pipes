...
public class AttributeModifierIdFix extends DataFix {
    ...
    protected TypeRewriteRule makeRule() {
        ...
        return TypeRewriteRule.seq(
            this.fixTypeEverywhereTyped(
                ...
                type,
                p_350333_ -> p_350333_.updateTyped(
                    opticfinder, p_350905_ -> p_350905_.update(DSL.remainderFinder(), AttributeModifierIdFix::fixItemStackComponents)
                )
            ),
    ...
    private static Stream<Dynamic<?>> fixModifiers(Stream<Dynamic<?>> modifiers) {
        ...
        modifiers.forEach(p_350336_ -> {
            UUID uuid = uuidFromIntArray(p_350336_.get("uuid").asIntStream().toArray());
            String s = p_350336_.get("name").asString("");
            String s1 = uuid != null ? ID_MAP.get(uuid) : null;
            String s2 = NAME_MAP.get(s);
            if (s1 != null) {
                p_350336_ = p_350336_.set("id", p_350336_.createString(s1));
                map.put(s1, p_350336_.remove("uuid").remove("name"));
            } else if (s2 != null) {
                Dynamic<?> dynamic = map.get(s2);
                if (dynamic == null) {
                    p_350336_ = p_350336_.set("id", p_350336_.createString(s2));
                    map.put(s2, p_350336_.remove("uuid").remove("name"));
                } else {
                    double d0 = dynamic.get("amount").asDouble(0.0);
                    double d1 = p_350336_.get("amount").asDouble(0.0);
                    map.put(s2, dynamic.set("amount", p_350336_.createDouble(d0 + d1)));
                }
            } else {
                String s3 = "minecraft:" + (uuid != null ? uuid.toString().toLowerCase(Locale.ROOT) : "unknown");
                p_350336_ = p_350336_.set("id", p_350336_.createString(s3));
                map.put(s3, p_350336_.remove("uuid").remove("name"));
            }
        });
    ...
    private static Dynamic<?> convertModifierForEntity(Dynamic<?> modifier) {
        return modifier.renameField("UUID", "uuid")
            ...
            .renameAndFixField("Operation", "operation", p_350969_ -> {
                return p_350969_.createString(switch (p_350969_.asInt(0)) {
                    case 0 -> "add_value";
    ...
    private static Dynamic<?> fixItemStackComponents(Dynamic<?> tag) {
        return tag.update(
            ...
            p_350851_ -> p_350851_.update(
                "modifiers",
                p_350352_ -> DataFixUtils.orElse(
                    p_350352_.asStreamOpt().result().map(AttributeModifierIdFix::fixModifiersTypeWrapper).map(p_350352_::createList), p_350352_
                )
            )
        );
    }
    ...
    private static Dynamic<?> fixAttribute(Dynamic<?> tag) {
        return tag.renameField("Name", "id")
            ...
            .renameAndFixField(
                "Modifiers",
                "modifiers",
                p_350892_ -> DataFixUtils.orElse(
                    p_350892_.asStreamOpt()
                        .result()
                        .map(p_350343_ -> p_350343_.map(AttributeModifierIdFix::convertModifierForEntity))
    ...
    private static Typed<?> fixEntity(Typed<?> entity) {
        return entity.update(
            DSL.remainderFinder(),
            p_350693_ -> p_350693_.renameAndFixField(
                "Attributes",
                "attributes",
                p_350587_ -> DataFixUtils.orElse(
                    p_350587_.asStreamOpt().result().map(p_350341_ -> p_350341_.map(AttributeModifierIdFix::fixAttribute)).map(p_350587_::createList),
                    p_350587_
                )
            )
        );
    ...
    public static @Nullable UUID uuidFromIntArray(int[] intArray) {
        return intArray.length != 4
            ? null
            : new UUID((long)intArray[0] << 32 | intArray[1] & 4294967295L, (long)intArray[2] << 32 | intArray[3] & 4294967295L);
    }
}
