...
public interface Container extends Clearable, SlotProvider, Iterable<ItemStack>, net.neoforged.neoforge.common.extensions.ContainerExtension {
    ...
    static boolean stillValidBlockEntity(BlockEntity blockEntity, Player player, float distance) {
        ...
        if (level == null) {
            ...
            return level.getBlockEntity(blockpos) != blockEntity ? false : player.isWithinBlockInteractionRange(blockpos, distance);
        }
    }

    @Override
    default @Nullable SlotAccess getSlot(final int p_460691_) {
        return p_460691_ >= 0 && p_460691_ < this.getContainerSize() ? new SlotAccess() {
            @Override
            public ItemStack get() {
                return Container.this.getItem(p_460691_);
            }

            @Override
            public boolean set(ItemStack p_461080_) {
                Container.this.setItem(p_460691_, p_461080_);
                return true;
            }
        } : null;
    }
