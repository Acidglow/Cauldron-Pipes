...
public class DebugSubscriptions<T> {
    ...
    private static <T> DebugSubscription<T> registerWithValue(String name, StreamCodec<? super RegistryFriendlyByteBuf, T> streamCodec) {
        return Registry.register(BuiltInRegistries.DEBUG_SUBSCRIPTION, Identifier.withDefaultNamespace(name), new DebugSubscription<>(streamCodec));
    }

    private static <T> DebugSubscription<T> registerTemporaryValue(String name, StreamCodec<? super RegistryFriendlyByteBuf, T> streamCodec, int expireAfterTicks) {
        return Registry.register(
            BuiltInRegistries.DEBUG_SUBSCRIPTION, Identifier.withDefaultNamespace(name), new DebugSubscription<>(streamCodec, expireAfterTicks)
        );
    }
}
