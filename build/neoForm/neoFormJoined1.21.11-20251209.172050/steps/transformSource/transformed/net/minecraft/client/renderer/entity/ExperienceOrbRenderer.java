...
public class ExperienceOrbRenderer extends EntityRenderer<ExperienceOrb, ExperienceOrbRenderState> {
    ...
    public void submit(ExperienceOrbRenderState p_451085_, PoseStack p_435966_, SubmitNodeCollector p_435813_, CameraRenderState p_451215_) {
        p_435966_.pushPose();
        int i = p_451085_.icon;
        ...
        float f6 = 0.25F;
        float f7 = 255.0F;
        float f8 = p_451085_.ageInTicks / 2.0F;
        int j = (int)((Mth.sin(f8 + 0.0F) + 1.0F) * 0.5F * 255.0F);
        int k = 255;
        int l = (int)((Mth.sin(f8 + (float) (Math.PI * 4.0 / 3.0)) + 1.0F) * 0.1F * 255.0F);
        p_435966_.translate(0.0F, 0.1F, 0.0F);
        p_435966_.mulPose(p_451215_.orientation);
        float f9 = 0.3F;
        p_435966_.scale(0.3F, 0.3F, 0.3F);
        p_435813_.submitCustomGeometry(p_435966_, RENDER_TYPE, (p_433894_, p_433372_) -> {
            vertex(p_433372_, p_433894_, -0.5F, -0.25F, j, 255, l, f, f3, p_451085_.lightCoords);
            vertex(p_433372_, p_433894_, 0.5F, -0.25F, j, 255, l, f1, f3, p_451085_.lightCoords);
            vertex(p_433372_, p_433894_, 0.5F, 0.75F, j, 255, l, f1, f2, p_451085_.lightCoords);
            vertex(p_433372_, p_433894_, -0.5F, 0.75F, j, 255, l, f, f2, p_451085_.lightCoords);
        });
        p_435966_.popPose();
        super.submit(p_451085_, p_435966_, p_435813_, p_451215_);
    }
