...
public class ClientDebugSubscriber {
    ...
    private void onSubscriptionsChanged(Set<DebugSubscription<?>> subscriptions) {
        ...
        this.initializeSubscriptions(subscriptions);
        this.connection.send(new ServerboundDebugSubscriptionRequestPacket(subscriptions));
    }

    private void initializeSubscriptions(Set<DebugSubscription<?>> subscriptions) {
    ...
    <V> ClientDebugSubscriber.@Nullable ValueMaps<V> getValueMaps(DebugSubscription<V> subscription) {
    ...
    private <K, V> ClientDebugSubscriber.@Nullable ValueMap<K, V> getValueMap(
        DebugSubscription<V> subscription, ClientDebugSubscriber.ValueMapType<K, V> type
    ) {
        ClientDebugSubscriber.ValueMaps<V> valuemaps = this.getValueMaps(subscription);
        return valuemaps != null ? type.get(valuemaps) : null;
    }

    <K, V> @Nullable V getValue(DebugSubscription<V> subscription, K key, ClientDebugSubscriber.ValueMapType<K, V> type) {
    ...
    public DebugValueAccess createDebugValueAccess(final Level level) {
        return new DebugValueAccess() {
            ...
            @Override
            public <T> @Nullable T getChunkValue(DebugSubscription<T> p_449512_, ChunkPos p_449631_) {
            ...
            @Override
            public <T> @Nullable T getBlockValue(DebugSubscription<T> p_449946_, BlockPos p_449548_) {
            ...
            @Override
            public <T> @Nullable T getEntityValue(DebugSubscription<T> p_449411_, Entity p_449503_) {
