...
public class TextureSlots {
    ...
    public static TextureSlots.Data parseTextureMap(JsonObject textureMap) {
        TextureSlots.Data.Builder textureslots$data$builder = new TextureSlots.Data.Builder();

        for (Entry<String, JsonElement> entry : textureMap.entrySet()) {
    ...
    private static void parseEntry(String name, String texture, TextureSlots.Data.Builder builder) {
        if (isTextureReference(texture)) {
            builder.addReference(name, texture.substring(1));
        } else {
            ...
            builder.addTexture(name, new Material(ModelManager.BLOCK_OR_ITEM, identifier));
        }
    }
