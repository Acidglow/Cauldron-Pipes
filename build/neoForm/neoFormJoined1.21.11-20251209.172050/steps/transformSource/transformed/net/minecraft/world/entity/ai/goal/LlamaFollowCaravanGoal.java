...
public class LlamaFollowCaravanGoal extends Goal {
    ...
    private boolean firstIsLeashed(Llama llama, int recursionDepth) {
        if (recursionDepth > 8) {
            return false;
        } else if (llama.inCaravan()) {
            return llama.getCaravanHead().isLeashed() ? true : this.firstIsLeashed(llama.getCaravanHead(), ++recursionDepth);
        } else {
            return false;
        }
    }
