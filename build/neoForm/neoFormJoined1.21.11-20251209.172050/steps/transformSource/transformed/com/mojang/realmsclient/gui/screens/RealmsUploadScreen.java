...
public class RealmsUploadScreen extends RealmsScreen implements RealmsWorldUploadStatusTracker {
    ...
    @Override
    public boolean keyPressed(KeyEvent p_446380_) {
        if (p_446380_.key() == 256) {
            if (this.showDots) {
                this.onCancel();
            } else {
                this.onBack();
            ...
            return super.keyPressed(p_446380_);
        }
    }
    ...
    public void render(GuiGraphics p_282140_, int p_90097_, int p_90098_, float p_90099_) {
        ...
        if (this.showDots) {
            p_282140_.drawString(this.font, DOTS[this.tickCount / 10 % DOTS.length], this.width / 2 + this.font.width(this.status) / 2 + 5, 50, -1);
        }
    ...
    private void drawUploadSpeed0(GuiGraphics guiGraphics, long bytesPerSecond) {
        ...
        if (bytesPerSecond > 0L && s != null) {
            ...
            guiGraphics.drawString(this.font, s1, this.width / 2 + i / 2 + 15, 84, -1);
        }
    }
    ...
    private void upload() {
        ...
        RealmsSlot realmsslot = new RealmsSlot(
            this.slotId, realmsworldoptions, List.of(RealmsSetting.hardcoreSetting(this.selectedLevel.getSettings().hardcore()))
        );
        ...
        if (!this.currentUpload.compareAndSet(null, realmsworldupload)) {
            ...
            realmsworldupload.packAndUpload().handleAsync((p_373651_, p_373652_) -> {
                if (p_373652_ != null) {
                    if (p_373652_ instanceof CompletionException completionexception) {
                        p_373652_ = completionexception.getCause();
                    }

                    if (p_373652_ instanceof RealmsUploadException realmsuploadexception) {
                        ...
                        this.status = Component.translatable("mco.upload.failed", p_373652_.getMessage());
                    }
                } else {
    ...
    private void setErrorMessage(Component @Nullable ... errorMessage) {
