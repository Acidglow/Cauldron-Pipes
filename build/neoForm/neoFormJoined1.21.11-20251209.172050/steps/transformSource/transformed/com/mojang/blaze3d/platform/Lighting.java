...
public class Lighting implements AutoCloseable {
    ...
    public Lighting() {
        ...
        this.updateBuffer(
            ...
            matrix4f.transformDirection(DIFFUSE_LIGHT_0, new Vector3f()),
            matrix4f.transformDirection(DIFFUSE_LIGHT_1, new Vector3f())
        );
        ...
        this.updateBuffer(
            ...
            matrix4f1.transformDirection(DIFFUSE_LIGHT_0, new Vector3f()),
            matrix4f1.transformDirection(DIFFUSE_LIGHT_1, new Vector3f())
        );
        ...
        this.updateBuffer(
            ...
            matrix4f2.transformDirection(INVENTORY_DIFFUSE_LIGHT_0, new Vector3f()),
            matrix4f2.transformDirection(INVENTORY_DIFFUSE_LIGHT_1, new Vector3f())
        );
    }
    ...
    private void updateBuffer(Lighting.Entry entry, Vector3f light0, Vector3f light1) {
        try (MemoryStack memorystack = MemoryStack.stackPush()) {
            ByteBuffer bytebuffer = Std140Builder.onStack(memorystack, UBO_SIZE).putVec3(light0).putVec3(light1).get();
            RenderSystem.getDevice()
                .createCommandEncoder()
                .writeToBuffer(this.buffer.slice(entry.ordinal() * this.paddedSize, this.paddedSize), bytebuffer);
        }
    }
