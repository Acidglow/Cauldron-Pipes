...
public class RealmsSelectFileToUploadScreen extends RealmsScreen {
    ...
    public void init() {
        ...
        this.searchBox.setResponder(p_438701_ -> {
            if (this.list != null) {
                this.list.updateFilter(p_438701_);
            }
        });
        ...
        this.uploadButton = linearlayout1.addChild(
            Button.builder(Component.translatable("mco.upload.button.name"), p_438705_ -> this.list.getSelectedOpt().ifPresent(this::upload)).build()
        );
        linearlayout1.addChild(Button.builder(CommonComponents.GUI_BACK, p_438702_ -> this.onClose()).build());
        this.updateButtonState(null);
        this.layout.visitWidgets(p_438704_ -> {
            AbstractWidget abstractwidget = this.addRenderableWidget(p_438704_);
        });
    ...
    private void updateButtonState(@Nullable LevelSummary summary) {
        if (this.list != null && this.uploadButton != null) {
            this.uploadButton.active = this.list.getSelected() != null;
        }
    }

    private void upload(WorldSelectionList.WorldListEntry entry) {
        this.minecraft.setScreen(new RealmsUploadScreen(this.realmCreationTask, this.realmId, this.slotId, this.lastScreen, entry.getLevelSummary()));
    }
