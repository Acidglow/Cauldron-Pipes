...
public class GraphicsWorkarounds {
    ...
    private GraphicsWorkarounds(GpuDevice device) {
        ...
        this.isGlOnDx12 = isGlOnDx12(device);
        this.isAmd = isAmd(device);
    }
    ...
    private static boolean isIntelGen11(GpuDevice device) {
        ...
        String s1 = device.getRenderer().toLowerCase(Locale.ROOT);
    ...
    private static boolean isGlOnDx12(GpuDevice device) {
        boolean flag = Util.getPlatform() == Util.OS.WINDOWS && Util.isAarch64();
        return flag || device.getRenderer().startsWith("D3D12");
    }

    private static boolean isAmd(GpuDevice device) {
