...
class RealmsWorldsTab extends GridLayoutTab implements RealmsConfigurationTab {
    ...
    RealmsWorldsTab(RealmsConfigureWorldScreen configurationScreen, Minecraft minecraft, RealmsServer serverData) {
        ...
        this.optionsButton = gridlayout$rowhelper2.addChild(
            Button.builder(
                    ...
                    p_454120_ -> minecraft.setScreen(
                        new RealmsSlotOptionsScreen(configurationScreen, serverData.slots.get(serverData.activeSlot).copy(), serverData.worldType, serverData.activeSlot)
                    )
                )
                .bounds(0, 0, 150, 20)
                .build()
        ...
        this.backupButton = gridlayout$rowhelper2.addChild(
            Button.builder(
                    ...
                    p_454116_ -> minecraft.setScreen(new RealmsBackupScreen(configurationScreen, serverData.copy(), serverData.activeSlot))
                )
                .bounds(0, 0, 150, 20)
                .build()
        );
    ...
    public void updateData(RealmsServer p_419949_) {
        ...
        this.resetWorldButton.active = !p_419949_.expired;
        if (this.isMinigame()) {
            this.resetWorldButton.setMessage(Component.translatable("mco.configure.world.buttons.switchminigame"));
        } else {
            boolean flag = p_419949_.slots.containsKey(p_419949_.activeSlot) && p_419949_.slots.get(p_419949_.activeSlot).options.empty;
            if (flag) {
