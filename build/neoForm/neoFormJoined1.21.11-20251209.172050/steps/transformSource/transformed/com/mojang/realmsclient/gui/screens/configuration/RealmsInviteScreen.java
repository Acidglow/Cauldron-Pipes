...
public class RealmsInviteScreen extends RealmsScreen {
    ...
    public void init() {
        ...
        this.inviteButton = linearlayout.addChild(Button.builder(TITLE, p_419525_ -> this.onInvite()).width(200).build());
        ...
        this.layout.visitWidgets(p_419675_ -> {
            AbstractWidget abstractwidget = this.addRenderableWidget(p_419675_);
        });
    ...
    private void onInvite() {
        if (this.inviteButton != null && this.profileName != null) {
            if (StringUtil.isBlank(this.profileName.getValue())) {
            ...
            } else if (this.serverData.players.stream().anyMatch(p_454103_ -> p_454103_.name.equalsIgnoreCase(this.profileName.getValue()))) {
                ...
                CompletableFuture.<Boolean>supplyAsync(() -> this.configureScreen.invitePlayer(i, s), Util.ioPool()).thenAcceptAsync(p_419916_ -> {
                    if (p_419916_) {
