...
public class TextureUtil {
    ...
    public static void writeAsPNG(Path p_path, String filename, GpuTexture texture, int mipLevel, IntUnaryOperator pixelUpdater) {
        ...
        for (int j = 0; j <= mipLevel; j++) {
            i += (long)texture.getFormat().pixelSize() * texture.getWidth(j) * texture.getHeight(j);
        }
    ...
    public static void solidify(NativeImage image) {
        ...
        for (int k2 = 0; k2 < i; k2++) {
            for (int i3 = 0; i3 < j; i3++) {
                int k3 = image.getPixel(k2, i3);
                if (ARGB.alpha(k3) == 0) {
                    image.setPixel(k2, i3, ARGB.color(0, aint[pack(k2, i3, i)]));
                } else {
                    image.setPixel(k2, i3, k3);
                }
            }
        }
    }
    ...
    public static void fillEmptyAreasWithDarkColor(NativeImage image) {
        ...
        for (int i4 = 0; i4 < i; i4++) {
            for (int j4 = 0; j4 < j; j4++) {
                int k4 = image.getPixel(i4, j4);
                if (ARGB.alpha(k4) == 0) {
                    image.setPixel(i4, j4, l3);
                }
            }
        }
    }
