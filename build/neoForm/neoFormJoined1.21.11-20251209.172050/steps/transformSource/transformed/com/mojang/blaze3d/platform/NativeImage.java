...
public final class NativeImage implements AutoCloseable {
    ...
    public static NativeImage read(NativeImage.@Nullable Format format, InputStream textureStream) throws IOException {
    ...
    public static NativeImage read(byte[] bytes) throws IOException {
        ...
        if (i < bytes.length) {
            ByteBuffer bytebuffer1 = MemoryUtil.memAlloc(bytes.length);

            NativeImage nativeimage1;
            try {
                nativeimage1 = putAndRead(bytebuffer1, bytes);
            } finally {
            ...
            try (MemoryStack memorystack1 = MemoryStack.stackPush()) {
                ByteBuffer bytebuffer = memorystack1.malloc(bytes.length);
                nativeimage = putAndRead(bytebuffer, bytes);
            }
    ...
    public static NativeImage read(NativeImage.@Nullable Format format, ByteBuffer textureData) throws IOException {
        if (format != null && !format.supportedByStb()) {
            ...
            try (MemoryStack memorystack = MemoryStack.stackPush()) {
                ...
                nativeimage = new NativeImage(
                    format == null ? NativeImage.Format.getStbFormat(intbuffer2.get(0)) : format, intbuffer.get(0), intbuffer1.get(0), true, i
                );
            }
    ...
    public void setPixelABGR(int x, int y, int color) {
        if (this.format != NativeImage.Format.RGBA) {
            ...
            this.checkAllocated();
            long i = (x + (long)y * this.width) * 4L;
            MemoryUtil.memPutInt(this.pixels + i, color);
        }
    }
        ...
        if (other.format() != this.format) {
            ...
            if (this.width == other.width) {
                ...
                for (int l = 0; l < k; l++) {
                    ...
                    MemoryUtil.memCopy(other.pixels + i1, this.pixels + j1, j);
                }
            }
        }
    }
    ...
    public void copyRect(
        NativeImage target, int xFrom, int yFrom, int xTo, int yTo, int width, int height, boolean mirrorX, boolean mirrorY
    ) {
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                int k = mirrorX ? width - 1 - j : j;
                int l = mirrorY ? height - 1 - i : i;
                int i1 = this.getPixelABGR(xFrom + j, yFrom + i);
                target.setPixelABGR(xTo + k, yTo + l, i1);
            }
        }
    }
    ...
    public void resizeSubRectTo(int x, int y, int width, int height, NativeImage image) {
        ...
        if (image.format() != this.format) {
            ...
            STBImageResize.nstbir_resize_uint8(
                this.pixels + (x + y * this.getWidth()) * i,
                width,
                height,
                this.getWidth() * i,
                image.pixels,
