...
public class InputConstants {
    ...
    public static void setupKeyboardCallbacks(Window window, GLFWKeyCallbackI keyCallback, GLFWCharModsCallbackI charModsCallback) {
        GLFW.glfwSetKeyCallback(window.handle(), keyCallback);
        GLFW.glfwSetCharModsCallback(window.handle(), charModsCallback);
    }

    public static void setupMouseCallbacks(
        Window window, GLFWCursorPosCallbackI cursorPosCallback, GLFWMouseButtonCallbackI mouseButtonCallback, GLFWScrollCallbackI scrollCallback, GLFWDropCallbackI dropCallback
    ) {
        ...
        GLFW.glfwSetScrollCallback(window.handle(), scrollCallback);
        GLFW.glfwSetDropCallback(window.handle(), dropCallback);
    }

    public static void grabOrReleaseMouse(Window window, int inputMode, double mouseX, double mouseY) {
        GLFW.glfwSetCursorPos(window.handle(), mouseX, mouseY);
        GLFW.glfwSetInputMode(window.handle(), 208897, inputMode);
    }
    ...
    public static void updateRawMouseInput(Window window, boolean rawMouseInput) {
        if (isRawMouseInputSupported()) {
            GLFW.glfwSetInputMode(window.handle(), GLFW_RAW_MOUSE_MOTION, rawMouseInput ? 1 : 0);
        }
    }
